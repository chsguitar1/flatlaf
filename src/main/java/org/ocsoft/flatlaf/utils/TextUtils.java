...
public final class TextUtils {
    ...
    public static String getWord(final String text, final int location) {
        ...
        while (wordEnd < text.length() - 1
                && !textSeparators.contains(text
                        .substring(wordEnd, wordEnd + 1))) {
            wordEnd++;
        }
        
        // At the word end
        while (wordStart > 0
                && !textSeparators.contains(text.substring(wordStart - 1,
                        wordStart))) {
            wordStart--;
        }
     ...
     *
     * @param text
     *            text to retrieve the word end index from
     * @param location
     *            word location
    ...
    public static Point parsePoint(final String text, final String separator) {
        ...
        return parts.length == 2 ? new Point(Integer.parseInt(parts[0].trim()),
                Integer.parseInt(parts[1].trim())) : null;
    }
    
    /**
     * Returns text with all control symbols removed. This method works faster
     * than simple replaceAll("\\p{Cntrl}", "").
     *
     * @param text
     *            text to modify
    ...
    public static String shortenText(final String text, final int maxLength,
        ...
        return text.length() <= maxLength ? text : text.substring(0,
                maxLength > 3 && addDots ? maxLength - 3 : maxLength)
                + (addDots ? "..." : "");
    }
     ...
     * @return shortened text
     */
    public static String shortenTextEnd(final String text, final int maxLength,
            final boolean addDots) {
        return text.length() <= maxLength ? text
                : (addDots ? "..." : "")
                        + text.substring(text.length()
                                - (maxLength > 3 && addDots ? maxLength - 3
                                        : maxLength));
    }
    ...
    /**
     * Returns single text combined using list of strings and specified
     * separator.
     *
     * @param list
     *            list to combine into single text
     * @param separator
     *            text parts separator
     * @return single text
     */
    ...
    public static <T> String listToString(final List<T> list,
            final String separator, final TextProvider<T> textProvider) {
    ...
    /**
     * Converts list of enumeration constants into string with list of
     * enumeration constants and returns it.
     *
     * @param enumList
     ...
     */
    public static <E extends Enum<E>> String enumListToString(
    ...
    /**
     * Converts string with list of enumeration constants into real list of
     * enumeration constants and returns it.
     *
     * @param enumString
     *            enumeration constants string list
     * @param enumClass
     *            enumeration class
     * @param <E>
     *            enumeration type
     ...
     */
    public static <E extends Enum<E>> List<E> enumStringToList(
            final String enumString, final Class<E> enumClass) {
        final List<E> enumerations;
        if (enumString != null) {
            final StringTokenizer tokenizer = new StringTokenizer(enumString,
                    ",", false);
            enumerations = new ArrayList<E>();
            while (tokenizer.hasMoreTokens()) {
                enumerations.add(Enum.valueOf(enumClass, tokenizer.nextToken()
                        .trim()));
            }
        } else {
    ...
    public static String generateId(final String prefix, final String suffix) {
        return (prefix == null ? defaultIdPrefix : prefix) + "-"
                + generateId(idPartLength) + "-" + generateId(idPartLength)
