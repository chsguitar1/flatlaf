 ...
 * AccessControlContext or use a doPrivileged to resolve the class name. It's
 * intented for use in places in Swing where we need ProxyLazyValue, this should
 * never be used in a place where the developer could supply the arguments.
 *
 * @author Mikle Garin
 */
    ...
    private Class[] getClassArray(final Object[] args) {
        ...
        if (args != null) {
            ...
            for (int i = 0; i < args.length; i++) {
                /*
                 * PENDING(ges): At present only the primitive types used are
                 * handled correctly; this should eventually handle all
                 * primitive types
                 */
                if (args[i] instanceof java.lang.Integer) {
                    ...
                    /*
                     * PENDING(ges) Currently the Reflection APIs do not search
                     * superclasses of parameters supplied for
                     * constructor/method lookup. Since we only have one case
                     * where this is needed, we substitute directly instead of
                     * adding a massive amount of mechanism for this. Eventually
                     * this will probably need to handle the general case as
                     * well.
                     */
