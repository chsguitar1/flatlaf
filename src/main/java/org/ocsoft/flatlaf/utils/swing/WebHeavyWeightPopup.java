 ...
 * same basic methods to manipulate popup window and its settings.
 *
 * @author Mikle Garin
 */

public class WebHeavyWeightPopup extends WebPanel implements
        WindowMethods<JWindow> {
    /**
     * Popup listeners.
     */
    ...
    /**
     * Single animation step progress. Making this value bigger will speedup the
     * animation, reduce required resources but will also make it less soft.
     */
    protected float stepProgress = 0.05f;
    
    /**
     * Popup window display progress. When popup is fully displayed = 1f. When
     * popup is fully hidden = 0f;
     */
    ...
    public WebHeavyWeightPopup(final LayoutManager layout,
            final Painter painter, final Component... components) {
    ...
    /**
     * Shows popup window. Depending on settings it might take a while to
     * animate the show action.
     *
     * @param invoker
     *            invoker component
     * @param location
     ...
     * @return this popup
     */
    public WebHeavyWeightPopup showPopup(final Component invoker,
     ...
     *            invoker component
     * @param x
     *            popup X coordinate relative to invoker
     * @param y
     *            popup Y coordinate relative to invoker
     * @return this popup
     */
    public WebHeavyWeightPopup showPopup(final Component invoker, final int x,
            final int y) {
    ...
    /**
     * Performs popup show operation. Depending on settings it might take a
     * while to animate the show action.
     *
     * @param invoker
     *            invoker component
     * @param x
     ...
     *            popup Y coordinate relative to invoker
     */
    protected void showPopupImpl(final Component invoker, final int x,
            final int y) {
        synchronized (sync) {
            ...
            if (invokerWindow != null) {
                ...
                this.popup = ProprietaryUtils.createHeavyweightPopup(invoker,
                        this, bos.x + x, bos.y + y);
            } else {
                this.popup = ProprietaryUtils.createHeavyweightPopup(invoker,
                        this, x, y);
            }
            ...
            // Creating menu hide mouse event listener (when mouse pressed
            // outside of the menu)
            ...
            // Creating menu hide focus event listener (when focus leaves
            // application)
            ...
            if (animate) {
                showAnimator = WebTimer.repeat(
                        FlatLafStyleConstants.fastAnimationDelay, 0L,
                        new ActionListener() {
                            @Override
    ...
    /**
     * Hides popup window. Depending on settings it might take a while to
     * animate the hide action.
     *
     * @return this popup
     */
    ...
    /**
     * Performs popup hide operation. Depending on settings it might take a
     * while to animate the hide action.
     */
    protected void hidePopupImpl() {
        synchronized (sync) {
            // Ignore action if popup is being hidden or already hidden
            if (hiding || window == null || window != null
                    && !window.isShowing()) {
                return;
            }
    ...
    /**
     * Called with each show animation step performed. This method is a
    ...
    /**
     * Called with each hide animation step performed. This method is a
     * placeholder for overriding classes.
     */
    ...
    /**
     * Performs provided action when menu is fully displayed. Might be useful to
     * display sub-menus or perform some other actions. Be aware that this
     * action will be performed only once and then removed from the actions
     * list.
     *
     * @param action
     *            action to perform
    ...
    /**
     * Performs provided action when menu is fully hidden. Be aware that this
     * action will be performed only once and then removed from the actions
     * list.
     *
     * @param action
     ...
     */
    @Override
    public JWindow center(final Component relativeTo, final int width,
            final int height) {
