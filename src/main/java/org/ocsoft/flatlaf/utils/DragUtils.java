...
public final class DragUtils {
    ...
    public static List<File> getImportedFiles(final Transferable t) {
        ...
        try {
            if (hasURIListFlavor(t.getTransferDataFlavors())) {
                ...
                return textURIListToFileList((String) t
                        .getTransferData(getUriListDataFlavor()));
            }
    ...
    public static List<File> textURIListToFileList(final String data) {
        ...
        for (final StringTokenizer st = new StringTokenizer(data,
                uriListSeparator); st.hasMoreTokens();) {
    ...
    /**
     * Returns whether can pass drop action to closest component parent that has
     * its own TransferHandler. This might be used to make some components that
     * has drag handler transparent for drop actions.
     *
     * @param info
     *            transfer support
     ...
     *
     * @param component
     *            component to pass drop action from
     * @param info
     *            transfer support
     * @return true if drop action succeed, false otherwise
     */
    public static boolean canPassDrop(final Component component,
            final TransferHandler.TransferSupport info) {
        final Container parent = component.getParent();
        if (parent != null && parent instanceof JComponent) {
            final TransferHandler th = ((JComponent) parent)
                    .getTransferHandler();
    ...
    /**
     * Passes drop action to closest component parent that has its own
     * TransferHandler. This might be used to make some components that has drag
     * handler transparent for drop actions.
     *
     * @param info
     *            transfer support
     * @return true if drop action succeed, false otherwise
     */
    public static boolean passDropAction(
            final TransferHandler.TransferSupport info) {
     ...
     * @return true if drop action succeed, false otherwise
     */
    public static boolean passDropAction(final Component component,
            final TransferHandler.TransferSupport info) {
        final Container parent = component.getParent();
        if (parent != null && parent instanceof JComponent) {
            final TransferHandler th = ((JComponent) parent)
                    .getTransferHandler();
            return th != null ? th.importData(info) : passDropAction(parent,
                    info);
        }
        return false;
    }
