...
/**
 * This class provides a set of utilities to work with points and various
 * shapes.
 *
 * @author Mikle Garin
 */
...
public final class GeometryUtils {
    ...
    public static Rectangle getContainingRect(final List<Point> points) {
        return points != null && points.size() > 0 ? getContainingRect(points
                .toArray(new Point[points.size()])) : null;
    }
     ...
     * @param r2
     *            second rectangle
     * @return rectangle containing two others or null if both rectangles are
     *         null
     */
    public static Rectangle getContainingRect(final Rectangle r1,
            final Rectangle r2) {
        if (r1 == null && r2 != null) {
            return r2;
     ...
     * @return rectangle expanded in four directions for the specified values
     */
    public static Rectangle expand(final Rectangle rect, final int top,
            final int left, final int bottom, final int right) {
        return new Rectangle(rect.x - left, rect.y - top, rect.width + left
                + right, rect.height + top + bottom);
    }
    ...
    public static double getAngle(final int x1, final int y1, final int x2,
            final int y2) {
        final double angle = Math.asin((y2 - y1)
                / Math.sqrt(MathUtils.sqr(x2 - x1) + MathUtils.sqr(y2 - y1)));
