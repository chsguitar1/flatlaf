...
/**
 * This class allows you to create key-value table from which you can either
 * request a key by value or a value by key. Both key and value should be unique
 * and adding same key or value will replace existing one in the lists.
 *
 * @author Mikle Garin
 */
...
public class ValuesTable<K, V> implements Serializable {
    ...
    /**
     * Adds a single entry (key-value pair) into this ValuesTable at the
     * specified index.
     *
     * @param index
     *            entry index
    ...
    public void put(int index, final K key, final V value) {
        ...
        if (existingKeyIndex != -1 && existingKeyIndex < index
                && existingValueIndex != -1 && existingValueIndex < index) {
            index -= 2;
        } else if (existingKeyIndex != -1 && existingKeyIndex < index
                || existingValueIndex != -1 && existingValueIndex < index) {
            index--;
        }
    ...
    /**
     * Returns whether this ValuesTable contains entry with specified key or
     * not.
     *
     * @param key
     *            entry key
    ...
    /**
     * Returns whether this ValuesTable contains entry with specified value or
     * not.
     *
     * @param value
     *            entry value
