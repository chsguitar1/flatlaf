 ...
 * applications.
 * <p/>
 * Basically it parses nine-patch image data (patches at the side of .9.png
 * image) into understandable values and uses them to stretch the image properly
 * when it is painted anywhere.
 *
 * @author Mikle Garin
...
public class NinePatchIcon implements Icon {
    ...
    /**
     * Horizontal stretch intervals taken from image patches (top image
     * patches). Note that pixel parts are also included here but marked with
     * "pixel=true" boolean value.
     */
    protected List<NinePatchInterval> horizontalStretch;
    
    /**
     * Vertical stretch intervals taken from image patches (left image patches).
     * Note that pixel parts are also included here but marked with "pixel=true"
     * boolean value.
     */
    protected List<NinePatchInterval> verticalStretch;
    
    /**
     * Content margin taken from image patches (right and bottom patches). This
     * margin is generally valuable for components which uses this icon as a
     * background to set their style margins properly.
     */
    protected Insets margin;
    
    /**
     * Cached fixed areas width of the nine-patch image with additional 1px for
     * each stretchable area.
     */
    ...
    /**
     * Cached fixed areas height of the nine-patch image with additional 1px for
     * each stretchable area.
     */
    ...
    /**
     * Constructs new NinePatchIcon using the nine-patch image from the
     * specified URL.
     *
     * @param url
     ...
     *            component atop of which icon will be stretched
     * @param parsePatches
     *            whether should parse image patches or not
     */
    protected NinePatchIcon(final BufferedImage bufferedImage,
            final Component component, final boolean parsePatches) {
        super();
        ...
        if (parsePatches) {
            ...
            if (bufferedImage.getWidth() < 3 || bufferedImage.getHeight() < 3) {
                throw new IllegalArgumentException(
                        "Buffered image must be atleast 3x3 pixels size");
            }
            ...
            g2d.drawImage(bufferedImage, 0, 0, w, h, 1, 1,
                    bufferedImage.getWidth() - 1,
                    bufferedImage.getHeight() - 1, null);
            g2d.dispose();
            
            // Parsing stretch variables
            horizontalStretch = NinePatchUtils.parseIntervals(bufferedImage,
                    NinePatchIntervalType.horizontalStretch);
            verticalStretch = NinePatchUtils.parseIntervals(bufferedImage,
                    NinePatchIntervalType.verticalStretch);
            
            // Incorrect image
            if (!((horizontalStretch.size() > 1 || horizontalStretch.size() == 1
                    && !horizontalStretch.get(0).isPixel()) && (verticalStretch
                    .size() > 1 || verticalStretch.size() == 1
                    && !verticalStretch.get(0).isPixel()))) {
                throw new IllegalArgumentException(
                        "There must be stretch constraints specified on image");
            }
            
            // Parsing content margins
            final List<NinePatchInterval> vc = NinePatchUtils.parseIntervals(
                    bufferedImage, NinePatchIntervalType.verticalContent);
            final List<NinePatchInterval> hc = NinePatchUtils.parseIntervals(
                    bufferedImage, NinePatchIntervalType.horizontalContent);
            final int top = vc.size() == 0 ? 0 : vc.get(0).getStart();
            final int bottom = vc.size() == 0 ? 0 : rawImage.getHeight()
                    - vc.get(0).getEnd() - 1;
            final int left = hc.size() == 0 ? 0 : hc.get(0).getStart();
            final int right = hc.size() == 0 ? 0 : rawImage.getWidth()
                    - hc.get(0).getEnd() - 1;
    ...
    public void setHorizontalStretch(
            final List<NinePatchInterval> horizontalStretch) {
     ...
     *
     * @param interval
     *            horizontal stretch interval to add
     */
     ...
     *            interval end
     * @param pixel
     *            whether fixed interval or not
     */
    public void addHorizontalStretch(final int start, final int end,
            final boolean pixel) {
     ...
     *            list of vertical stretch intervals
     */
     ...
     *
     * @param interval
     *            vertical stretch interval to add
     */
     ...
     *            whether fixed interval or not
     */
    public void addVerticalStretch(final int start, final int end,
            final boolean pixel) {
    ...
    public Insets getStretchMargin() {
        ...
        final NinePatchInterval bottom = verticalStretch.get(verticalStretch
                .size() - 1);
        final NinePatchInterval right = horizontalStretch.get(horizontalStretch
                .size() - 1);
        return new Insets(top.getLength(), left.getLength(),
                bottom.getLength(), right.getLength());
    }
     ...
     *            icon height
     */
    public void paintIcon(final Graphics2D g2d, final int x, final int y,
            final int width, final int height) {
        final int availableWidth = Math.max(width, getFixedPixelsWidth(true));
        final int availableHeight = Math
                .max(height, getFixedPixelsHeight(true));
        ...
        for (final NinePatchInterval intervalY : verticalStretch) {
            // Percent part height
            final int intervalHeight = intervalY.getEnd()
                    - intervalY.getStart() + 1;
            final int finalHeight;
            if (intervalY.isPixel()) {
                finalHeight = intervalHeight;
            } else {
                final float percents = (float) intervalHeight
                        / (rawImage.getHeight() - fixedPixelsY);
            ...
            for (final NinePatchInterval intervalX : horizontalStretch) {
                // Percent part width
                final int intervalWidth = intervalX.getEnd()
                        - intervalX.getStart() + 1;
                final int finalWidth;
                if (intervalX.isPixel()) {
                    finalWidth = intervalWidth;
                } else {
                    final float percents = (float) intervalWidth
                            / (rawImage.getWidth() - fixedPixelsX);
                    finalWidth = Math.round(percents * unfixedX);
                }
                
                // Drawing image part
                g2d.drawImage(rawImage, currentX, currentY, currentX
                        + finalWidth, currentY + finalHeight,
                        intervalX.getStart(), intervalY.getStart(),
                        intervalX.getStart() + intervalWidth,
                        intervalY.getStart() + intervalHeight, null);
    ...
    public int getIconWidth() {
        return Math.max(component != null ? component.getWidth() : 0,
                getFixedPixelsWidth(true));
    }
    ...
    public int getIconHeight() {
        return Math.max(component != null ? component.getHeight() : 0,
                getFixedPixelsHeight(true));
    }
    ...
    public Dimension getPreferredSize() {
        return new Dimension(getFixedPixelsWidth(true),
                getFixedPixelsHeight(true));
    }
    ...
    public Dimension getRealImageSize() {
        return new Dimension(getRawImage().getWidth(), getRawImage()
                .getHeight());
    }
}