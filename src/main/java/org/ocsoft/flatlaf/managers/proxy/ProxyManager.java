...
/**
 * This manager allows you to quickly manage application's global proxy
 * settings. When activated this manager might cause a proxy chooser or an
 * authentification popup to pop. That will happen in case it will detect that
 * your proxy settings does not match system proxy settings.
 *
 * @author Mikle Garin
 * @see <a
 *      href="https://github.com/mgarin/weblaf/wiki/How-to-use-ProxyManager">How
 *      to use ProxyManager</a>
 * @see org.ocsoft.flatlaf.managers.proxy.ProxySettings
...
public class ProxyManager {
    ...
    public static synchronized void initialize() {
        if (!initialized) {
            ...
            if (autoDetectionEnabled) {
                ...
                if (proxySettings.isUseProxy() != systemProxySettings
                        .isUseProxy()
                        || !CompareUtils.equals(proxySettings.getProxyHost(),
                                systemProxySettings.getProxyHost())
                        || !CompareUtils.equals(proxySettings.getProxyPort(),
                                systemProxySettings.getProxyPort())) {
                    // Checking whether we have proxy confirmation support or
                    // auto settings are enabled
                    if (systemProxyConfirmationSupport == null
                            || isAutoSettingsInitialization()) {
                        // Checking whether we are allowed to use system
                        // settings
                        ...
                        // Asking proxy confirmation support whether we should
                        // use system proxy or leave current settings intact
                        final boolean useSystemProxy = systemProxyConfirmationSupport
                                .shouldUseSystemProxy();
                        if (useSystemProxy) {
                            proxySettings = systemProxySettings;
                        }
                        // Saving the choice if confirmation shouldn't be
                        // pronpted anymore
     ...
     *
     * @param enabled
     *            whether enable auto initialization of proxy settings or not
     */
     ...
     *
     * @return true if auto initialization of proxy settings is enabled, false
     *         otherwise
     */
     ...
     *
     * @param useSystem
     *            whether enable system proxy settings usage or not
     */
    public static void setAlwaysUseSystemSettings(final boolean useSystem) {
        SettingsManager.set(SETTINGS_GROUP, ALWAYS_USE_SYSTEM_SETTINGS,
                useSystem);
    }
    ...
    public static boolean isAlwaysUseSystemSettings() {
        return SettingsManager.get(SETTINGS_GROUP, ALWAYS_USE_SYSTEM_SETTINGS,
                false);
    }
    
    /**
     * Returns current proxy settings or default proxy settings if none
     * installed.
     *
     * @return current proxy settings or default proxy settings if none
     *         installed
     */
    public static ProxySettings getProxySettings() {
        return SettingsManager.get(SETTINGS_GROUP, PROXY_SETTINGS,
                getSystemProxySettings());
    }
    
    /**
     * Installs saved or system proxy settings. This method will install
     * existing proxy settings that were saved before. If old proxy settings
     * could not be found then system proxy settigns will be retrieved and
     * installed.
     */
    ...
    /**
     * Installs proxy settings with specified host and port. If the specified
     * proxy requires authentification user will get prompted.
     *
     * @param host
     *            proxy host
     * @param port
    ...
    /**
     * Installs proxy settings with specified host, port and proxy login and
     * password.
     *
     * @param host
     *            proxy host
     ...
     *            single proxy settings object
     * @param save
     *            whether save these settings or not
     */
    public static void setProxySettings(final ProxySettings setttings,
            final boolean save) {
        proxySet = true;
    ...
    public static URLConnection getURLConnection(final URL url)
        ...
        if (proxySettings.isUseProxy()) {
            final URLConnection connection = url.openConnection(new Proxy(
                    Proxy.Type.HTTP, new InetSocketAddress(proxySettings
                            .getProxyHost(), proxySettings.getProxyPortInt())));
    ...
    public static void setupProxy(final URLConnection urlConnection) {
        ...
        final ProxySettings proxySettings = getProxySettings();
        if (proxySettings.isUseProxy()
                && proxySettings.isUseProxyAuthentification()) {
            final String toEncode = proxySettings.getProxyLogin() + ":"
                    + proxySettings.getProxyPassword();
            urlConnection.setRequestProperty("Proxy-Authorization", "Basic "
                    + EncryptionUtils.base64encode(toEncode));
        }
    }
     ...
     *            system proxy settings confirmation dialog support
     */
    public static void setSystemProxyConfirmationSupport(
            final SystemProxyConfirmationSupport support) {
