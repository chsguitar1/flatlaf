...
/**
 * This manager allows you to quickly and easily save any serializable data into
 * settings files using simple XML format.
 * <p/>
 * You can define settings group file name (they are equal to the SettingsGroup
 * name and should be unique), its location on local or remote drive and each
 * separate property key. Any amount of properties could be stored in each
 * group. Also there are some additional manager settings that allows you to
 * configure SettingsManager to work the way you want it to. The rest of the
 * work is done by the SettingsManager - it decides where, when and how to save
 * the settings files.
 * <p/>
 * Settings data aliasing (see XStream documentation and XmlUtils class) or
 * SettingsGroup file location changes should be done before requesting any of
 * the settings, preferably right at the application startup. Otherwise data
 * might not be properly read and settings will appear empty.
 *
 * @author Mikle Garin
 * @see <a
...
public class SettingsManager {
     ...
     */
    protected static final Map<String, Map<String, List<SettingsListener>>> settingsListeners = new HashMap<String, Map<String, List<SettingsListener>>>();
    ...
    /**
     * Save-on-change save delay in milliseconds. If larger than 0 then settings
     * will be accumulated and saved all at once as soon as no new changes came
    ...
    /**
     * Whether settings log is enabled or not. Log will display what settings
     * are being loaded and saved and when that happens. Log might contain
     * exceptions if settings cannot be read due to corrupted file or modified
     * object stucture.
     */
    ...
    /**
     * Whether should allow saving settings into files or not. If set to false
     * settings will be available only in runtime and will be lost after
     * application finishes working.
     */
     ...
     * <p/>
     * Also registered component will be: - listened for settings changes to
     * save them when requested - automatically updated with any loaded settings
     * for that key if requested - automatically updated with any changes made
     * in its settings if requested
     *
     * @param component
     ...
     *            component default value class
     * @param <T>
     *            default value type
     * @see DefaultValue
     */
    public static <T extends DefaultValue> void registerComponent(
     ...
     */
    public static void registerComponent(final Component component,
            final String group, final String key) {
     ...
     */
    public static <T extends DefaultValue> void registerComponent(
            final Component component, final String group, final String key,
            final Class<T> defaultValueClass) {
        registerComponent(component, group, key,
                getDefaultValue(defaultValueClass));
    }
     ...
     */
    public static void registerComponent(final Component component,
            final String group, final String key, final Object defaultValue) {
     ...
     * @param key
     *            component settings key
     * @param loadInitialSettings
     *            whether to load initial available settings into the component
     *            or not
     * @param applySettingsChanges
     *            whether to apply settings changes to the component or not
     */
    public static void registerComponent(final Component component,
            final String key, final boolean loadInitialSettings,
            final boolean applySettingsChanges) {
        registerComponent(component, key, null, loadInitialSettings,
    ...
    public static <T extends DefaultValue> void registerComponent(
            ...
            final boolean applySettingsChanges) {
        registerComponent(component, group, key,
                getDefaultValue(defaultValueClass), loadInitialSettings,
                applySettingsChanges);
    }
    ...
    public static void registerComponent(final Component component,
            ...
            final boolean loadInitialSettings,
            final boolean applySettingsChanges) {
        ComponentSettingsManager.registerComponent(new SettingsProcessorData(
                component, group, key, defaultValue, loadInitialSettings,
                applySettingsChanges));
    }
    ...
    public static void registerComponent(final Component component,
        ...
        ComponentSettingsManager
                .registerComponent(component, settingsProcessor);
    }
    ...
    public static <T extends SettingsProcessor> void registerSettingsProcessor(
        ...
        ComponentSettingsManager.registerSettingsProcessor(componentType,
                settingsProcessor);
    }
     ...
     * @return typed value
     */
    public static <T extends DefaultValue> T get(final String key,
            final Class<T> defaultValueClass) {
        return get(defaultSettingsGroup, key,
                getDefaultValue(defaultValueClass));
    }
     ...
     * @return typed value
     */
    public static <T extends DefaultValue> T get(final String group,
     ...
     * @return typed value
     */
    public static <T> T get(final String group, final String key,
        ...
        try {
            ...
            FlatLafLogger.error(SettingsManager.class,
                    "Unable to load settings value for group \"" + group
                            + "\" and key \"" + key
     ...
     */
    public static <T extends DefaultValue> T getDefaultValue(
            final Class<T> defaultValueClass) {
        if (defaultValueClass == null) {
            // Workaround for cases with accidental method calls
            // Also works fine if this method was called but class is actually
            // null
            return null;
        } else {
            // Call special static method that should be in each class that
            // implements DefaultValue interface
            return ReflectUtils.callStaticMethodSafely(defaultValueClass,
                    "getDefaultValue");
        }
    }
    ...
    /**
     * Resets all settings within the specified settings group. This will also
     * reset saved settings to ensure they won't be read again.
     *
     * @param group
     *            settings group
     */
    ...
    protected static SettingsGroup loadSettingsGroup(final String group) {
        ...
        if (dir.exists() && dir.isDirectory()) {
            ...
            // todo 1. Backup reading occurs only if original file cannot be
            // read
            // todo 2. Do not delete the settings that cannot be read right
            // away, just move them aside with ".failed" extension mark
            if (file.exists() && file.isFile() || backupFile.exists()
                    && backupFile.isFile()) {
                ...
                if (file.exists() && file.isFile()) {
                    try {
                        ...
                        groupState.put(group, new SettingsGroupState(
                                readFromBackup ? ReadState.restored
                                        : ReadState.ok));
                        
                        final String state = readFromBackup ? "restored from backup"
                                : "loaded";
                        FlatLafLogger.info(SettingsManager.class,
                                "Settings group \"" + group + "\" " + state
                                        + " successfully");
                    } catch (final Throwable e) {
                        FlatLafLogger.error(SettingsManager.class,
                                "Unable to load settings group \"" + group
                ...
                // No group settings file or backup exists, new SettingsGroup
                // will be created
                groupState
                        .put(group, new SettingsGroupState(ReadState.created));
                
                FlatLafLogger.info(SettingsManager.class, "Settings group \""
     ...
     *
     * @param group
     *            name of the settings group to save
     */
        ...
        if (allowSave) {
            try {
                ...
                FlatLafLogger.error(
                        ...
                        "Unable to save settings group \""
                                + settingsGroup.getName()
                                + "\" due to unexpected exception:", e);
            }
        }
    }
    ...
    protected static File getGroupBackupFile(final String group, final File dir) {
        return new File(dir, group + settingsFilesExtension
                + backupFilesExtension);
    }
    
    /**
     * Delays settings group save or performs it immediately according to
        ...
        if (saveOnChangeDelay > 0) {
            ...
            synchronized (saveOnChangeLock) {
                ...
                if (groupSaveScheduler == null
                        || !groupSaveScheduler.isRunning()) {
                    if (groupSaveScheduler == null) {
                        groupSaveScheduler = new WebTimer(
                                "SettingsManager.groupSaveScheduler",
                                saveOnChangeDelay, new ActionListener() {
                                    @Override
     ...
     *            value
     */
    protected static void saveSettings(final String fileName,
     ...
     * @param fileName
     *            settings file name
     * @return true if settings file with the specified name exists, false
     *         otherwise
     */
     ...
     *
     * @param settingsFilesExtension
     *            new settings files extension
     */
    public static void setSettingsFilesExtension(
            final String settingsFilesExtension) {
     ...
     *
     * @param backupFilesExtension
     *            new backup files extension
     */
     ...
     *
     * @param defaultSettingsDir
     *            new default settings directory
     */
     ...
     *
     * @param defaultSettingsDir
     *            new default settings directory name
     */
     ...
     *
     * @param defaultSettingsGroup
     *            new default settings group name
     */
    ...
    /**
     * Sets custom directory where settings group file will be saved. Location
     * that was used for this group before will be simply ignored.
     *
     * @param group
     *            settings group name
     * @param dir
     ...
     *
     * @return true if should save settings right after any changes made, false
     *         otherwise
     */
     ...
     *
     * @param saveOnChange
     *            whether should save settings right after any changes made or
     *            not
     */
     ...
     *
     * @return true if should save provided default value in "get" calls, false
     *         otherwise
     */
     ...
     *            whether should save provided default value in "get" calls or
     *            not
     */
    ...
    /**
     * Sets save-on-change delay in milliseconds. If larger than 0 then settings
     * will be accumulated and saved all at once as soon as no new changes came
     * within the delay time.
     *
     * @param saveOnChangeDelay
     *            new save-on-change delay in milliseconds
     */
    ...
    /**
     * Returns whether should display settings load and save error messages or
     * not.
     *
     * @return true if should display settings load and save error messages,
     *         false otherwise
     */
     ...
     *            whether should display settings load and save error messages
     *            or not
     */
     ...
     * @param listener
     *            settings listener to add
     */
    public static void addSettingsListener(final String key,
     ...
     *
     * @param group
     *            settings group to listen to
     * @param key
     *            settings key to listen to
     * @param listener
     *            settings listener to add
     */
    public static void addSettingsListener(final String group,
            final String key, final SettingsListener listener) {
        if (!settingsListeners.containsKey(group)) {
            settingsListeners.put(group,
                    new HashMap<String, List<SettingsListener>>());
        }
        if (!settingsListeners.get(group).containsKey(key)) {
            settingsListeners.get(group).put(key,
                    new ArrayList<SettingsListener>());
        }
     ...
     * @param listener
     *            settings listener to remove
     */
    public static void removeSettingsListener(final String key,
            final SettingsListener listener) {
     ...
     *            settings listener to remove
     */
    public static void removeSettingsListener(final String group,
            final String key, final SettingsListener listener) {
        if (settingsListeners.containsKey(group)) {
            if (settingsListeners.get(group).containsKey(key)) {
     ...
     *            new value
     */
    protected static void fireSettingsChanged(final String group,
            final String key, final Object newValue) {
        if (settingsListeners.containsKey(group)) {
            if (settingsListeners.get(group).containsKey(key)) {
                for (final SettingsListener listener : CollectionUtils
                        .copy(settingsListeners.get(group).get(key))) {
