...
/**
 * This abstract class represents core component settings tracking
 * functionality. Extend and register it in SettingsManager or
 * ComponentSettingsManager to provide additional components support.
 * <p/>
 * SettingsProcessor is also defended from recursive settings load/save which
 * might occur if component sends additional data change events when new data is
 * loaded into it (doesn't matter from SettingsProcessor or some other source).
 * <p/>
 * To register new SettingsProcessor use
 * {@code registerSettingsProcessor(Class, Class)} method from SettingsManager
 * or ComponentSettingsManager class (they both do the same).
 *
 * @author Mikle Garin
 * @see <a
 ...
 * @see org.ocsoft.flatlaf.managers.settings.SettingsManager#registerSettingsProcessor(Class,
 *      Class)
 * @see org.ocsoft.flatlaf.managers.settings.ComponentSettingsManager
 * @see org.ocsoft.flatlaf.managers.settings.ComponentSettingsManager#registerSettingsProcessor(Class,
 *      Class)
 */
public abstract class SettingsProcessor<C extends Component, V extends Serializable> {
     ...
     */
    public SettingsProcessor(final Component component, final String group,
            final String key, final Object defaultValue,
            final boolean loadInitialSettings,
            final boolean applySettingsChanges) {
        this(new SettingsProcessorData(component, group, key, defaultValue,
    ...
    public SettingsProcessor(final SettingsProcessorData data) {
        ...
        if (data.isLoadInitialSettings()) {
            try {
                ...
                if (SettingsManager.isLoggingEnabled()) {
                    FlatLafLogger.error(
                            this,
                            "Unable to load initial component settings for group \""
                                    + data.getGroup() + "\" and key \""
                                    + data.getKey()
        ...
        try {
            ...
            if (SettingsManager.isLoggingEnabled()) {
                FlatLafLogger.error(
                        this,
                        "Unable to initialize specific processor settings for component with group \""
                                + data.getGroup() + "\" and key \""
                                + data.getKey()
                                + "\" due to unexpected exception:", e);
            }
        ...
        if (data.isApplySettingsChanges()) {
            SettingsManager.addSettingsListener(getGroup(), getKey(),
                    new SettingsListener() {
                        @Override
                        public void settingsChanged(final String group,
    ...
    /**
     * Saves settings taken from the component. This method might be called from
     * the component listeners to provide auto-save functionality.
     */
     ...
     *
     * @param onChange
     *            whether this save is called from component change listeners
     */
    ...
    /**
     * Called on component settings load. It doesn't matter if it is invoked by
     * SettingsProcessor or some other source.
     *
     * @param component
     *            component to load settings into
     */
    protected abstract void doLoad(C component);
    
    /**
     * Called on component settings save. It doesn't matter if it is invoked by
     * SettingsProcessor or some other source.
     *
     * @param component
