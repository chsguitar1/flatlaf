...
public class WebLabelPainter<E extends JLabel> extends AbstractPainter<E>
    ...
    public Insets getMargin(final E label) {
        return backgroundPainter != null ? backgroundPainter.getMargin(label)
                : super.getMargin(label);
    }
    ...
    public void paint(final Graphics2D g2d, final Rectangle bounds,
        ...
        final Map textHints = drawShade ? FlatLafStyleConstants.defaultTextRenderingHints
                : FlatLafStyleConstants.textRenderingHints;
        ...
        final String clippedText = layout(label, fm, label.getWidth(),
                label.getHeight());
     ...
     */
    protected String layout(final E label, final FontMetrics fm,
     ...
     * @param iconR
     *            icon rectangle dummy
     * @param textR
     *            text rectangle dummy
     * @return clipped or full label text
     */
    protected String layoutCL(final E label, final FontMetrics fm,
            final String text, final Icon icon, final Rectangle viewR,
            final Rectangle iconR, final Rectangle textR) {
        return SwingUtilities.layoutCompoundLabel(label, fm, text, icon,
    ...
    protected void paintEnabledText(final E label, final Graphics2D g2d,
        ...
        if (drawShade) {
            ...
            g2d.setColor(label.getForeground());
            SwingUtils.drawStringUnderlineCharAt(g2d, text, mnemIndex, textX,
                    textY);
        }
    }
    ...
    protected void paintDisabledText(final E label, final Graphics2D g2d,
        ...
        if (label.isEnabled() && drawShade) {
            ...
            SwingUtils.drawStringUnderlineCharAt(g2d, text, accChar, textX + 1,
                    textY + 1);
            g2d.setColor(background.darker());
            SwingUtils.drawStringUnderlineCharAt(g2d, text, accChar, textX,
                    textY);
        }
    }
     ...
     *            text Y coordinate
     */
    protected void paintShadowText(final Graphics2D g2d, final String text,
    ...
    public Dimension getPreferredSize(final E label) {
        ...
        if ((icon == null)
            ...
            final int x2 = Math.max(iconR.x + iconR.width, textR.x
                    + textR.width);
            final int y1 = Math.min(iconR.y, textR.y);
            final int y2 = Math.max(iconR.y + iconR.height, textR.y
                    + textR.height);
