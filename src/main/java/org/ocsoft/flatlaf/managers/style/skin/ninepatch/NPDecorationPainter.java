 ...
 * is similar to {@code WebDecorationPainter} but this one is based on 9-patch
 * icons. It uses 9-patch icons for normal and focused component states and its
 * side separators.
 *
 * @author Mikle Garin
 * @see org.ocsoft.flatlaf.managers.style.skin.web.WebDecorationPainter
 */

public class NPDecorationPainter<E extends JComponent> extends
        AbstractPainter<E> implements PartialDecoration {
    /**
     * Style settings.
     */
    ...
    /**
     * Returns whether single decoration side size should be detected by content
     * patches on the image or not. When true - hidden panel sides will cut
     * appropriate 9-patch image sides using its content patches. When false -
     * hidden panel sides will cut appropriate 9-patch image sides using its
     * stretch patches.
     *
     * @return true if single decoration side size should be detected by content
     *         patches on the image, false otherwise
     */
    ...
    /**
     * Set whether single decoration side size should be detected by content
     * patches on the image or not. If set to true - hidden panel sides will cut
     * appropriate 9-patch image sides using its content patches. If set to -
     * hidden panel sides will cut appropriate 9-patch image sides using its
     * stretch patches.
     *
     * @param detectSideByContentPatches
     *            whether single decoration side size should be detected by
     *            content patches on the image or not
     */
    public void setDetectSideByContentPatches(
            final boolean detectSideByContentPatches) {
    ...
    /**
     * Sets additional spacing provided for side with hidden decoration. This
     * might be useful if you want to keep default component size the same even
     * if some sides are hidden.
     *
     * @param spacing
    ...
    /**
     * Returns default side separator 9-patch icon. This icon is used instead of
     * specific side separator icon if one not specified.
     *
    ...
    /**
     * Sets default side separator 9-patch icon. This icon is used instead of
     * specific side separator icon if one not specified.
     *
     * @param icon
    ...
    public Insets getMargin(final E c) {
        ...
        if (!undecorated && backgroundIcon != null) {
            ...
            if (!paintTop) {
                ...
                margin.top = (paintTopLine && icon != null ? icon
                        .getPreferredSize().height : 0) + hiddenSideSpacing;
            }
            if (!paintLeft) {
                final NinePatchIcon icon = getCurrentLeftSeparatorIcon();
                margin.left = (paintLeftLine && icon != null ? icon
                        .getPreferredSize().width : 0) + hiddenSideSpacing;
            }
            if (!paintBottom) {
                final NinePatchIcon icon = getCurrentBottomSeparatorIcon();
                margin.bottom = (paintBottomLine && icon != null ? icon
                        .getPreferredSize().height : 0) + hiddenSideSpacing;
            }
            if (!paintRight) {
                final NinePatchIcon icon = getCurrentRightSeparatorIcon();
                margin.right = (paintRightLine && icon != null ? icon
                        .getPreferredSize().width : 0) + hiddenSideSpacing;
            }
            return margin;
        } else {
    ...
    public void paint(final Graphics2D g2d, final Rectangle bounds, final E c) {
        if (!undecorated) {
            ...
            if (backgroundIcon != null) {
                backgroundIcon.paintIcon(g2d,
                        getBackgroundBounds(backgroundIcon, bounds, c));
            }
            ...
            if (isTopLineShouldBePainted()) {
                ...
                if (icon != null) {
                    ...
                    icon.paintIcon(g2d, bounds.x, bounds.y, bounds.width,
                            ps.height);
                }
            }
            if (isLeftLineShouldBePainted()) {
                ...
                if (icon != null) {
                    final Dimension ps = icon.getPreferredSize();
                    icon.paintIcon(g2d, bounds.x, bounds.y, ps.width,
                            bounds.height);
                }
            }
            if (isBottomLineShouldBePainted()) {
                ...
                if (icon != null) {
                    final Dimension ps = icon.getPreferredSize();
                    icon.paintIcon(g2d, bounds.x, bounds.y + bounds.height
                            - ps.height, bounds.width, ps.height);
                }
            }
            if (isRightLineShouldBePainted()) {
                ...
                if (icon != null) {
                    final Dimension ps = icon.getPreferredSize();
                    icon.paintIcon(g2d, bounds.x + bounds.width - ps.width,
                            bounds.y, ps.width, bounds.height);
                }
            }
        }
    }
    ...
    /**
     * Returns top separator 9-patch icon that should be painted according to
     * provided icons.
     *
     * @return top separator 9-patch icon that should be painted according to
     *         provided icons
     */
    ...
    /**
     * Returns left separator 9-patch icon that should be painted according to
     * provided icons.
     *
     * @return left separator 9-patch icon that should be painted according to
     *         provided icons
     */
    ...
    /**
     * Returns bottom separator 9-patch icon that should be painted according to
     * provided icons.
     *
     * @return bottom separator 9-patch icon that should be painted according to
     *         provided icons
     */
    protected NinePatchIcon getCurrentBottomSeparatorIcon() {
        return bottomSeparatorIcon != null ? bottomSeparatorIcon
                : separatorIcon;
    }
    
    /**
     * Returns right separator 9-patch icon that should be painted according to
     * provided icons.
     *
     * @return right separator 9-patch icon that should be painted according to
     *         provided icons
     */
    ...
    /**
     * Returns whether at least one side line should be painted or not. This
     * method is used to optimize painting performance.
     *
     ...
     * These bounds might not always fit component bounds to make a clipped side
     * trick.
     *
     * @param icon
     *            background 9-patch icon
    ...
    protected Rectangle getBackgroundBounds(final NinePatchIcon icon,
        ...
        if (!undecorated && icon != null) {
            final Insets margin = detectSideByContentPatches ? icon.getMargin()
                    : icon.getStretchMargin();
            ...
            return new Rectangle(b.x - ls, b.y - ts, b.width + ls + rs,
                    b.height + ts + bs);
        } else {
            return b;
        }
