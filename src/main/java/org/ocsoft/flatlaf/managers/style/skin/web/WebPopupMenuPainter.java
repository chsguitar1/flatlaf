...
public class WebPopupMenuPainter<E extends JPopupMenu> extends
        WebPopupPainter<E> implements PopupMenuPainter<E> {
    /**
     * todo 1. Incorrect menu placement when corner is off (spacing == shade)
     * todo 2. When using popupMenuWay take invoker shade into account (if its
     * UI has one -> ShadeProvider interface) todo 3. Add left/right corners
     * display support
     */
    ...
    /**
     * Paints dropdown-styled popup menu corner fill if menu item near it is
     * selected.
     *
     * @param g2d
     *            graphics context
     ...
     *            menu size
     */
    protected void paintDropdownCornerFill(final Graphics2D g2d,
            final E popupMenu, final Dimension menuSize) {
        ...
        if (popupStyle == PopupStyle.dropdown && round == 0) {
            ...
            final boolean stick = top ? (pmui.getMargin().top + margin.top == 0)
                    : (pmui.getMargin().bottom + margin.bottom == 0);
            if (stick) {
                // Checking that we can actually retrieve what item wants to
                // fill corner with
                ...
                if (component instanceof JMenuItem) {
                    ...
                    if (menuItem.isEnabled()
                            && (menuItem.getModel().isArmed() || menuItem
                                    .isSelected())) {
                        // Filling corner properly
                        if (menuItem.getUI() instanceof WebMenuUI) {
                            final WebMenuUI ui = (WebMenuUI) menuItem.getUI();
                            g2d.setPaint(top ? ui.getNorthCornerFill() : ui
                                    .getSouthCornerFill());
                            g2d.fill(getDropdownCornerShape(popupMenu,
                                    menuSize, true));
                        } else if (menuItem.getUI() instanceof WebMenuItemUI) {
                            final WebMenuItemUI ui = (WebMenuItemUI) menuItem
                                    .getUI();
                            g2d.setPaint(top ? ui.getNorthCornerFill() : ui
                                    .getSouthCornerFill());
                            g2d.fill(getDropdownCornerShape(popupMenu,
    ...
    public Point preparePopupMenu(final E popupMenu, final Component invoker,
        ...
        if (invoker != null) {
            ...
            final Point los = invoker.isShowing() ? invoker
            ...
            if (invoker instanceof JMenu) {
                if (invoker.getParent() instanceof JPopupMenu) {
                    ...
                    setPopupStyle(PopupStyle.simple);
                    if (fixLocation) {
                        // Invoker X-location on screen also works as
                        // orientation indicator, so we don't need to check it
                        // here
                        x += (los.x <= x ? -1 : 1)
                                * (transparent ? sideWidth - menuSpacing
                                        : -menuSpacing);
                        y += (los.y <= y ? -1 : 1)
                                * (transparent ? sideWidth + 1 + round : round);
                    }
                    ...
                    if (fixLocation) {
                        ...
                        // here
                        x += (los.x <= x ? -1 : 1)
                                * (transparent ? sideWidth : 0);
                        y += (los.y <= y ? -1 : 1)
                                * (transparent ? sideWidth - cornerWidth : 0);
                    }
                ...
                if (invoker instanceof JComboBox
                        && popupMenu.getName().equals("ComboPopup.popup")) {
                    cornerSide = los.y <= y ? TOP : BOTTOM;
                    if (fixLocation) {
                        x += transparent ? -sideWidth : 0;
                        if (cornerSide == TOP) {
                            y -= transparent ? (sideWidth - (dropdown ? cornerWidth
                                    : 0))
                                    : 0;
                        } else {
                            // Invoker preferred size is required instead of
                            // actual height
                            // This is because the original position takes it
                            // into account instead of height
                            final int ih = invoker.getPreferredSize().height;
                            y -= ih
                                    + (transparent ? (sideWidth - (dropdown ? cornerWidth
                                            : 0))
                                            : 0);
                        }
                    }
                    ...
                    if (fixLocation) {
                        ...
                        if (popupMenuWay != null) {
                            ...
                            case aboveMiddle: {
                                ...
                                y = los.y - ps.height + cornerShear;
                                relativeCorner = los.x + invoker.getWidth() / 2
                                        - x;
                                break;
                            }
                            case aboveEnd: {
