 ...
 * WebPopupMenuUI default styling but might also be used in other cases. Be
 * aware that you will have to manually setup this painter if used outside of
 * the WebPopupMenuUI.
 *
 * @author Mikle Garin
 */
...
public class WebPopupPainter<E extends JComponent> extends AbstractPainter<E>
     ...
     *
     * @param round
     *            new popup border corners rounding
     */
     ...
     *
     * @param transparent
     *            whether popup is transparent or not
     */
    ...
    public Insets getMargin(final E c) {
        ...
        final Insets m = new Insets(margin.top, ltr ? margin.left
                : margin.right, margin.bottom, ltr ? margin.right : margin.left);
    ...
    /**
     * Paints transparent popup version. This one is used when popup component
     * can be transparent, otherwise a simple popup version is painted.
     *
     * @param g2d
     *            graphics context
     * @param popup
    ...
    /**
     * Paints simple popup version. This one is used when popup component is
     * opaque.
     *
     * @param g2d
     *            graphics context
    ...
    protected void paintSimplePopup(final Graphics2D g2d, final E popup) {
        ...
        g2d.fillRoundRect(1, 1, popup.getWidth() - 2, popup.getHeight() - 2,
                round * 2, round * 2);
        
        // Border
        g2d.setColor(borderColor);
        g2d.drawRoundRect(0, 0, popup.getWidth() - 1, popup.getHeight() - 1,
                round * 2, round * 2);
    }
     ...
     *            popup size
     */
    protected void paintShade(final Graphics2D g2d, final E popup,
            final Dimension popupSize) {
        if (shadeWidth > 0) {
            final NinePatchIcon shade = NinePatchUtils.getShadeIcon(shadeWidth,
                    round * 2, getCurrentShadeTransparency());
    ...
    /**
     * Returns current shade transparency. This part moved to a separate method
     * to make it simple to override without touchin the actual transparency
     * value.
     *
     ...
     *            popup size
     */
    protected void paintBackground(final Graphics2D g2d, final E popup,
            final Dimension popupSize) {
     ...
     *            popup size
     */
    protected void paintBorder(final Graphics2D g2d, final E popup,
            final Dimension popupSize) {
        if (borderColor != null) {
    ...
    protected Color getBackgroundColor(final E popup) {
        ...
        return !transparent || transparency >= 1f ? bg : ColorUtils
                .getTransparentColor(bg,
                        Math.max(0, Math.min((int) (transparency * 255), 255)));
    }
    
    /**
     * Returns popup background color. This call moved into a separate method so
     * you can override it to provide different background color.
     *
     * @param popup
     *            popup component
     * @return popup background color
    ...
    protected Rectangle getShadeBounds(final Dimension popupSize) {
        ...
        case dropdown: {
            ...
            return new Rectangle(spacing, spacing, popupSize.width - spacing
                    * 2, popupSize.height - spacing * 2);
        }
        default: {
            return null;
        }
     ...
     */
    protected Shape getBorderShape(final E popup, final Dimension popupSize,
            final boolean fill) {
        switch (popupStyle) {
        case simple: {
            return ShapeCache.getShape(popup, fill ? SIMPLE_FILL_SHAPE
                    : SIMPLE_BORDER_SHAPE, new DataProvider<Shape>() {
                @Override
        ...
        case dropdown: {
            return ShapeCache.getShape(popup, fill ? DROPDOWN_FILL_SHAPE
                    : DROPDOWN_BORDER_SHAPE, new DataProvider<Shape>() {
                @Override
                public Shape provide() {
    ...
    protected Object[] getCachedShapeSettings(final E popup) {
        return new Object[] { round, shadeWidth, cornerWidth, cornerSide,
                relativeCorner, cornerAlignment, popup.getSize(),
     ...
     */
    protected GeneralPath createSimpleShape(final E popup,
            final Dimension popupSize, final boolean fill) {
     ...
     */
    protected GeneralPath createDropdownShape(final E popup,
            final Dimension popupSize, final boolean fill) {
        ...
        final boolean rightCorner = cornerSide == RIGHT
                || cornerSide == TRAILING;
        ...
        final int length = topCorner || bottomCorner ? popupSize.width
                : popupSize.height;
        final int spacing;
        if (cornerAlignment == CENTER
                || length < sideWidth * 2 + round * 2 + cornerWidth * 4) {
            spacing = length / 2 - sideWidth - round - cornerWidth * 2;
        } else if (cornerAlignment == LEFT || cornerAlignment == LEADING && ltr
                || cornerAlignment == TRAILING && !ltr) {
            spacing = 0;
        } else if (cornerAlignment == RIGHT || cornerAlignment == TRAILING
                && ltr || cornerAlignment == LEADING && !ltr) {
            spacing = length - cornerShear * 2;
        } else {
            spacing = relativeCorner < sideWidth + round + cornerWidth * 2 ? 0
                    : Math.min(relativeCorner - cornerShear, length
                            - cornerShear * 2);
        }
        ...
        if (topCorner) {
            ...
            shape.lineTo(left + round + spacing + cornerWidth * 2, top
                    - cornerWidth);
            shape.lineTo(left + round + spacing + cornerWidth * 2 + 1, top
                    - cornerWidth);
        ...
        if (rightCorner) {
            ...
            shape.lineTo(right + cornerWidth, top + round + spacing
                    + cornerWidth * 2);
            shape.lineTo(right + cornerWidth, top + round + spacing
                    + cornerWidth * 2 + 1);
        ...
        if (bottomCorner) {
            ...
            shape.lineTo(left + round + spacing + cornerWidth * 2 + 1, botom
                    + cornerWidth);
            shape.lineTo(left + round + spacing + cornerWidth * 2, botom
                    + cornerWidth);
        ...
        if (leftCorner) {
            ...
            shape.lineTo(left - cornerWidth, top + round + spacing
                    + cornerWidth * 2 + 1);
            shape.lineTo(left - cornerWidth, top + round + spacing
                    + cornerWidth * 2);
    ...
    /**
     * Returns dropdown style corner shape. It is used to paint corner fill when
     * menu item at the same as corner side of popup menu is selected.
     *
     * @param popupMenu
     *            popup menu
     * @param menuSize
     ...
     */
    protected Shape getDropdownCornerShape(final E popupMenu,
            final Dimension menuSize, final boolean fill) {
        return ShapeCache.getShape(popupMenu, fill ? "dropdown-corner-fill"
                : "dropdown-corner-border", new DataProvider<Shape>() {
            @Override
            public Shape provide() {
    ...
    /**
     * Creates and returns dropdown style corner shape. It is used to paint
     * corner fill when menu item at the same as corner side of popup menu is
     * selected.
     *
     * @param popupMenu
     *            popup menu
     ...
     * @return dropdown style corner shape
     */
    protected GeneralPath createDropdownCornerShape(final E popupMenu,
            final Dimension menuSize, final boolean fill) {
        final boolean topCorner = cornerSide == TOP;
        final boolean bottomCorner = cornerSide == BOTTOM;
        final boolean leftCorner = cornerSide == LEFT || cornerSide == LEADING;
        ...
        final int cornerShear = sideWidth + shear + round + cornerWidth * 2;
        final int length = topCorner || bottomCorner ? menuSize.width
                : menuSize.height;
        final int spacing;
        if (cornerAlignment == CENTER
                || length < sideWidth * 2 + round * 2 + cornerWidth * 4) {
    ...
    /**
     * Returns popup side space width. This is the space taken by popup shade
     * and corner.
     *
     * @return popup side space width
     */
    protected int getSideWidth() {
        return popupStyle == PopupStyle.dropdown ? Math.max(cornerWidth,
