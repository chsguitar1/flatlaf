 ...
 * of component painters and settings to provide an unique visual style.
 *
 * @author Mikle Garin
 * @see <a
 ...
 * @see org.ocsoft.flatlaf.managers.style.skin.CustomSkin
 */
...
public abstract class WebLafSkin {
    ...
    /**
     * Returns unique skin ID. Used to collect and manage skins within
     * StyleManager.
     *
    ...
    /**
     * Returns whether this skin is supported or not. This method reflects the
     * default mechanism of checking skin support. You can override it in your
     * own skin to provide any custom checks.
     *
    ...
    public boolean isSupported() {
        ...
        return supportsAny
                && (systems.contains(ALL_SYSTEMS_SUPPORTED) || systems
                        .contains(FlatLafSystemUtils.getShortOsName()));
    }
    ...
    /**
     * Returns style for the specified supported component type. Custom style ID
     * can be specified in any Web-component or Web-UI to override default
     * component style. If style for such custom ID is not found in skin
     * descriptor then default style for that component is used.
     *
     * @param component
     *            component instance
     * @param type
     ...
     */
    protected SupportedComponent getSupportedComponentTypeImpl(
            final JComponent component) {
        final SupportedComponent type = SupportedComponent
                .getComponentTypeByUIClassID(component.getUIClassID());
        if (type == null) {
            throw new StyleException("Unknown component UI class ID: "
    ...
    /**
     * Returns component style. This method does a check that style exists in
     * addition to abstract one.
     *
     * @param component
     *            component instance
     * @return component style
    ...
    protected ComponentStyle getComponentStyleImpl(final JComponent component) {
        return getComponentStyleImpl(component,
                getSupportedComponentTypeImpl(component));
    }
    ...
    protected ComponentStyle getComponentStyleImpl(final JComponent component,
        ...
        if (style == null) {
            throw new StyleException(
                    "Skin doesn't contain style for UI class ID: "
                            + component.getUIClassID());
        }
        return style;
    }
    ...
    protected ComponentUI getComponentUIImpl(final JComponent component) {
        ...
        if (ui == null) {
            throw new StyleException("Unable to retrieve UI from component: "
                    + component);
        }
        return ui;
    }
    ...
    /**
     * Applies this skin to the specified component. Returns whether sking was
    ...
    public boolean applySkin(final JComponent component) {
        return applySkin(component,
                StyleManager.getCustomPainterProperties(component),
                StyleManager.getCustomPainters(component));
    }
     ...
     * @param component
     *            component to apply skin to
     * @param customPainterProperties
     *            custom painter properties to apply
     * @param customPainters
     *            custom painters to apply
     * @return true if skin was applied, false otherwise
     */
    public boolean applySkin(final JComponent component,
            final Map<String, Map<String, Object>> customPainterProperties,
        ...
        try {
            ...
            for (final PainterStyle painterStyle : style.getPainters()) {
                ...
                // Custom painter can be null - that will just mean that
                // component should not have painter installed
                final Painter painter;
                if (customPainters != null
                        && customPainters.containsKey(painterId)) {
                    // Using provided custom painter
                    // This might be set using Web-component
                    // "set...Painter"-like methods
                    painter = customPainters.get(painterId);
                } else {
                    // Creating painter instance
                    // Be aware that all painters must have default constructor
                    painter = ReflectUtils.createInstanceSafely(painterStyle
                            .getPainterClass());
                    if (painter == null) {
                        throw new StyleException("Unable to create painter \""
                                + painterStyle.getPainterClass()
                                + "\" for component: " + component);
                    }
                    
                    // Applying painter properties
                    // These properties are applied only for style-provided
                    // painters
                    // Customly provided painters are not affected by these
                    // properties to avoid unexpected changes in them
                    final Map<String, Object> cpp = getCustomPainterProperties(
                            customPainterProperties, painterStyle, painterId);
                    applyProperties(painter, painterStyle.getProperties(), cpp);
                }
                
                // Installing painter into the UI
                final String setterMethod = ReflectUtils
                        .getSetterMethodName(painterId);
     ...
     */
    protected Map<String, Object> getCustomPainterProperties(
            final Map<String, Map<String, Object>> customPainterProperties,
            final PainterStyle painterStyle, final String painterId) {
        return customPainterProperties != null ? customPainterProperties
                .get(painterStyle.isBase() ? null : painterId) : null;
    }
    ...
    public boolean removeSkin(final JComponent component) {
        try {
            ...
            for (final PainterStyle painterStyle : style.getPainters()) {
                final String setterMethod = ReflectUtils
                        .getSetterMethodName(painterStyle.getId());
    ...
    /**
     * Returns painter property value from the specified component. Specified
     * property is searched only inside the base painter so far.
     *
     * @param component
     *            component to retrieve style property from
     * @param key
     *            style property key
     * @param <T>
     *            style property value type
     * @return style property value
     */
    public <T> T getPainterPropertyValue(final JComponent component,
            final String key) {
     ...
     */
    public <T> T getPainterPropertyValue(final JComponent component,
            final String painterId, final String key) {
        ...
        if (painter != null) {
            ...
            throw new StyleException("Painter with ID \"" + painterId
                    + "\" was not found for component: " + component);
        }
    }
    
    /**
     * Sets custom value for painter property for the specified component. This
     * tricky method retrieves component painter throught its UI and skin
     * settings and applies the specified style property.
     *
     * @param component
     *            component to apply custom style property to
     * @param key
     ...
     */
    public boolean setCustomPainterProperty(final JComponent component,
     ...
     */
    public boolean setCustomPainterProperty(final JComponent component,
            final String painterId, final String key, final Object value) {
        final Painter painter = getPainter(component, painterId);
        if (painter != null) {
     ...
     *            custom properties to apply, these properties are provided
     *            directly into the specific component
     */
    protected void applyProperties(final Object object,
            final Map<String, Object> skinProperties,
        ...
        if (customProperties != null && customProperties.size() > 0) {
            if (skinProperties != null && skinProperties.size() > 0) {
                // Custom properties are added after skin properties to replace
                // existing values
                mergedProperties = new HashMap<String, Object>(Math.max(
                        skinProperties.size(), customProperties.size()));
        ...
        if (mergedProperties != null && mergedProperties.size() > 0) {
            for (final Map.Entry<String, Object> entry : mergedProperties
                    .entrySet()) {
     ...
     * modify even private object fields. Note that this method might also work
     * even if there is no real field with the specified name but there is
     ...
     */
    public static boolean setFieldValue(final Object object,
        ...
        try {
            final Field actualField = ReflectUtils.getField(object.getClass(),
                    field);
     ...
     * object fields. Note that this method might also work even if there is no
     * real field with the specified name but there is fitting getter method.
     *
     * @param object
     *            object instance
     * @param field
    ...
    public static <T> T getFieldValue(final Object object, final String field) {
        ...
        // Note that this method might work even if there is no real field with
        // the specified name but there is fitting getter method
        // This was made to improve call speed (no real field check) and avoid
        // accessing field directly (in most of cases)
        try {
        ...
        // This one is rarely used and in most of times will be called when
        // inappropriate property is set
        try {
    ...
    public <T extends Painter> T getPainter(final JComponent component,
        ...
        final String pid = painterId != null ? painterId
                : getComponentStyleImpl(component).getBasePainter().getId();
        final ComponentUI ui = getComponentUIImpl(component);
