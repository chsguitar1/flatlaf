...
public class ValueConverter implements Converter {
    ...
    public void marshal(final Object source,
        ...
        } else if (value.getTooltips() != null
                && value.getTooltips().size() == 1
    ...
    public Object unmarshal(final HierarchicalStreamReader reader,
        ...
        value.setMnemonic(mnemonicValue != null ? mnemonicValue.charAt(0)
                : null);
        ...
        final TooltipType tipType = TooltipConverter.parseType(reader
                .getAttribute(TIP_TYPE));
        final TooltipWay tipWay = TooltipConverter.parseWay(reader
                .getAttribute(TIP_WAY));
        final Integer tipDelay = TooltipConverter.parseDelay(reader
                .getAttribute(TIP_DELAY));
        ...
        while (reader.hasMoreChildren()) {
            ...
            if (reader.getNodeName().equals("text")) {
                ...
                tooltips.add((Tooltip) context.convertAnother(value,
                        Tooltip.class));
            }
            reader.moveUp();
        }
        ...
        if (texts.size() == 0 && tooltips.size() == 0) {
            ...
            if (state != null && state.equals(TOOLTIP_KEY)) {
                value.setTooltips(CollectionUtils.copy(new Tooltip(tipType,
                        tipWay, tipDelay, text)));
                value.setTexts(null);
            } else {
