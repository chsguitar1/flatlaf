...
public final class Value implements Serializable, Cloneable {
    ...
    public Value(final String lang, final Character mnemonic,
            final String hotkey, final String text) {
        super();
        this.lang = lang;
        this.mnemonic = mnemonic;
        this.hotkey = hotkey;
    ...
    public Text getTextObject(final String state, final boolean defaultState) {
        if (texts != null) {
            for (final Text text : texts) {
                if (CompareUtils.equals(text.getState(), state) || defaultState
                        && text.getState() == null) {
                    return text;
                }
            }
    ...
    public Tooltip addTooltip(final TooltipWay way, final Integer delay,
            final String text) {
    ...
    public Value clone() {
        ...
        value.setTooltips(tooltips != null ? CollectionUtils.clone(tooltips)
                : null);
        return value;
    }
    ...
    public String toString() {
        return langText()
                ...
                + (hotkey != null ? " (" + hotkey + ")" : "")
                + (tooltips != null && tooltips.size() > 0 ? " (tooltips: "
                        + tooltips.size() + ")" : "");
    }
}