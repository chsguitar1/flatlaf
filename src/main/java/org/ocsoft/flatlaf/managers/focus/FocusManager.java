...
/**
 * This manager allows you to track certain component their childs focus state
 * by adding your custom FocusTracker or global focus listeners to track
...
public class FocusManager {
     ...
     */
    protected static final List<GlobalFocusListener> globalFocusListeners = new ArrayList<GlobalFocusListener>(
            2);
    ...
    public static synchronized void initialize() {
        ...
        if (!initialized) {
                        ...
                        public void eventDispatched(final AWTEvent event) {
                            if (event instanceof FocusEvent) {
                                if (globalFocusListeners.size() > 0) {
                                    // Filtering unnecessary events (each focus
                                    // change within application generates 2
                                    // events - lost/gained)
                                    ...
                                    if (focusEvent.getID() == FocusEvent.FOCUS_LOST
                                        ...
                                        fireGlobalFocusChanged(
                                                focusEvent.getComponent(), null);
                                    } else if (focusEvent.getID() == FocusEvent.FOCUS_GAINED) {
                                        // Focus changed within the application
                                        // (or might have just entered the
                                        // window)
                                        fireGlobalFocusChanged(focusEvent
            ...
            registerGlobalFocusListener(new GlobalFocusListener() {
                @Override
                public void focusChanged(final Component oldFocus,
                    ...
                    if (FlatLafConstants.DEBUG) {
                        final String oldName = oldFocus != null ? oldFocus
                                .getClass().getName() : null;
                        final String newName = newFocus != null ? newFocus
                                .getClass().getName() : null;
                        FlatLafLogger.debug(this, "Focus changed: " + oldName
                                + " --> " + newName);
                    }
                    
                    // Checking all added trackers
                    // Iterating through registered components
                    for (final Map.Entry<Component, Map<FocusTracker, Boolean>> entry : getTrackersCopy()
                            .entrySet()) {
                        // Retrieving tracked component
                        final Component tracked = entry.getKey();
                        if (tracked != null) {
                            // Iterating through registered component trackers
                            for (final Map.Entry<FocusTracker, Boolean> innerEntry : entry
                                    .getValue().entrySet()) {
                                // Skip if tracker is disabled
                                final FocusTracker focusTracker = innerEntry
                                        .getKey();
                                if (focusTracker.isTrackingEnabled()) {
                                    // Checking whether or not component is
                                    // related to this focus change
                                    final boolean isOldFocused = focusTracker
                                            .isInvolved(oldFocus, tracked);
                                    final boolean isNewFocused = focusTracker
                                            .isInvolved(newFocus, tracked);
                                    
                                    // Informing object only if it is involved
                                    // in changes
                                    if (isOldFocused || isNewFocused) {
                                        // Informing about focus changes if
                                        // needed
                                        final Boolean trackerStateCache = innerEntry
                                                .getValue();
                                        if (trackerStateCache == null
                                                || trackerStateCache != isNewFocused) {
                                            // Informing tracker about focus
                                            // change
                                            focusTracker
                                                    .focusChanged(isNewFocused);
                                            
                                            // Caching focus state
                                            synchronized (trackersLock) {
                                                final Map<FocusTracker, Boolean> ct = trackers
                                                        .get(tracked);
                                                if (ct != null
                                                        && ct.containsKey(focusTracker)) {
                                                    ct.put(focusTracker,
                                                            isNewFocused);
                                                }
                                            }
    ...
    protected static Map<Component, Map<FocusTracker, Boolean>> getTrackersCopy() {
        // Checking all added trackers
        synchronized (trackersLock) {
            final Map<Component, Map<FocusTracker, Boolean>> copy = new HashMap<Component, Map<FocusTracker, Boolean>>(
                    trackers.size());
            for (final Map.Entry<Component, Map<FocusTracker, Boolean>> entry : trackers
                    .entrySet()) {
                final Map<FocusTracker, Boolean> trackers = entry.getValue();
                final Map<FocusTracker, Boolean> trackersCopy = new HashMap<FocusTracker, Boolean>(
                        trackers.size());
                for (final Map.Entry<FocusTracker, Boolean> innerEntry : trackers
                        .entrySet()) {
                    trackersCopy
     ...
     *
     * @param listener
     *            new global focus listener
     */
    public static void registerGlobalFocusListener(
            final GlobalFocusListener listener) {
     ...
     *
     * @param listener
     *            global focus listener to unregister
     */
    public static void unregisterGlobalFocusListener(
            final GlobalFocusListener listener) {
        synchronized (listenersLock) {
     ...
     */
    protected static void fireGlobalFocusChanged(final Component oldComponent,
     ...
     * <p/>
     * Be aware that when all links to either component or focus tracker
     * (outside of the FocusManager) are lost all component focus trackers or
     * specific focus tracker will be disposed. So make sure you keep strong
     * references to whatever you pass here.
     *
     * @param focusTracker
     *            new focus tracker
     */
    public static void addFocusTracker(final Component component,
            final FocusTracker focusTracker) {
        synchronized (trackersLock) {
            Map<FocusTracker, Boolean> componentTrackers = trackers
                    .get(component);
            if (componentTrackers == null) {
                // Trackers must also be kept in a weak references as they might
                // have links leading to component
                // That caused most of memory leak issues in previous tracker
                // versions
            ...
            componentTrackers.put(focusTracker,
                    focusTracker.isInvolved(getFocusOwner(), component));
        }
    }
     ...
     *
     * @param focusTracker
     *            focus tracker to unregister
     */
    public static void removeFocusTracker(final FocusTracker focusTracker) {
        synchronized (trackersLock) {
            final Iterator<Map.Entry<Component, Map<FocusTracker, Boolean>>> iterator = trackers
                    .entrySet().iterator();
            while (iterator.hasNext()) {
                final Map.Entry<Component, Map<FocusTracker, Boolean>> entry = iterator
                        .next();
                final Map<FocusTracker, Boolean> componentTrackers = entry
                        .getValue();
    ...
    public static void removeFocusTrackers(final Component component) {
        synchronized (trackersLock) {
            final Map<FocusTracker, Boolean> allTrackers = trackers
                    .get(component);
