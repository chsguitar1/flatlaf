 ...
 * tooltips, highlights, animations, transitions and other custom stuff atop of
 * all components visible in window/applet. It can also be used to bring your
 * own custom features.
 *
 * @author Mikle Garin
...
public class WebGlassPane extends WebPanel {
    ...
    /**
     * Returns whether WebGlassPane should absorb mouse event from the specified
     * point or not. Custom hit shape is used to allow mouse events passthrough
     ...
     * @param y
     *            mouse Y coordinate
     * @return true if WebGlassPane should absorb mouse event from the specified
    ...
    /**
     * Returns custom hit shape that is used to allow mouse events passthrough
     * where it is required.
     *
     * @return custom hit shape that is used to allow mouse events passthrough
     *         where it is required
     */
    ...
    /**
     * Sets custom hit shape that is used to allow mouse events passthrough
     * where it is required.
     *
     * @param hitShape
     *            custom hit shape that is used to allow mouse events
     *            passthrough where it is required
     */
     ...
     *            image opacity
     */
    public void setPaintedImage(final BufferedImage image,
            final Point location, final int opacity) {
        ...
        if (this.imageOpacity != 0 && this.paintedImage != null
                && this.imageLocation != null) {
            repaintRect = getPaintedImageBounds();
            if (oldRect != null) {
                repaintRect = new Rectangle(Math.max(0,
                        Math.min(oldRect.x, repaintRect.x)), Math.max(0,
                        Math.min(oldRect.y, repaintRect.y)), Math.max(oldRect.x
                        + oldRect.width, repaintRect.x + repaintRect.width),
                        Math.max(oldRect.y + oldRect.height, repaintRect.y
                                + repaintRect.height));
            }
        }
        
        final Rectangle finalRepaintRect = repaintRect != null ? repaintRect
                : oldRect;
     ...
     *
     * @param components
     *            components to remove highlight from
     */
    ...
    // public void displayFocusChange ( Component oldComponent, Component
    // newComponent )
    // {
    ...
    protected void paintComponent(final Graphics g) {
        ...
        if (highlightedComponents.size() > 0) {
            final Rectangle baseBounds = SwingUtils.getRelativeBounds(
                    highlightBase, WebGlassPane.this);
            final Area area = new Area(new Rectangle(baseBounds.x - 1,
                    baseBounds.y - 1, baseBounds.width + 1,
                    baseBounds.height + 1));
            for (final Component component : highlightedComponents) {
                if (component.isShowing()) {
                    final Rectangle bounds = SwingUtils.getRelativeBounds(
                            component, WebGlassPane.this);
                    final RoundRectangle2D.Double shape = new RoundRectangle2D.Double(
                            bounds.x - highlightSpacing, bounds.y
                                    - highlightSpacing, bounds.width
                                    + highlightSpacing * 2 - 1, bounds.height
                                    + highlightSpacing * 2 - 1, 8, 8);
        ...
        if (imageOpacity != 0 && paintedImage != null && imageLocation != null) {
            ...
            if (imageOpacity != 100) {
                g2d.setComposite(AlphaComposite.getInstance(
                        AlphaComposite.SRC_OVER, (float) imageOpacity / 100));
            }
