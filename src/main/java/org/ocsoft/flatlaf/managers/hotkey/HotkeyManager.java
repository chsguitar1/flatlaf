 ...
 * on menu items in menubar menus) for any Swing component. Additionally you can
 * specify a component which will limit hotkey events to its area (meaning that
 * hotkey event will occur only if this component or any of its childs is
 * focused when hotkey pressed).
 * <p/>
 * TooltipManager is integrated with this manager to automatically show
 * component hotkeys in its tooltip if needed/allowed by tooltip and hotkey
 * settings.
 * <p/>
 * All hotkeys are stored into WeakHashMap so hotkeys will be removed as soon as
 * the component for which hotkey is registered gets finalized. HotkeyInfo also
 * keeps a weak reference to both top and hotkey components.
 *
 * @author Mikle Garin
 */
...
public class HotkeyManager {
            ...
            Toolkit.getDefaultToolkit().addAWTEventListener(
                    new AWTEventListener() {
                        ...
                        public void eventDispatched(final AWTEvent event) {
                            // Only if hotkeys enabled and we recieved a
                            // KeyEvent
    ...
    /**
     * Returns a full copy of hotkeys map. Returned map is a HashMap instead of
     * WeakHashMap used in manager and will keep stong references to hotkey
     * components.
     *
    ...
    protected static Map<Component, List<HotkeyInfo>> copyHotkeys() {
        synchronized (sync) {
            final Map<Component, List<HotkeyInfo>> copy = new HashMap<Component, List<HotkeyInfo>>(
                    hotkeys.size());
            for (final Map.Entry<Component, List<HotkeyInfo>> entry : hotkeys
                    .entrySet()) {
    ...
    protected static Map<Container, List<HotkeyCondition>> copyContainerConditions() {
        synchronized (sync) {
            final Map<Container, List<HotkeyCondition>> copy = new HashMap<Container, List<HotkeyCondition>>(
                    containerConditions.size());
            for (final Map.Entry<Container, List<HotkeyCondition>> entry : containerConditions
                    .entrySet()) {
                copy.put(entry.getKey(), CollectionUtils.copy(entry.getValue()));
            }
            return copy;
    ...
    /**
     * Returns whether at least one hotkey for the specified key event exists or
     * not.
     *
     * @param keyEvent
     *            key event to search hotkeys for
     * @return true if at least one hotkey for the specified key event exists,
     *         false otherwise
     */
    protected static boolean hotkeyForEventExists(final KeyEvent keyEvent) {
        ...
        for (final Map.Entry<Component, List<HotkeyInfo>> entry : copyHotkeys()
                .entrySet()) {
    ...
    protected static void processHotkeys(final KeyEvent e) {
        ...
        for (final Map.Entry<Component, List<HotkeyInfo>> entry : hotkeysCopy
                .entrySet()) {
            for (final HotkeyInfo hotkeyInfo : entry.getValue()) {
                ...
                if (forComponent == null) {
                    // Checking hotkey
                    if (hotkeyInfo.getHotkeyData().isTriggered(e)
                            && hotkeyInfo.getAction() != null) {
                    ...
                    topComponent = topComponent != null ? topComponent
                            : SwingUtils.getWindowAncestor(forComponent);
                    ...
                    if (SwingUtils.hasFocusOwner(topComponent)) {
                        ...
                        if (hotkeyInfo.getHotkeyData().isTriggered(e)
                                && hotkeyInfo.getAction() != null) {
                            // Checking that hotkey meets parent containers
                            // conditions
                            if (meetsParentConditions(forComponent)) {
                                ...
                                // Performing hotkey action
                                SwingUtils.invokeLater(hotkeyInfo.getAction(),
                                        e);
                            }
                        }
                    }
    ...
    protected static boolean meetsParentConditions(final Component forComponent) {
        for (final Map.Entry<Container, List<HotkeyCondition>> entry : copyContainerConditions()
                .entrySet()) {
    ...
    public static HotkeyInfo registerHotkey(final HotkeyData hotkeyData,
    ...
    public static HotkeyInfo registerHotkey(final Component forComponent,
            final HotkeyData hotkeyData, final HotkeyRunnable action) {
    ...
    public static HotkeyInfo registerHotkey(final Component topComponent,
            final Component forComponent, final HotkeyData hotkeyData,
            final HotkeyRunnable action) {
        return registerHotkey(topComponent, forComponent, hotkeyData, action,
                false);
    }
    ...
    public static HotkeyInfo registerHotkey(final AbstractButton forComponent,
            final HotkeyData hotkeyData) {
        return registerHotkey(null, forComponent, hotkeyData);
    }
    
    public static HotkeyInfo registerHotkey(final AbstractButton forComponent,
            final HotkeyData hotkeyData, final boolean hidden) {
        return registerHotkey(null, forComponent, hotkeyData, hidden);
    }
    
    public static HotkeyInfo registerHotkey(final AbstractButton forComponent,
            final HotkeyData hotkeyData, final TooltipWay tooltipWay) {
        return registerHotkey(null, forComponent, hotkeyData, tooltipWay);
    }
    
    public static HotkeyInfo registerHotkey(final Component topComponent,
            final AbstractButton forComponent, final HotkeyData hotkeyData) {
    ...
    public static HotkeyInfo registerHotkey(final Component topComponent,
            final AbstractButton forComponent, final HotkeyData hotkeyData,
            final TooltipWay tooltipWay) {
        return registerHotkey(topComponent, forComponent, hotkeyData,
                createAction(forComponent), false, tooltipWay);
    }
    ...
    public static HotkeyInfo registerHotkey(final Component topComponent,
            ...
            final boolean hidden) {
        return registerHotkey(topComponent, forComponent, hotkeyData,
                createAction(forComponent), hidden, null);
    }
    
    protected static HotkeyRunnable createAction(
    ...
    public static void setComponentHotkeyDisplayWay(final Component component,
            final TooltipWay tooltipWay) {
        synchronized (sync) {
    ...
    public static void addContainerHotkeyCondition(final Container container,
            final HotkeyCondition hotkeyCondition) {
        synchronized (sync) {
    ...
    public static void removeContainerHotkeyCondition(
            final Container container, final HotkeyCondition hotkeyCondition) {
        synchronized (sync) {
            final List<HotkeyCondition> clist = getContainerHotkeyConditionsCache(container);
    ...
    public static List<HotkeyCondition> getContainerHotkeyConditions(
            final Container container) {
        synchronized (sync) {
            final List<HotkeyCondition> list = containerConditions
                    .get(container);
            return list != null ? CollectionUtils.copy(list)
                    : new ArrayList<HotkeyCondition>();
        }
    }
    
    protected static List<HotkeyCondition> getContainerHotkeyConditionsCache(
            final Container container) {
        synchronized (sync) {
            final List<HotkeyCondition> list = containerConditions
                    .get(container);
    ...
    public static List<HotkeyInfo> getComponentHotkeys(final Component component) {
        synchronized (sync) {
            ...
            return list != null ? CollectionUtils.copy(list)
                    : new ArrayList<HotkeyInfo>();
        }
    }
    
    /**
     * All added hotkeys. Make sure you know what you are doing if you want to
     * modify this map from outside, otherwise your actions might change
     * HotkeyManager behavior.
     */
    ...
    protected static void cacheHotkey(final HotkeyInfo hotkeyInfo) {
        synchronized (sync) {
            final List<HotkeyInfo> hlist = getComponentHotkeysCache(hotkeyInfo
                    .getForComponent());
    ...
    protected static List<HotkeyInfo> getComponentHotkeysCache(
        ...
        for (final Map.Entry<Component, List<HotkeyInfo>> entry : copyHotkeys()
            ...
            for (final HotkeyInfo hotkeyInfo : entry.getValue()) {
                if (!hotkeyInfo.isHidden()) {
                    final Component forComponent = hotkeyInfo.getForComponent();
                    if (forComponent != null
                            && !shown.contains(forComponent)
                            && forComponent.isVisible()
                            && forComponent.isShowing()
                            && SwingUtils.getWindowAncestor(forComponent) == window) {
                        final WebLabel tip = new WebLabel(
                                HotkeyManager
                                        .getComponentHotkeysString(forComponent));
                        tip.setStyleId("custom-tooltip-hotkey-label");
                        tip.setBoldFont();
                        TooltipManager.showOneTimeTooltip(forComponent, null,
                                tip, hotkeyInfo.getHotkeyDisplayWay());
    ...
    public static void installShowAllHotkeysAction(
            final Component topComponent, final HotkeyData hotkeyData) {
        HotkeyManager.registerHotkey(topComponent, topComponent, hotkeyData,
                new HotkeyRunnable() {
                    @Override
    ...
    protected static String getComponentHotkeysString(
            final List<HotkeyInfo> infoList) {
