...
public class WebTableHeaderUI extends BasicTableHeaderUI {
    ...
    public void paint(final Graphics g, final JComponent c) {
        ...
        final Paint bgPaint = createBackgroundPaint(0, 1, 0,
                header.getHeight() - 1);
        ...
        g2d.drawLine(0, header.getHeight() - 1, header.getWidth(),
                header.getHeight() - 1);
        ...
        if (draggedColumn != null) {
            ...
            // Background
            g2d.setPaint(bgPaint);
            g2d.fillRect(draggedCellRect.x - 1, draggedCellRect.y,
                    draggedCellRect.width, draggedCellRect.height - 1);
            
            // Header cell
            paintCell(g, draggedCellRect, draggedColumnIndex, draggedColumn,
                    draggedColumn, cm);
        }
    }
    
    public static Paint createBackgroundPaint(final int x1, final int y1,
            final int x2, final int y2) {
        if (bottomBgColor == null
                || CompareUtils.equals(topBgColor, bottomBgColor)) {
            return topBgColor;
        } else {
    ...
    private void paintCell(final Graphics g, final Rectangle rect,
            final int columnIndex, final TableColumn tc, final TableColumn dc,
        ...
        rendererPane.paintComponent(g, component, header, rect.x, rect.y,
                rect.width, rect.height, true);
        
        // Right side border
        if (tc == dc
                || (ltr ? columnIndex != cm.getColumnCount() - 1
                        : columnIndex != 0)) {
            g.setColor(WebTableStyle.gridColor);
            g.drawLine(rect.x + rect.width - 1, rect.y + 2, rect.x + rect.width
                    - 1, rect.y + rect.height - 4);
        }
    }
    
    private Component getHeaderRenderer(final int columnIndex) {
        final TableColumn aColumn = header.getColumnModel().getColumn(
                columnIndex);
        ...
        final boolean hasFocus = !header.isPaintingForPrint()
                && header.hasFocus();
