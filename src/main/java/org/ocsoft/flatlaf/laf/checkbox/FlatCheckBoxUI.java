...
public class FlatCheckBoxUI extends BasicCheckBoxUI implements ShapeProvider {
    /**
    ...
    public void installUI(final JComponent c) {
        ...
        checkIcon.setState(checkBox.isSelected() ? CheckState.checked
    ...
    protected void installRolloverListeners() {
        ...
        bgTimer = new WebTimer("WebCheckBoxUI.bgUpdater", UPDATE_DELAY,
                new ActionListener() {
                    @Override
    ...
    protected void installStateChangeListeners() {
        ...
        checkTimer = new WebTimer("WebCheckBoxUI.iconUpdater", UPDATE_DELAY,
                new ActionListener() {
                    @Override
                    public void actionPerformed(final ActionEvent e) {
    ...
    /**
     * Performs visual state change. In case animation is possible and enabled
     * state change will be animated.
     */
    protected void performStateChanged() {
        if (isAnimationAllowed() && isAnimated() && checkBox.isEnabled()) {
            checkIcon.setNextState(checkBox.isSelected() ? CheckState.checked
                    : CheckState.unchecked);
            checkTimer.start();
        } else {
            checkTimer.stop();
    ...
    public Shape provideShape() {
        return LafUtils
                .getWebBorderShape(checkBox, getShadeWidth(), getRound());
    }
    ...
    protected boolean isAnimatedByDefault() {
        ...
        return WebCheckBoxStyle.animated
                && !ReflectUtils.containsInClassOrSuperclassName(
                        checkBox.getClass(),
    ...
    protected boolean isAnimationAllowed() {
        ...
        if (parent == null) {
            ...
            return !ReflectUtils.containsInClassOrSuperclassName(parentClass,
                    "com.jidesoft.swing.CheckBoxList")
                    && !ReflectUtils.containsInClassOrSuperclassName(
                            parentClass,
                            "com.jidesoft.swing.CheckBoxTreeCellRenderer");
        }
    }
    ...
    public boolean isAnimated() {
        return animated
                && (checkBox == null || checkBox.getParent() == null || !(checkBox
                        .getParent() instanceof WebListElement || checkBox
                        .getParent() instanceof TreeCellRenderer));
    }
     ...
     *
     * @param animated
     *            whether checkbox is animated or not
     */
     ...
     *            whether should draw dark border only on rollover or not
     */
    ...
    protected Color[] getBgColors() {
        if (checkBox.isEnabled()) {
            ...
            if (darkness < 1f) {
                return new Color[] {
                        ColorUtils.getIntermediateColor(topBgColor,
                                topSelectedBgColor, darkness),
                        ColorUtils.getIntermediateColor(bottomBgColor,
                                bottomSelectedBgColor, darkness) };
            } else {
     ...
     *            2D graphics context
     * @param x
     *            icon location X coordinate
     * @param y
     *            icon location Y coordinate
     */
     ...
     *            icon location Y coordinate
     */
    protected void paintIconBackground(final Graphics2D g2d, final int x,
            final int y) {
        final boolean enabled = checkBox.isEnabled();
        
        // Button size and shape
        final Rectangle iconRect = new Rectangle(x + shadeWidth,
                y + shadeWidth, iconWidth - shadeWidth * 2 - 1, iconHeight
                        - shadeWidth * 2 - 1);
        final RoundRectangle2D shape = new RoundRectangle2D.Double(iconRect.x,
                iconRect.y, iconRect.width, iconRect.height, round * 2,
                round * 2);
        
        // Shade
        if (enabled) {
            final Color shadeColor = checkBox.isFocusOwner() ? FlatLafStyleConstants.fieldFocusColor
                    : FlatLafStyleConstants.shadeColor;
            GraphicsUtils.drawShade(g2d, shape, shadeColor, shadeWidth);
        }
        
        // Background
        final int radius = Math.round((float) Math.sqrt(iconRect.width
                * iconRect.width / 2));
        final int cx = iconRect.x + iconRect.width / 2;
        final int cy = iconRect.y + iconRect.height / 2;
        g2d.setPaint(new RadialGradientPaint(cx, cy, radius, fractions,
                getBgColors()));
        g2d.fill(shape);
        
        // Border
        final Stroke os = GraphicsUtils.setupStroke(g2d, borderStroke);
        g2d.setPaint(enabled ? (rolloverDarkBorderOnly ? ColorUtils
                .getIntermediateColor(borderColor, darkBorderColor,
                        getBgDarkness()) : darkBorderColor)
                : disabledBorderColor);
        g2d.draw(shape);
    ...
    protected class CheckBoxIcon implements Icon {
        ...
        public void paintIcon(final Component c, final Graphics g, final int x,
            ...
            FlatCheckBoxUI.this.paintIcon(g2d, x, y);
        ...
        @Override
        public int getIconWidth() {
            return FlatCheckBoxUI.this.getIconWidth();
        }
        ...
        @Override
        public int getIconHeight() {
            return FlatCheckBoxUI.this.getIconHeight();
        }
    }
}