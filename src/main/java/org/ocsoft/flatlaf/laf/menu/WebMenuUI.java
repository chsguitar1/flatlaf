...
public class WebMenuUI extends BasicMenuUI implements BorderMethods {
    ...
    /**
     * Returns an instance of the WebMenuUI for the specified component. This
     ...
     *
     * @param shadeWidth
     *            new top-level menu shade width
     */
     ...
     *
     * @param gap
     *            new gap between menu icon/text and submenu arrow
     */
    ...
    public void paint(final Graphics g, final JComponent c) {
        ...
        final boolean selected = menu.isEnabled()
        ...
        if (arrowIcon != null) {
            final Composite oc = GraphicsUtils.setupAlphaComposite(g2d, 0.4f,
                    selected);
            arrowIcon.paintIcon(menu, g2d,
                    ltr ? w - bi.right - arrowIcon.getIconWidth() : bi.left, y
                            + ih / 2 - arrowIcon.getIconHeight() / 2);
    ...
    protected void paintBackground(final Graphics2D g2d, final JMenu menu,
        ...
        if (painter != null) {
            ...
            if (menu.getParent() instanceof JPopupMenu) {
                if (selected) {
                    g2d.setPaint(new GradientPaint(0, 0, selectedTopBg, 0, menu
                            .getHeight(), selectedBottomBg));
                    g2d.fillRect(0, 0, menu.getWidth(), menu.getHeight());
                }
            } else {
                if (menu.isEnabled() && (selected || mouseover)) {
                    LafUtils.drawWebStyle(g2d, menu,
                            FlatLafStyleConstants.shadeColor, shadeWidth,
                            round, menu.isEnabled(), !selected && mouseover,
                            selected ? FlatLafStyleConstants.averageBorderColor
    ...
    protected void paintIcon(final Graphics2D g2d, final JMenu menu,
            ...
            final boolean selected, final boolean ltr) {
        final Icon icon = menu.isEnabled() ? menu.getIcon() : menu
                .getDisabledIcon();
        if (icon != null) {
            ...
            icon.paintIcon(menu, g2d, iconX, y + h / 2 - icon.getIconHeight()
                    / 2);
        }
    }
    ...
    protected void paintText(final Graphics2D g2d, final JMenu menu,
        ...
        if (html != null) {
            ...
            final int mnem = FlatLookAndFeel.isMnemonicHidden() ? -1 : menu
                    .getDisplayedMnemonicIndex();
            SwingUtils.drawStringUnderlineCharAt(g2d, menu.getText(), mnem, x,
                    y + h / 2 + LafUtils.getTextCenterShearY(fm));
        }
    }
    ...
    protected Icon getArrowIcon(final JMenu menu) {
        if (menu.getParent() instanceof JPopupMenu) {
            ...
            if (menu.isEnabled()) {
                ...
                return ltr ? ImageUtils.getDisabledCopy("Menu.arrowRightIcon",
                        arrowRightIcon) : ImageUtils.getDisabledCopy(
                        "Menu.arrowLeftIcon", arrowLeftIcon);
            }
        } else {
            return null;
        }
    ...
    public Dimension getPreferredSize(final JComponent c) {
        ...
        // Icon-Text gap
        final int gap = textWidth > 0 && iconPlaceholderWidth > 0 ? menu
                .getIconTextGap() : 0;
        
        // Sub-menu arrow icon
        final Icon subMenuArrowIcon = getArrowIcon(menu);
        final int arrowWidth = subMenuArrowIcon != null ? arrowGap
                + subMenuArrowIcon.getIconWidth() : 0;
        
        // Content height
        final int iconHeight = menu.getIcon() != null ? menu.getIcon()
                .getIconHeight() : 0;
        ...
        return new Dimension(bi.left + iconPlaceholderWidth + gap + textWidth
                + arrowWidth + bi.right, bi.top + contentHeight + bi.bottom);
    }
}