...
public class WebMenuItemUI extends BasicMenuItemUI implements BorderMethods {
    ...
    public void updateBorder() {
        if (menuItem != null) {
            ...
            // Actual margin
            final boolean ltr = menuItem.getComponentOrientation()
                    .isLeftToRight();
    ...
    /**
     * Returns paint used to fill north popup menu corner when this component is
     * first in the menu.
     *
     * @return paint used to fill north popup menu corner when this component is
     *         first in the menu
     */
    ...
    /**
     * Returns paint used to fill south popup menu corner when this component is
     * last in the menu.
     *
     * @return paint used to fill south popup menu corner when this component is
     *         last in the menu
     */
    ...
    public void paint(final Graphics g, final JComponent c) {
        ...
        final int iconPlaceholderWidth = MenuUtils.getIconPlaceholderWidth(
                menuItem, alignTextToMenuIcons);
        final int gap = iconPlaceholderWidth > 0 ? menuItem.getIconTextGap()
                : 0;
        int x = ltr ? bi.left : w - bi.right - iconPlaceholderWidth;
        paintBackground(g2d, menuItem, x, y, iconPlaceholderWidth, ih,
                selected, ltr);
        ...
        if (hasText || hasAccelerator) {
            ...
            // Painting text
            if (hasText) {
                final FontMetrics fm = menuItem.getFontMetrics(menuItem
                        .getFont());
                final View html = (View) menuItem
                        .getClientProperty(BasicHTML.propertyKey);
                final int tw = html != null ? (int) html
                        .getPreferredSpan(View.X_AXIS) : fm.stringWidth(text);
            ...
            if (hasAccelerator) {
                final FontMetrics afm = menuItem
                        .getFontMetrics(acceleratorFont);
                final int aw = afm.stringWidth(accText);
                
                x = ltr ? w - bi.right - aw : bi.left;
                paintAcceleratorText(g2d, menuItem, accText, afm, x, y, aw, ih,
                        selected, ltr);
            }
     ...
     *            menu item
     * @param x
     *            icon placeholder X coordinate
     * @param y
     *            icon placeholder Y coordinate
     * @param w
     *            icon placeholder width
     * @param h
     *            icon placeholder height
     * @param selected
     *            whether menu item is selected or not
     * @param ltr
    ...
    protected void paintBackground(final Graphics2D g2d,
        ...
        if (painter != null) {
            ...
            if (selected) {
                g2d.setPaint(new GradientPaint(0, 0, selectedTopBg, 0, menuItem
                        .getHeight(), selectedBottomBg));
    ...
    protected void paintIcon(final Graphics2D g2d, final JMenuItem menuItem,
        ...
        final Icon icon = menuItem.isSelected()
                && menuItem.getSelectedIcon() != null ? (enabled ? menuItem
                .getSelectedIcon() : menuItem.getDisabledSelectedIcon())
        ...
        if (icon != null) {
            ...
            final int iconX = left ? x : center ? x + w / 2
                    - icon.getIconWidth() / 2 : x + w - icon.getIconWidth();
            icon.paintIcon(menuItem, g2d, iconX,
                    y + h / 2 - icon.getIconHeight() / 2);
        }
    }
    ...
    protected void paintText(final Graphics2D g2d, final JMenuItem menuItem,
            ...
            final int h, final boolean selected, final boolean ltr) {
        g2d.setPaint(menuItem.isEnabled() ? menuItem.getForeground()
                : disabledFg);
        ...
        if (html != null) {
            ...
            final int mnem = FlatLookAndFeel.isMnemonicHidden() ? -1 : menuItem
                    .getDisplayedMnemonicIndex();
            SwingUtils.drawStringUnderlineCharAt(g2d, menuItem.getText(), mnem,
                    x, y + h / 2 + LafUtils.getTextCenterShearY(fm));
        }
    }
     ...
     *            accelerator text
     * @param fm
     *            accelerator text font metrics
     * @param x
     *            accelerator text X coordinate
     * @param y
     *            accelerator text rectangle Y coordinate
     * @param w
     *            accelerator text width
     * @param h
     *            accelerator text rectangle height
     * @param selected
     *            whether menu item is selected or not
     * @param ltr
     *            whether menu item has left-to-right orientation or not
    ...
    protected void paintAcceleratorText(final Graphics2D g2d,
        ...
        g2d.setPaint(menuItem.isEnabled() ? acceleratorFg
    ...
    public Dimension getPreferredSize(final JComponent c) {
        ...
        if (html != null) {
            ...
            // Text isn't HTML
            final String text = menuItem.getText();
            textWidth = text != null && text.length() > 0 ? fm
                    .stringWidth(text) : 0;
            textHeight = fm.getHeight();
        }
        
        // Icon-Text gap
        final int gap = textWidth > 0 && iconPlaceholderWidth > 0 ? menuItem
                .getIconTextGap() : 0;
        
        // Acceleration text and its gap
        final String accelerationText = MenuUtils.getAcceleratorText(menuItem);
        final int accWidth = accelerationText != null ? acceleratorGap
                + afm.stringWidth(accelerationText) : 0;
        
        // Content height
        final int iconHeight = menuItem.getIcon() != null ? menuItem.getIcon()
                .getIconHeight() : 0;
        final int contentHeight = MathUtils.max(iconHeight, textHeight,
                afm.getHeight());
        
        return new Dimension(bi.left + iconPlaceholderWidth + gap + textWidth
                + accWidth + bi.right, bi.top + contentHeight + bi.bottom);
    }
}