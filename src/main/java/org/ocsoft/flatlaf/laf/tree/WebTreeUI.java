...
public class WebTreeUI extends BasicTreeUI {
    ...
    public static ImageIcon DISABLED_COLLAPSE_ICON = ImageUtils
            .createDisabledCopy(COLLAPSE_ICON);
    ...
    public void installUI(final JComponent c) {
        ...
        // Forces tree to save changes when another tree node is selected
        ...
        dropLocationChangeListener = new PropertyChangeListener() {
            ...
            public void propertyChange(final PropertyChangeEvent evt) {
                ...
                final JTree.DropLocation oldLocation = (JTree.DropLocation) evt
                        .getOldValue();
                if (oldLocation != null) {
                    tree.repaint(getNodeDropLocationBounds(oldLocation
                            .getPath()));
                }
                
                // Repainting current drop location
                final JTree.DropLocation newLocation = (JTree.DropLocation) evt
                        .getNewValue();
                if (newLocation != null) {
                    tree.repaint(getNodeDropLocationBounds(newLocation
                            .getPath()));
                }
            }
        };
        ...
        mouseAdapter = new MouseAdapter() {
                ...
                if (SwingUtilities.isLeftMouseButton(e)) {
                    ...
                    if (getRowForPoint(e.getPoint(), false) == -1) {
                        if (isSelectorAvailable()) {
                            ...
                            // Avoiding selection start when pressed on tree
                            // expand handle
                            final TreePath path = getClosestPathForLocation(
                                    tree, e.getX(), e.getY());
                            if (path == null
                                    || !isLocationInExpandControl(path,
                                            e.getX(), e.getY())
                                    && !isLocationInCheckBoxControl(path,
                                            e.getX(), e.getY())) {
                                // Avoid starting multiselection if row is
                                // selected and drag is possible
                                final int rowForPath = getRowForPath(tree, path);
                                if (isDragAvailable()
                                        && getRowBounds(rowForPath).contains(
                                                e.getX(), e.getY())
                            ...
                            if (path != null
                                ...
                                if (tree.getSelectionModel().getSelectionMode() == TreeSelectionModel.SINGLE_TREE_SELECTION) {
                                    tree.setSelectionRow(getRowForPoint(
                                            e.getPoint(), true));
                                }
            ...
            public void mouseDragged(final MouseEvent e) {
                if (draggablePath != null) {
                    final TransferHandler transferHandler = tree
                            .getTransferHandler();
                    transferHandler.exportAsDrag(tree, e,
                            transferHandler.getSourceActions(tree));
                    draggablePath = null;
                }
                if (isSelectorAvailable() && selectionStart != null) {
                    ...
                    if (!tree.getVisibleRect().contains(e.getPoint())) {
                        tree.scrollRectToVisible(new Rectangle(e.getPoint(),
                                new Dimension(0, 0)));
                    }
                }
            }
            ...
            public void mouseReleased(final MouseEvent e) {
                ...
                if (isSelectorAvailable() && selectionStart != null) {
                    ...
                    // Rectangle fr = GeometryUtils.getContainingRect (
            ...
            private void validateSelection(final MouseEvent e) {
                // todo Possibly optimize selection? - modify it instead of
                // overwriting each time
                
                // Selection rect
                final Rectangle selection = GeometryUtils.getContainingRect(
                        selectionStart, selectionEnd);
                ...
                if (!CollectionUtils.areEqual(getSelectionRowsList(),
                        newSelection)) {
                    if (newSelection.size() > 0) {
                        tree.setSelectionRows(CollectionUtils
                                .toArray(newSelection));
                    } else {
                        tree.clearSelection();
                    }
                }
            ...
            private void repaintSelector() {
                ...
                // final Rectangle sb1 = GeometryUtils.getContainingRect (
                // selectionStart, selectionPrevEnd );
                // final Rectangle sb2 = GeometryUtils.getContainingRect (
                // selectionStart, selectionEnd );
                //
                ...
                // Replaced with full repaint due to strange tree lines painting
                // bug
    ...
    protected boolean isDragAvailable() {
        return tree != null && tree.isEnabled() && tree.getDragEnabled()
     ...
     *
     * @param autoExpandSelectedNode
     *            whether tree should expand nodes on selection or not
     */
    ...
    public void setSelectionShadeWidth(final int shadeWidth) {
        if (this.selectionShadeWidth != shadeWidth) {
            ...
            // Properly updating the whole tree structure since this value might
            // affect renderer's size
    ...
    /**
     * Returns whether tree selection style points that the whole line is a
     * single cell or not.
     *
     * @return true if tree selection style points that the whole line is a
     *         single cell, false otherwise
     */
    ...
    /**
     * Returns row index for specified point on the tree. This method takes
     * selection style into account.
     *
     * @param point
     *            point on the tree
     ...
     *            point on the tree
     * @param countFullRow
     *            whether take the whole row into account or just node renderer
     *            rect
     * @return row index for specified point on the tree
     */
    ...
    /**
     * Returns row bounds by its index. This method takes selection style into
     * account.
     *
     * @param row
     *            row index
     ...
     * @return true if location is in the checkbox tree checkbox control, false
     *         otherwise
     */
    protected boolean isLocationInCheckBoxControl(final TreePath path,
        ...
        if (tree instanceof WebCheckBoxTree) {
            ...
            if (checkBoxTree.isCheckingByUserEnabled()) {
                ...
                if (checkBoxTree.isCheckBoxVisible(node)
                        && checkBoxTree.isCheckBoxEnabled(node)) {
                    final Rectangle checkBoxBounds = checkBoxTree
                            .getCheckBoxBounds(path);
                    return checkBoxBounds != null
                            && checkBoxBounds.contains(x, y);
                } else {
                    return false;
                }
            } else {
     ...
     */
    @Override
    protected void drawCentered(final Component c, final Graphics g,
            final Icon icon, final int x, final int y) { // x-icon.getIconWidth
                                                         // ()/2-2
        icon.paintIcon(c, g, findCenteredX(x, icon.getIconWidth()),
    ...
    protected int findCenteredX(final int x, final int iconWidth) {
        return leftToRight ? (x - 2 - (int) Math.ceil(iconWidth / 2.0))
                : (x - 1 - (int) Math.floor(iconWidth / 2.0));
    }
    
    /**
     * Repaints all rectangles containing tree selections. This method is
     * optimized to repaint only those area which are actually has selection in
     * them.
     */
    ...
    /**
     * Returns list of tree selections bounds. This method takes selection style
     * into account.
     *
        ...
        for (final int row : rows) {
            if (selectionStyle.equals(TreeSelectionStyle.single)) {
                ...
                if (lastRow == -1 || lastRow + 1 == row) {
                    ...
                    // Increase rect
                    maxRect = lastRow == -1 ? b : GeometryUtils
                            .getContainingRect(maxRect, b);
    ...
    /**
     * Paints special WebLaF tree nodes selection. It is rendered separately
     * from nodes allowing you to simplify your tree cell renderer component.
     *
     * @param g2d
     *            graphics context
     */
    protected void paintSelection(final Graphics2D g2d) {
        if (tree.getSelectionCount() > 0) {
            ...
            for (final Rectangle rect : selections) {
                ...
                rect.width -= selectionShadeWidth * 2
                        + (selectionBorderColor != null ? 1 : 0);
                rect.height -= selectionShadeWidth * 2
                        + (selectionBorderColor != null ? 1 : 0);
                
                // Painting selection
                LafUtils.drawCustomWebBorder(g2d, tree,
                        new RoundRectangle2D.Double(rect.x, rect.y, rect.width,
                                rect.height, selectionRound * 2,
                                selectionRound * 2),
                        FlatLafStyleConstants.shadeColor, selectionShadeWidth,
                        true, webColoredSelection, selectionBorderColor,
    ...
    protected void paintRolloverNodeHighlight(final Graphics2D g2d) {
        if (tree.isEnabled() && highlightRolloverNode && rolloverRow != -1
                && !tree.isRowSelected(rolloverRow)) {
            final Rectangle rect = isFullLineSelection() ? getFullRowBounds(rolloverRow)
                    : tree.getRowBounds(rolloverRow);
            if (rect != null) {
                // Bounds fix
                rect.x += selectionShadeWidth;
                rect.y += selectionShadeWidth;
    ...
    /**
     * Paints all base tree elements. This is almost the same method as in
     * BasicTreeUI but it doesn't paint drop line.
     *
     * @param g2d
     *            graphics context
     */
    protected void paintTree(final Graphics2D g2d) {
        ...
        final Insets insets = tree.getInsets();
        final TreePath initialPath = getClosestPathForLocation(tree, 0,
                paintBounds.y);
        final Enumeration paintingEnumerator = treeState
                ...
                if (path != null) {
                    ...
                    if (parentPath != null) {
                        if (drawingCache.get(parentPath) == null) {
                            paintVerticalPartOfLeg(g2d, paintBounds, insets,
                                    parentPath);
                            drawingCache.put(parentPath, Boolean.TRUE);
                        }
                        paintHorizontalPartOfLeg(g2d, paintBounds, insets,
                                bounds, path, row, isExpanded, hasBeenExpanded,
                                isLeaf);
                    ...
                    if (shouldPaintExpandControl(path, row, isExpanded,
                        ...
                        paintExpandControl(g2d, paintBounds, insets, bounds,
                                path, row, isExpanded, hasBeenExpanded, isLeaf);
                    }
                    paintRow(g2d, paintBounds, insets, bounds, path, row,
    ...
    protected void paintDropLocation(final Graphics2D g2d) {
        ...
        if (dropLocation != null) {
            ...
            if (isDropLine(dropLocation)) {
                ...
                final Color dropLineColor = UIManager
                        .getColor("Tree.dropLineColor");
                final Color[] colors = { background, dropLineColor,
                        dropLineColor, background };
                final Rectangle rect = getDropLineRect(dropLocation);
                g2d.setPaint(new LinearGradientPaint(rect.x, rect.y, rect.x
                        + rect.width, rect.y, fractions, colors));
                ...
                final NinePatchIcon dropShade = NinePatchUtils.getShadeIcon(
                        dropCellShadeWidth, selectionRound, 1f);
    ...
    protected void paintMultiselector(final Graphics2D g2d) {
        if (isSelectorAvailable() && selectionStart != null
            ...
            final Rectangle sb = GeometryUtils.getContainingRect(
                    selectionStart, selectionEnd);
    ...
    /**
     * Returns whether the specified drop location should be displayed as line
     * or not.
     *
     * @param loc
     *            drop location
     * @return true if the specified drop location should be displayed as line,
     *         false otherwise
     */
        ...
        if (tree.getRowCount() == 0) {
            ...
            if (path.getLastPathComponent() == root
                ...
                if (!tree.isRootVisible()) {
                    ...
                    final TreePath lastChildPath = path.pathByAddingChild(model
                ...
                rect = tree.getPathBounds(path.pathByAddingChild(model
     ...
     * @return path bounds
     */
    protected Rectangle getPathBounds(final TreePath path, final Insets insets,
            Rectangle bounds) {
    ...
    protected Shape getSelectionShape(final Rectangle sb, final boolean fill) {
        ...
        if (selectorRound > 0) {
            return new RoundRectangle2D.Double(sb.x + shear, sb.y + shear,
                    sb.width - shear, sb.height - shear, selectorRound * 2,
                    selectorRound * 2);
        } else {
            return new Rectangle2D.Double(sb.x + shear, sb.y + shear, sb.width
                    - shear, sb.height - shear);
        }
    }
