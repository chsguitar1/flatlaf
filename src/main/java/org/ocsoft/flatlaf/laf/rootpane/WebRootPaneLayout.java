...
public class WebRootPaneLayout extends AbstractLayoutManager {
    ...
    public void layoutContainer(final Container parent) {
        ...
        final boolean showWindowButtons = windowButtons != null
                && rootUI.isShowWindowButtons()
                && (rootUI.isShowMinimizeButton()
                        || rootUI.isShowMaximizeButton() || rootUI
                            .isShowCloseButton());
        final boolean showTitleComponent = titleComponent != null
                && rootUI.isShowTitleComponent();
        final boolean showMenuBar = menuBar != null && rootUI.isShowMenuBar();
        final boolean showResizeCorner = resizeCorner != null
                && rootUI.isResizable() && rootUI.isShowResizeCorner()
                && !rootUI.isFrameMaximized();
        ...
        if (showWindowButtons) {
            ...
            final int x = ltr ? s.width - i.right - buttonsShear - ps.width
                    : i.left + buttonsShear;
        ...
        if (showTitleComponent) {
            ...
            titleComponent.setBounds(ltr ? i.left : i.left + buttonsWidth,
                    i.top, w - buttonsWidth, ps.height);
            nextY += ps.height;
        ...
        if (layeredPane != null) {
            layeredPane.setBounds(i.left + ci.left, i.top + ci.top, w - ci.left
                    - ci.right, h - ci.top - ci.bottom);
        }
        ...
        if (contentPane != null) {
            contentPane.setBounds(0, nextY, w - ci.left - ci.right,
                    h < nextY ? 0 : h - nextY - ci.top - ci.bottom);
        }
        ...
        if (showResizeCorner) {
            ...
            resizeCorner.setBounds(s.width - i.right - ps.width - 2, s.height
                    - i.bottom - ps.height - 2, ps.width, ps.height);
     ...
     */
    private Dimension calculateSize(final Container parent,
        ...
        final boolean showMenuBar = menuBar != null && rootUI.isShowMenuBar();
        final boolean showResizeCorner = resizeCorner != null
                && rootUI.isShowResizeCorner() && !rootUI.isFrameMaximized();
            ...
            if (showWindowButtons) {
                ...
                if (showTitleComponent) {
                    if (showMenuBar) {
                        ...
                        tpWidth = Math.max(titleDim.width + buttonsDim.width
                                + buttonsShear, menuDim.width);
                    ...
                    if (showMenuBar) {
                        ...
                        // Buttons only
                        tpWidth = buttonsShear + buttonsDim.width
                                + buttonsShear;
                        tpHeight = 0;
                    }
                }
        ...
        if (showResizeCorner) {
            // Placing resize corner at the top of all other components within
            // the root pane
            // Called from here since root pane content might be changed and we
        ...
        return shadeIcon != null ? SwingUtils.max(d,
                shadeIcon.getPreferredSize()) : d;
    }
    ...
    private int getButtonsShear(final WebRootPaneUI webRootPaneUI) {
        ...
        return webRootPaneUI.isAttachButtons() && round > 0 ? round
