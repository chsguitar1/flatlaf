...
public class WebRootPaneUI extends BasicRootPaneUI implements SwingConstants {
    ...
    public void setShadeWidth(final int shadeWidth) {
        ...
        if (styled && window != null) {
            ...
            window.setBounds(b.x - diff, b.y - diff, b.width + diff * 2,
                    b.height + diff * 2);
            installBorder();
            root.revalidate();
            root.repaint();
    ...
    public ImageIcon getWatermark() {
        if (watermark == null && drawWatermark) {
            watermark = new ImageIcon(
                    WebRootPaneUI.class.getResource("icons/watermark.png"));
        }
        return watermark;
    }
    ...
    protected void installBorder() {
        if (!isFrameMaximized()) {
            root.setBorder(BorderFactory.createEmptyBorder(shadeWidth + 1
                    + margin.top, shadeWidth + 1 + margin.left, shadeWidth + 1
                    + margin.bottom, shadeWidth + 1 + margin.right));
        } else {
            root.setBorder(BorderFactory.createEmptyBorder(1 + margin.top,
                    1 + margin.left, 1 + margin.bottom, 1 + margin.right));
        }
    }
    ...
    /**
     * Appropriate LayoutManager for the window decorations install and
     * uninstall methods
     */
    ...
    protected JComponent createDefaultTitleComponent() {
        ...
        titleLabel.addComponentListener(new ComponentAdapter() {
            ...
            public void componentResized(final ComponentEvent e) {
                titleLabel.setHorizontalAlignment(titleLabel.getRequiredSize().width > titleLabel
                        .getWidth() ? LEADING : CENTER);
            }
        });
        ...
        final ComponentMoveAdapter cma = new ComponentMoveAdapter() {
            ...
            public void mouseClicked(final MouseEvent e) {
                if (isFrame() && isShowMaximizeButton()
    ...
    public class TitleLabel extends WebLabel {
        /**
         * Returns window title text. There is a small workaround to show window
         * title even when it is empty. That workaround allows window dragging
         * even when title is not set.
         *
        ...
        /**
         * Returns preferred title size. There is also a predefined title width
         * limit to force it shrink.
         *
        ...
        if (showMaximizeButton && isResizable() && isFrame) {
            final WebButton maximize = new WebButton(maximizeIcon) {
                ...
                public Icon getRolloverIcon() {
                    return isFrameMaximized() ? restoreActiveIcon
                            : maximizeActiveIcon;
                }
            };
    ...
    protected void updateWindowButtonsStyle() {
        if (windowButtons != null) {
            ...
            windowButtons
                    .setButtonsShadeWidth(WebRootPaneStyle.buttonsShadeWidth);
    ...
    protected ImageIcon getWindowIcon() {
        final List<Image> images = window != null ? window.getIconImages()
                : null;
    ...
    protected boolean isResizable() {
        return isDialog() ? dialog.isResizable() : isFrame()
                && frame.isResizable();
    }
    ...
    public void paint(final Graphics g, final JComponent c) {
        if (styled) {
            ...
            if (max) {
                ...
                // Watermark
                if (drawWatermark) {
                    final Shape old = GraphicsUtils.intersectClip(g2d,
                            getWatermarkClip(c));
                ...
                // Shade
                if (shadeWidth > 0) {
                    final int diff = isActive(c) ? 0 : shadeWidth
                            - inactiveShadeWidth;
                    getShadeIcon(c).paintIcon(g2d, diff, diff,
                            c.getWidth() - diff * 2, c.getHeight() - diff * 2);
                }
                
                // Background
                g2d.setPaint(new GradientPaint(0, shadeWidth, topBg, 0,
                        shadeWidth + 30, middleBg));
                g2d.fillRoundRect(shadeWidth, shadeWidth, c.getWidth()
                        - shadeWidth * 2, c.getHeight() - shadeWidth * 2,
                        round * 2, round * 2);
                
                // Border
                g2d.setPaint(borderColor);
                g2d.drawRoundRect(shadeWidth, shadeWidth, c.getWidth()
                        - shadeWidth * 2 - 1, c.getHeight() - shadeWidth * 2
                        - 1, round * 2 - 2, round * 2 - 2);
                g2d.setPaint(innerBorderColor);
                g2d.drawRoundRect(shadeWidth + 1, shadeWidth + 1, c.getWidth()
                        - shadeWidth * 2 - 3, c.getHeight() - shadeWidth * 2
                        - 3, round * 2 - 4, round * 2 - 4);
                
                // Watermark
                if (drawWatermark) {
                    final Shape old = GraphicsUtils.intersectClip(g2d,
                            getWatermarkClip(c));
                    g2d.drawImage(getWatermark().getImage(), shadeWidth + 2,
                            shadeWidth + 2, null);
                    GraphicsUtils.restoreClip(g2d, old);
                }
            }
    ...
    protected RoundRectangle2D.Double getWatermarkClip(final JComponent c) {
        return new RoundRectangle2D.Double(shadeWidth + 2, shadeWidth + 2,
                c.getWidth() - shadeWidth * 2 - 3, c.getHeight() - shadeWidth
                        * 2 - 3, round * 2 - 4, round * 2 - 4);
    }
