...
public class WebListUI extends BasicListUI {
    /**
     * todo 1. Visual shade effect on sides when scrollable todo 2. Even-odd
     * cells highlight
     */
    ...
    /**
     * Returns an instance of the WebListUI for the specified component. This
    ...
    public void installUI(final JComponent c) {
        ...
        mouseoverAdapter = new MouseAdapter() {
            ...
            private void rolloverIndexChanged(final int oldIndex,
                    final int newIndex) {
                // Destroy old tooltip
                if (oldIndex != -1) {
                    if (tooltip != null) {
                ...
                if (decorateSelection && highlightRolloverCell) {
                    final Rectangle oldBounds = list.getCellBounds(oldIndex,
                            oldIndex);
                    final Rectangle newBounds = list.getCellBounds(newIndex,
                            newIndex);
                    final Rectangle rect = GeometryUtils.getContainingRect(
                            oldBounds, newBounds);
                    if (rect != null) {
                        list.repaint(rect);
                    }
                ...
                if (newIndex != -1) {
                    ...
                    if (provider != null) {
                        ...
                        if (delay <= 0) {
                            ...
                            delayTimer = WebTimer.delay(delay,
                                    new ActionListener() {
                                        @Override
            ...
            private void showTooltip(final int index,
                    final ListToolTipProvider provider) {
                final Object value = list.getModel().getElementAt(index);
                final boolean selected = list.isSelectedIndex(index);
                tooltip = provider.getListCellToolTip(list, value, index,
                        selected);
                tooltip.setRelativeToBounds(provider.getTooltipSourceBounds(
                        list, value, index, selected));
            ...
            public void valueChanged(final ListSelectionEvent e) {
                if (autoScrollToSelection) {
                    if (list.getSelectedIndex() != -1) {
                        ...
                        final Rectangle selection = getCellBounds(list, index,
                                index);
                        if (selection != null
                                && !selection.intersects(list.getVisibleRect())) {
    ...
    protected ListToolTipProvider getToolTipProvider() {
        return list != null && list instanceof WebList ? ((WebList) list)
                .getToolTipProvider() : null;
    }
     ...
     *
     * @return true if should decorate selected and rollover cells, false
     *         otherwise
     */
     ...
     *
     * @param selectionRound
     *            new cells selection rounding
     */
     ...
     *
     * @param selectionShadeWidth
     *            new cells selection shade width
     */
     ...
     *
     * @return true if list is being automatically scrolled to selection, false
     *         otherwise
     */
    ...
    // float opacity = shadeWidth > vr.y ? 1f - ( float ) ( shadeWidth - vr.y )
    // / shadeWidth : 1f;
    // NinePatchIcon npi = NinePatchUtils.getShadeIcon ( shadeWidth, 0, opacity
    // );
    // Dimension ps = npi.getPreferredSize ();
    // npi.paintIcon ( ( Graphics2D ) g, vr.x - shadeWidth, vr.y + shadeWidth -
    // ps.height, vr.width + shadeWidth * 2, ps.height );
    // }
    // }
     ...
     */
    @Override
    protected void paintCell(final Graphics g, final int index,
            final Rectangle rowBounds, final ListCellRenderer cellRenderer,
            final ListModel dataModel, final ListSelectionModel selModel,
            final int leadIndex) {
        // if ( list.getLayoutOrientation () == WebList.VERTICAL && (
        // evenLineColor != null || oddLineColor != null ) )
        // {
        ...
        if (decorateSelection && (isSelected || index == rolloverIndex)) {
            ...
            final Composite oc = GraphicsUtils.setupAlphaComposite(g2d, 0.35f,
                    !isSelected);
            ...
            LafUtils.drawCustomWebBorder(
                    ...
                    new RoundRectangle2D.Double(rect.x, rect.y, rect.width,
                            rect.height, selectionRound * 2, selectionRound * 2),
        ...
        final Component rendererComponent = cellRenderer
                .getListCellRendererComponent(list, value, index, isSelected,
                        cellHasFocus);
        rendererPane.paintComponent(g, rendererComponent, list, rowBounds.x,
                rowBounds.y, rowBounds.width, rowBounds.height, true);
    }
}