...
public class LineColorChooser extends WebPanel {
    public static final ImageIcon LEFT_ICON = new ImageIcon(
            LineColorChooser.class.getResource("icons/left.png"));
    public static final ImageIcon RIGHT_ICON = new ImageIcon(
            LineColorChooser.class.getResource("icons/right.png"));
    ...
    private LineColorChooserPaint lineColorChooserPaint = new LineColorChooserPaint(
            2, 256);
    ...
    private class ColorLineMouseAdapter extends MouseAdapter {
        ...
        private void updateHue(int yCoordinate) {
            setHue(Math.round((float) 360
                    * ((float) (yCoordinate - 5) / (float) (colorLine
                            .getHeight() - 4))), true);
        }
    }
    ...
    public void setColor(Color color) {
        ...
        setHue(360 - Math.round((float) 360
                * Color.RGBtoHSB(color.getRed(), color.getGreen(),
                        color.getBlue(), null)[0]), false);
    }
    
    public Color getColor() {
        return lineColorChooserPaint.getColor(Math.round((float) (colorLine
                .getHeight() - 4) * (float) hue / 360));
    }
    ...
    public void paint(Graphics g) {
        ...
        g.drawImage(
                LEFT_ICON.getImage(),
                1,
                1 + Math.round(((float) hue / 360)
                        * (colorLine.getHeight() - 5)),
                LEFT_ICON.getImageObserver());
        g.drawImage(
                RIGHT_ICON.getImage(),
                getWidth() - RIGHT_ICON.getIconWidth() - 1,
                1 + Math.round(((float) hue / 360)
                        * (colorLine.getHeight() - 5)),
