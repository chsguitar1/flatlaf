...
public class WebTabbedPaneUI extends BasicTabbedPaneUI implements
        ShapeProvider, BorderMethods {
    ...
    public void updateBorder() {
        if (tabPane != null) {
            ...
            if (tabbedPaneStyle.equals(TabbedPaneStyle.standalone)) {
                ...
                tabPane.setBorder(LafUtils.createWebBorder(SwingUtils.max(
                        bgInsets, new Insets(shadeWidth, shadeWidth,
    ...
    public void setSelectedForegroundAt(final int tabIndex,
    ...
    protected boolean shouldPadTabRun(final int tabPlacement, final int run) {
        return !tabStretchType.equals(TabStretchType.never)
                && (tabStretchType.equals(TabStretchType.always) || tabStretchType
                        .equals(TabStretchType.multiline) && runCount > 1);
    }
    ...
    protected Insets getTabAreaInsets(final int tabPlacement) {
        ...
        rotateInsets(
                tabbedPaneStyle.equals(TabbedPaneStyle.standalone) ? new Insets(
                        tabPlacement == RIGHT ? 1 : 0, 1, 0, 2) : new Insets(
                        -1, -1, 0, 0), targetInsets, tabPlacement);
        return targetInsets;
    }
    ...
    protected Insets getSelectedTabPadInsets(final int tabPlacement) {
        ...
        rotateInsets(
                tabbedPaneStyle.equals(TabbedPaneStyle.standalone) ? new Insets(
                        2, 2, 2, 1) : new Insets(0, 0, 0, 0), targetInsets,
                tabPlacement);
        return targetInsets;
    }
    ...
    protected void paintTabBackground(final Graphics g, final int tabPlacement,
        ...
        // Border shape
        final GeneralPath borderShape = createTabShape(TabShapeType.border,
                tabPlacement, x, y, w, h, isSelected);
        
        // Tab shade
        if (tabbedPaneStyle.equals(TabbedPaneStyle.standalone)) {
            final GeneralPath shadeShape = createTabShape(TabShapeType.shade,
                    tabPlacement, x, y, w, h, isSelected);
            GraphicsUtils.drawShade(g2d, shadeShape,
                    FlatLafStyleConstants.shadeColor, shadeWidth,
                    new Rectangle2D.Double(0, 0, tabPane.getWidth(), y + h),
                    round > 0);
        }
        
        // Tab background
        final GeneralPath bgShape = createTabShape(TabShapeType.background,
                tabPlacement, x, y, w, h, isSelected);
        if (backgroundPainterAt.containsKey(tabIndex) && isSelected) {
            ...
            final Point bottomPoint = getBottomTabBgPoint(tabPlacement, x, y,
                    w, h);
            if (isSelected) {
                Color bg = tabPane.getBackgroundAt(tabIndex);
                bg = bg != null ? bg : tabPane.getBackground();
                g2d.setPaint(new GradientPaint(topPoint.x, topPoint.y,
                        selectedTopBg, bottomPoint.x, bottomPoint.y, bg));
            } else {
                g2d.setPaint(new GradientPaint(topPoint.x, topPoint.y, topBg,
                        bottomPoint.x, bottomPoint.y, bottomBg));
            }
        ...
        if (tabbedPaneStyle.equals(TabbedPaneStyle.standalone)) {
            LafUtils.drawCustomWebFocus(g2d, null,
                    FlatLafStyleConstants.focusType, borderShape, null,
                    drawFocus);
        }
        // else if ( drawFocus )
        // {
            ...
            if (tabPane.isEnabled() && tabPane.isEnabledAt(tabIndex)) {
                ...
                if (isSelected && (fg instanceof UIResource)) {
                    if (selectedForegroundAt.containsKey(tabIndex)) {
                        ...
                        final Color selectedFG = UIManager
                ...
                g.setColor(fg);
                SwingUtils.drawStringUnderlineCharAt(g, title, mnemIndex,
    ...
    private GeneralPath createTabShape(final TabShapeType tabShapeType,
            final int tabPlacement, int x, final int y, int w, final int h,
            final boolean isSelected) {
        ...
        final int actualRound = tabbedPaneStyle
                .equals(TabbedPaneStyle.standalone) ? round : 0;
    ...
    private Point getTopTabBgPoint(final int tabPlacement, final int x,
    ...
    private Point getBottomTabBgPoint(final int tabPlacement, final int x,
            final int y, final int w, final int h) {
        if (tabPlacement == JTabbedPane.TOP) {
    ...
    protected void paintContentBorder(final Graphics g, final int tabPlacement,
        ...
        // Selected tab bounds
        final Rectangle selected = selectedIndex != -1 ? getTabBounds(tabPane,
                selectedIndex) : null;
        
        // Background shape
        final Shape bs = createBackgroundShape(tabPlacement, tabAreaSize, bi,
                selected);
        
        if (tabbedPaneStyle.equals(TabbedPaneStyle.standalone)) {
            // Proper clip
            final GeneralPath clip = new GeneralPath(GeneralPath.WIND_EVEN_ODD);
            clip.append(new Rectangle2D.Double(0, 0, tabPane.getWidth(),
                    tabPane.getHeight()), false);
            clip.append(bs, false);
            GraphicsUtils.drawShade(g2d, bs, FlatLafStyleConstants.shadeColor,
                    shadeWidth, clip, round > 0);
            ...
            // g2d.fillRect ( bi.left + tai.left, bi.top + tabAreaSize, round,
            // round );
            //
            // g2d.setPaint ( StyleConstants.darkBorderColor );
            // g2d.drawLine ( bi.left + tai.left, bi.top + tabAreaSize, bi.left
            // + tai.left,
            // bi.top + tabAreaSize + round );
            // }
            // // if ( selected.x + selected.width < tabPane.getWidth () -
            // bi.right )
            // // {
            // // g2d.setPaint ( bottomBg );
            // // g2d.fillRect ( bi.rightbi.left + tai.left, bi.top +
            // tabAreaSize, round, round );
            // //
            // // g2d.setPaint ( StyleConstants.darkBorderColor );
            // // g2d.drawLine ( bi.left + tai.left, bi.top + tabAreaSize,
            // bi.left + tai.left,
            // // bi.top + tabAreaSize + round );
            // // }
            // }
            ...
            if (backgroundPainterAt.containsKey(selectedIndex)) {
                ...
                backgroundPainterAt.get(selectedIndex).paint(g2d,
                        bs.getBounds(), tabPane);
                GraphicsUtils.restoreClip(g2d, old);
            } else {
                final Color bg = selectedIndex != -1 ? tabPane
                        .getBackgroundAt(selectedIndex) : null;
            ...
            // Area focus
            LafUtils.drawCustomWebFocus(g2d, null,
                    FlatLafStyleConstants.focusType, bs, null,
            ...
            if (tabPlacement == JTabbedPane.TOP) {
                if (selected != null) {
                    if (bi.left < selected.x) {
                        g2d.drawLine(bi.left, bi.top + tabAreaSize, selected.x,
                                bi.top + tabAreaSize);
                    }
                    if (selected.x + selected.width < tabPane.getWidth()
                            - bi.right) {
                        g2d.drawLine(selected.x + selected.width, bi.top
                                + tabAreaSize, tabPane.getWidth() - bi.right,
                                bi.top + tabAreaSize);
                    }
                } else {
                    g2d.drawLine(bi.left, bi.top + tabAreaSize,
                            tabPane.getWidth() - bi.right, bi.top + tabAreaSize);
                }
            }
            // else if ( tabPlacement == JTabbedPane.BOTTOM )
            // {
    ...
    public int getTabAreaLength(final int tabPlacement) {
        return tabPlacement == JTabbedPane.TOP
                || tabPlacement == JTabbedPane.BOTTOM ? calculateTabAreaHeight(
                tabPlacement, runCount, maxTabHeight) - 1
                : calculateTabAreaWidth(tabPlacement, runCount, maxTabWidth) - 1;
    }
    
    private Shape createBackgroundShape(final int tabPlacement,
            final int tabAreaSize, final Insets bi, final Rectangle selected) {
        if (tabbedPaneStyle.equals(TabbedPaneStyle.standalone)) {
            if (selected != null) {
                ...
                if (tabPlacement == JTabbedPane.TOP) {
                    ...
                    if (round > 0) {
                        gp.lineTo(bi.left, tabPane.getHeight() - bi.bottom
                                - round);
                        gp.quadTo(bi.left, tabPane.getHeight() - bi.bottom,
                                bi.left + round, tabPane.getHeight()
                                        - bi.bottom);
                        gp.lineTo(tabPane.getWidth() - bi.right - round,
                                tabPane.getHeight() - bi.bottom);
                        gp.quadTo(tabPane.getWidth() - bi.right,
                                tabPane.getHeight() - bi.bottom,
                                tabPane.getWidth() - bi.right,
                                tabPane.getHeight() - bi.bottom - round);
                    } else {
                        gp.lineTo(bi.left, tabPane.getHeight() - bi.bottom);
                        gp.lineTo(tabPane.getWidth() - bi.right,
                                tabPane.getHeight() - bi.bottom);
                    }
                    if (selected.x + selected.width < tabPane.getWidth()
                            - bi.right - round
                        ...
                        gp.quadTo(tabPane.getWidth() - bi.right, topY,
                                tabPane.getWidth() - bi.right - round, topY);
                    } else {
                        gp.lineTo(tabPane.getWidth() - bi.right, topY);
                    }
                    gp.lineTo(selected.x + selected.width, topY);
                } else if (tabPlacement == JTabbedPane.BOTTOM) {
                    final int bottomY = tabPane.getHeight() - bi.bottom
                            - tabAreaSize;
                    ...
                    if (round > 0) {
                        ...
                        gp.quadTo(tabPane.getWidth() - bi.right, bi.top,
                                tabPane.getWidth() - bi.right, bi.top + round);
                    } else {
                    ...
                    if (selected.x + selected.width < tabPane.getWidth()
                        ...
                        gp.lineTo(tabPane.getWidth() - bi.right, bottomY
                                - round);
                        gp.quadTo(tabPane.getWidth() - bi.right, bottomY,
                                tabPane.getWidth() - bi.right - round, bottomY);
                    } else {
                        gp.lineTo(tabPane.getWidth() - bi.right, bottomY);
                    }
                    ...
                    if (round > 0) {
                        ...
                        gp.quadTo(tabPane.getWidth() - bi.right,
                                tabPane.getHeight() - bi.bottom,
                                tabPane.getWidth() - bi.right - round,
                                tabPane.getHeight() - bi.bottom);
                    } else {
                        gp.lineTo(tabPane.getWidth() - bi.right, bi.top);
                        gp.lineTo(tabPane.getWidth() - bi.right,
                    ...
                    if (selected.y + selected.height < tabPane.getHeight()
                            - bi.bottom - round
                            && round > 0) {
                        gp.lineTo(leftX + round, tabPane.getHeight()
                                - bi.bottom);
                        gp.quadTo(leftX, tabPane.getHeight() - bi.bottom,
                                leftX, tabPane.getHeight() - bi.bottom - round);
                    } else {
                        gp.lineTo(leftX, tabPane.getHeight() - bi.bottom);
                    }
                    gp.lineTo(leftX, selected.y + selected.height);
                } else {
                    final int rightX = tabPane.getWidth() - bi.right
                            - tabAreaSize;
                    ...
                    if (selected.y + selected.height < tabPane.getHeight()
                            - bi.bottom - round
                            && round > 0) {
                        gp.lineTo(rightX - round, tabPane.getHeight()
                                - bi.bottom);
                        gp.quadTo(rightX, tabPane.getHeight() - bi.bottom,
                                rightX, tabPane.getHeight() - bi.bottom - round);
                    } else {
                ...
                return new RoundRectangle2D.Double(bi.left
                        + (left ? tabAreaSize : 0), bi.top
                        + (top ? tabAreaSize : 0), tabPane.getWidth() - bi.left
                        - bi.right - (left || right ? tabAreaSize : 0),
                        tabPane.getHeight() - bi.top - bi.bottom
                                - (top || bottom ? tabAreaSize : 0), round * 2,
                        round * 2);
            }
        } else {
            final int x = bi.left
                    + (tabPlacement == JTabbedPane.LEFT ? tabAreaSize : 0);
            final int y = bi.top
                    + (tabPlacement == JTabbedPane.TOP ? tabAreaSize : 0);
            final int width = tabPane.getWidth()
                    - bi.left
                    - bi.right
                    - (tabPlacement == JTabbedPane.LEFT
                            || tabPlacement == JTabbedPane.RIGHT ? tabAreaSize
                            : 0);
            final int height = tabPane.getHeight()
                    - bi.top
                    - bi.bottom
                    - (tabPlacement == JTabbedPane.TOP
                            || tabPlacement == JTabbedPane.BOTTOM ? tabAreaSize
                            : 0);
    ...
    public Shape getContentClip() {
        ...
        if (tabPlacement == JTabbedPane.TOP) {
            clip = new RoundRectangle2D.Double(insets.left, insets.top
                    + tabAreaLength, tabPane.getWidth() - insets.left
                    - insets.right, tabPane.getHeight() - insets.top
                    - tabAreaLength - insets.bottom, round * 2, round * 2);
        } else if (tabPlacement == JTabbedPane.BOTTOM) {
            clip = new RoundRectangle2D.Double(insets.left, insets.top,
                    tabPane.getWidth() - insets.left - insets.right,
                    tabPane.getHeight() - insets.top - tabAreaLength
                            - insets.bottom, round * 2, round * 2);
        } else if (tabPlacement == JTabbedPane.LEFT) {
            clip = new RoundRectangle2D.Double(insets.left + tabAreaLength,
                    insets.top, tabPane.getWidth() - insets.left
                            - tabAreaLength - insets.right, tabPane.getHeight()
                            - insets.top - insets.bottom, round * 2, round * 2);
        } else if (tabPlacement == JTabbedPane.RIGHT) {
            clip = new RoundRectangle2D.Double(insets.left, insets.top,
                    tabPane.getWidth() - insets.left - tabAreaLength
                            - insets.right, tabPane.getHeight() - insets.top
                            - insets.bottom, round * 2, round * 2);
        }
    ...
    // }
    //
    // protected void normalizeTabRuns ( int tabPlacement, int tabCount, int
    // start, int max )
    // {
