...
public class NeoTabbedPaneUI extends BasicTabbedPaneUI {
    ...
    protected void paintTabArea(final Graphics g, final int tabPlacement,
        ...
        for (int i = runCount - 1; i >= 0; i--) {
            ...
            for (int j = start; j <= end; j++) {
                if (j != selectedIndex && rects[j].intersects(clipRect)) {
                    paintTab(g2d, tabPlacement, rects, runCount - 1 - i, col,
                            colsInRow, j, iconRect, textRect);
                }
                if (j == selectedIndex) {
                    sr = runCount - 1 - i;
                    sc = col;
        ...
        if (selectedIndex >= 0 && rects[selectedIndex].intersects(clipRect)) {
            paintTab(g2d, tabPlacement, rects, sr, sc, scr, selectedIndex,
                    iconRect, textRect);
        }
        ...
        // g2d.drawLine ( x+w, y + h - 1, tabPane.getWidth
        // ()-insets.right-tabAreaInsets.right, y + h - 1 );
        // g2d.setPaint ( new Color ( 200, 200, 200 ) );
        // g2d.drawLine ( x+w, y + h - 2, tabPane.getWidth
        // ()-insets.right-tabAreaInsets.right, y + h - 2 );
        // }
    }
    
    protected void paintTab(final Graphics2D g2d, final int tabPlacement,
            final Rectangle[] rects, final int row, final int col,
            final int colsInRow, final int tabIndex, final Rectangle iconRect,
            final Rectangle textRect) {
        final Rectangle tabRect = rects[tabIndex];
        final int selectedIndex = tabPane.getSelectedIndex();
        final boolean isSelected = selectedIndex == tabIndex;
        
        paintTabBackground(g2d, tabPlacement, row, col, colsInRow, tabIndex,
                tabRect.x, tabRect.y, tabRect.width, tabRect.height, isSelected);
        
        final String title = tabPane.getTitleAt(tabIndex);
        final Font font = tabPane.getFont();
        final FontMetrics metrics = SwingUtils.getFontMetrics(tabPane, g2d,
                font);
        ...
        if (customTabComponent) {
            ...
            paintText(g2d, tabPlacement, font, metrics, tabIndex, title,
                    textRect, isSelected);
        }
        // paintFocusIndicator ( g2d, tabPlacement, rects, tabIndex, iconRect,
        // textRect, isSelected, customTabComponent );
    }
    ...
    // protected void paintFocusIndicator ( Graphics2D g2d, int tabPlacement,
    ...
    protected void paintTabBackground(final Graphics2D g2d,
        ...
        // Background
        if (isSelected) {
            g2d.setPaint(new GradientPaint(x, y, Color.WHITE, x, y + h,
                    new Color(232, 232, 232)));
            g2d.fillRect(x + 1, y, w - 2, h);
        } else {
            g2d.setPaint(new GradientPaint(x, y, new Color(226, 226, 226), x, y
                    + h, new Color(205, 205, 205)));
        ...
        if (isSelected) {
            ...
            g2d.drawLine(x + 1, y + (topBorder ? 1 : 0), x + w
                    - (trailingBorder ? 2 : 1), y + (topBorder ? 1 : 0));
            g2d.drawLine(x + w - (trailingBorder ? 2 : 1), y + 1, x + w
                    - (trailingBorder ? 2 : 1), y + h - 1);
        } else {
            // Top shade
            if (row == 0) {
                g2d.setPaint(new Color(191, 191, 191));
    ...
    protected void paintContentBorder(final Graphics g, final int tabPlacement,
        ...
        default: {
            ...
            g2d.setPaint(new GradientPaint(x, y + 1, new Color(232, 232, 232),
                    x, y + 3, new Color(230, 230, 230)));
