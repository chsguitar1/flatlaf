...
public class WebFileChooserUI extends FileChooserUI {
    ...
    public void installUI(final JComponent c) {
        ...
        fileChooserPanel = new WebFileChooserPanel(getFileChooserType(),
        ...
        fileChooserPanel.setApproveListener(new ActionListener() {
            ...
            public void actionPerformed(final ActionEvent e) {
                ...
                final List<File> selectedFiles = fileChooserPanel
                        .getSelectedFiles();
        ...
        // {
        // ignoreFileSelectionChanges = true;
        // fileChooser.setSelectedFiles ( selectedFiles.toArray ( new File[
        // selectedFiles.size () ] ) );
        // ignoreFileSelectionChanges = false;
        // }
        // } );
    ...
    protected void propertyChanged(final PropertyChangeEvent event) {
        ...
        if (prop.equals(JFileChooser.APPROVE_BUTTON_TEXT_CHANGED_PROPERTY)) {
            fileChooserPanel.setApproveButtonText(fileChooser
                    .getApproveButtonText());
        } else if (prop
                .equals(JFileChooser.CONTROL_BUTTONS_ARE_SHOWN_CHANGED_PROPERTY)) {
            fileChooserPanel.setShowControlButtons(fileChooser
            ...
            final int initialCapacity = (filters != null ? filters.length : 0)
                    + (filter != null ? 1 : 0);
            final List<FileFilter> collected = new ArrayList<FileFilter>(
                    initialCapacity);
            if (filter != null) {
            ...
            // Applying filters
            if (collected.size() > 0) {
                fileChooserPanel.setFileFilters(collected
                        .toArray(new FileFilter[collected.size()]));
            } else {
                fileChooserPanel
            ...
            if (!ignoreFileSelectionChanges) {
                fileChooserPanel.setCurrentFolder(fileChooser
                        .getCurrentDirectory());
            }
            ...
            if (!ignoreFileSelectionChanges) {
                ...
                if (selectedFiles.length > 0) {
                    ...
                    // fileChooserPanel.setCurrentFolder ( selectedFiles[ 0
                    // ].getParentFile () );
                    // fileChooserPanel.setSelectedFiles ( selectedFiles );
                    fileChooserPanel.setSelectedFiles(selectedFiles);
                    
                } else {
                    // Simply pass the file, it will be selected when directory
                    // is opened
                    // fileChooserPanel.setCurrentFolder (
                    // fileChooser.getSelectedFile () );
                    fileChooserPanel.setSelectedFile(fileChooser
                            .getSelectedFile());
                }
            }
            ...
            fileChooserPanel.setShowHiddenFiles(!fileChooser
                    .isFileHidingEnabled());
        } else if (prop
    ...
    public String getDialogTitle(final JFileChooser fc) {
        ...
        return dialogTitle != null ? dialogTitle : LanguageManager
                .get("weblaf.filechooser.title");
    }
    ...
    public void ensureFileIsVisible(final JFileChooser fc, final File f) {
        // // This is pretty annoying and pointless method, will ignore it for
        // now
        // ignoreFileSelectionChanges = true;
