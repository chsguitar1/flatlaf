...
public class WebRadioButtonUI extends BasicRadioButtonUI implements
        ShapeProvider {
    ...
    static {
        ...
        for (int i = 1; i <= 3; i++) {
            CHECK_STATES.add(new ImageIcon(WebRadioButtonUI.class
                    .getResource("icons/c" + i + ".png")));
        }
        
        DISABLED_CHECK = ImageUtils.getDisabledCopy(
                "WebRadioButton.disabled.check",
                CHECK_STATES.get(CHECK_STATES.size() - 1));
    }
    ...
    public void installUI(final JComponent c) {
        ...
        bgTimer = new WebTimer("WebRadioButtonUI.bgUpdater", 40,
                new ActionListener() {
                    @Override
        ...
        checkTimer = new WebTimer("WebRadioButtonUI.iconUpdater", 40,
                new ActionListener() {
                    @Override
                    public void actionPerformed(final ActionEvent e) {
        ...
        itemListener = new ItemListener() {
            ...
            public void itemStateChanged(final ItemEvent e) {
                if (animated) {
                    ...
                    checkTimer.stop();
                    checkIcon = radioButton.isSelected() ? CHECK_STATES.size() - 1
                            : 0;
                    c.repaint();
                }
            }
    ...
    public Shape provideShape() {
        return LafUtils.getWebBorderShape(radioButton, getShadeWidth(),
                getRound());
    }
    ...
    private void updateBorder() {
        ...
        // Actual margin
        final boolean ltr = radioButton.getComponentOrientation()
                .isLeftToRight();
    ...
    public boolean isAnimated() {
        return animated
                && (radioButton == null || radioButton.getParent() == null || !(radioButton
                        .getParent() instanceof WebListElement || radioButton
                        .getParent() instanceof WebTreeElement));
    }
    ...
    private void updateIcon(final JRadioButton radioButton) {
        radioButton.setIcon(new Icon() {
            ...
            public void paintIcon(final Component c, final Graphics g,
                ...
                final Rectangle iconRect = new Rectangle(x + shadeWidth, y
                        + shadeWidth, iconWidth - shadeWidth * 2 - 1,
                        iconHeight - shadeWidth * 2 - 1);
                final RoundRectangle2D shape = new RoundRectangle2D.Double(
                        iconRect.x, iconRect.y, iconRect.width,
                        iconRect.height, round, round);
                
                // Shade
                if (c.isEnabled()) {
                    GraphicsUtils.drawShade(
                            g2d,
                            shape,
                            c.isEnabled() && c.isFocusOwner() ? FlatLafStyleConstants.fieldFocusColor
                ...
                final int radius = Math.round((float) Math.sqrt(iconRect.width
                        * iconRect.width / 2));
                g2d.setPaint(new RadialGradientPaint(iconRect.x
                        + iconRect.width / 2, iconRect.y + iconRect.height / 2,
                ...
                g2d.setPaint(c.isEnabled() ? (rolloverDarkBorderOnly ? ColorUtils
                        .getIntermediateColor(borderColor, darkBorderColor,
                                getProgress()) : darkBorderColor)
                        : disabledBorderColor);
                g2d.draw(shape);
                ...
                // Check icon
                if (checkIcon > 0) {
                    final ImageIcon icon = radioButton.isEnabled() ? CHECK_STATES
                            .get(checkIcon) : DISABLED_CHECK;
            ...
            if (progress < 1f) {
                return new Color[] {
                        ColorUtils.getIntermediateColor(topBgColor,
                                topSelectedBgColor, progress),
                        ColorUtils.getIntermediateColor(bottomBgColor,
                                bottomSelectedBgColor, progress) };
            } else {
