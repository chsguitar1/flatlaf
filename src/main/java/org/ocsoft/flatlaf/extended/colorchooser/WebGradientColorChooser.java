...
public class WebGradientColorChooser extends JComponent implements
        MouseListener, MouseMotionListener, FocusListener, SettingsMethods,
        SizeMethods<WebGradientColorChooser> {
    // Style constants
    private static final Color borderColor = Color.DARK_GRAY; // new Color ( 51,
                                                              // 51, 51 );
    ...
    private static final float[] overlayFractions = new float[] { 0f, 0.4f,
            0.41f, 1f };
    private static final Color[] lineOverlayColors = new Color[] {
            ColorUtils.white(160), ColorUtils.white(120),
            ColorUtils.white(100), ColorUtils.white(40) };
    private static final Color[] controlOverlayColors = new Color[] {
            ColorUtils.white(80), ColorUtils.white(50), ColorUtils.white(20),
    ...
    public void mouseClicked(final MouseEvent e) {
        ...
        if (SwingUtilities.isLeftMouseButton(e) && e.getClickCount() == 2) {
            final GradientColorData colorData = getColorDataUnderPoint(e
                    .getPoint());
            if (colorData != null) {
                final Color newColor = WebColorChooser.showDialog(
                        WebGradientColorChooser.this, colorData.getColor());
                if (newColor != null) {
                    // Updating color
    ...
    public void mousePressed(final MouseEvent e) {
        ...
        if (SwingUtilities.isLeftMouseButton(e)) {
            ...
            if (colorData != null) {
                if (SwingUtils.isAlt(e)) {
                    ...
                    draggedGripper.setLocation(findFreeLocation(draggedGripper
                            .getLocation()));
            ...
            if (draggedGripper == null && e.getY() > 0) {
                ...
                if (isFree(newLocation)) {
                    ...
                    draggedGripper = new GradientColorData(newLocation,
                            getColorForLocation(newLocation));
                    gradientData.addGradientColorData(draggedGripper);
                    draggedOut = false;
        ...
        } else if ((SwingUtils.isMiddleMouseButton(e) || SwingUtils
                .isRightMouseButton(e)) && gradientData.size() > 2) {
            final GradientColorData colorData = getColorDataUnderPoint(e
                    .getPoint());
            if (colorData != null) {
                // Removing color
    ...
    public void mouseDragged(final MouseEvent e) {
        ...
        if (draggedGripper != null) {
            if (e.getY() < -10
                    && gradientData.size() + (draggedOut ? 1 : 0) > 2) {
                if (!draggedOut) {
    ...
    public Rectangle getGripperBounds(final GradientColorData colorData) {
        ...
        final int x = lineBounds.x + 2
                + Math.round((lineBounds.width - 4) * location)
                - gripperSize.width / 2;
    ...
    public void setMargin(final int top, final int left, final int bottom,
            final int right) {
    ...
    private void updateBorder() {
        setBorder(BorderFactory.createEmptyBorder(margin.top, margin.left,
    ...
    protected void paintComponent(final Graphics g) {
        ...
        paintBorderedBackground(g2d, lineBounds,
                getLineBackgroundPainter(lineBounds),
                getLineOverlayPainter(lineBounds), false);
    ...
    private void paintControls(final Graphics2D g2d, final Rectangle lineBounds) {
        ...
        for (int i = 0; i < gradientData.size(); i++) {
            ...
            final int currentX = x + Math.round(w * currentLocation)
                    - gripperSize.width / 2;
            final Rectangle controlBounds = new Rectangle(currentX, y,
                    gripperSize.width, gripperSize.height);
            
            // Painting control
            paintBorderedBackground(g2d, controlBounds,
                    gradientColorData.getColor(),
                    getControlOverlayPainter(controlBounds), true);
            
            // Painting text and separators between controls if there is enough
            // space
            if (paintLabels && i < gradientData.size() - 1) {
                final GradientColorData nextGradientColorData = gradientData
                        .get(i + 1);
                final int gapX1 = currentX + gripperSize.width;
                final int gapX2 = x
                        + Math.round(w * nextGradientColorData.getLocation())
                        - gripperSize.width / 2;
                final int gapInPercents = Math.round((nextGradientColorData
                        .getLocation() - currentLocation) * 100);
                ...
                if (gapX2 - gapX1 > textWidth + 4) {
                    ...
                    if (gapX2 - gapX1 > textWidth + 14) {
                        g2d.setPaint(new GradientPaint(gapX1 + 2, 0,
                                FlatLafStyleConstants.transparent, textX - 2,
                                0, Color.LIGHT_GRAY));
                        g2d.drawLine(gapX1 + 2, lineY, textX - 2, lineY);
                        g2d.setPaint(new GradientPaint(textX + textWidth + 2,
                                0, Color.LIGHT_GRAY, gapX2 - 2, 0,
                                FlatLafStyleConstants.transparent));
                        g2d.drawLine(textX + textWidth + 2, lineY, gapX2 - 2,
                                lineY);
                    }
                }
            }
    ...
    private void paintBorderedBackground(final Graphics2D g2d,
            final Rectangle bounds, final Paint background,
            final Paint overlay, final boolean control) {
        // Shade & focus
        if (isEnabled() && !control) {
            GraphicsUtils.drawShade(g2d, new RoundRectangle2D.Double(bounds.x,
                    bounds.y, bounds.width - 1, bounds.height - 1, 4, 4),
                    isFocusOwner() ? FlatLafStyleConstants.fieldFocusColor
                            : FlatLafStyleConstants.borderColor, shadeWidth);
        }
        
        // Background
        g2d.setPaint(background);
        g2d.fillRoundRect(bounds.x + 1, bounds.y + 1, bounds.width - 2,
                bounds.height - 2, 2, 2);
        
        // Inner border
        g2d.setPaint(innerBorderColor);
        g2d.drawRoundRect(bounds.x + 1, bounds.y + 1, bounds.width - 3,
                bounds.height - 3, 2, 2);
        
        // Additional control overlay
        if (control) {
            g2d.setPaint(new GradientPaint(bounds.x + 2, bounds.y + 2,
                    ColorUtils.white(180), bounds.x + bounds.width * 2 / 3,
                    bounds.y + bounds.height - 2,
                    FlatLafStyleConstants.transparent));
            g2d.fillRect(bounds.x + 2, bounds.y + 2, bounds.width - 4,
                    bounds.height - 4);
        }
    ...
    private LinearGradientPaint getLineBackgroundPainter(
            final Rectangle lineBounds) {
        return new LinearGradientPaint(lineBounds.x + 1, 0, lineBounds.x
                + lineBounds.width - 1, 0, gradientData.getFractions(),
    ...
    private LinearGradientPaint getOverlayPainter(final Rectangle bounds,
            final float[] overlayFractions, final Color[] overlayColors) {
        return new LinearGradientPaint(0, bounds.y + 2, 0, bounds.y
                + bounds.height - 2, overlayFractions, overlayColors);
    }
    
    private Rectangle getLineBounds() {
        final Insets insets = getInsets();
        return new Rectangle(insets.left
                + Math.max(gripperSize.width / 2, shadeWidth) - 2, insets.top
                + shadeWidth, getWidth() - insets.left - insets.right
                - Math.max(gripperSize.width, shadeWidth * 2) + 4, lineWidth);
    }
    ...
    public Dimension getPreferredSize() {
        ...
        final int width = i.left + gripperSize.width * 5
                + Math.max(gripperSize.width, shadeWidth * 2) - 4 + i.right;
        final int height = i.top + shadeWidth * 2 + lineWidth
                + gripperSize.height / 2
                + (gripperSize.height % 2 == 0 ? 0 : 1) + i.bottom;
     ...
     */
    @Override
    public WebGradientColorChooser setPreferredSize(final int width,
            final int height) {
