...
public class TristateCheckIcon extends SimpleCheckIcon {
    ...
    protected static final Color[] colors = { new Color(28, 66, 97),
            new Color(55, 84, 108) };
    protected static final Color[] disabledColors = { new Color(112, 112, 112),
    ...
    public void paintIcon(final Component c, final Graphics2D g2d, final int x,
        ...
        if (cb != null) {
            ...
            if (mixedStep > -1) {
                ...
                final Rectangle b = new Rectangle(x + s, y + s, w - s * 2, h
                        - s * 2);
                final RoundRectangle2D shape = new RoundRectangle2D.Double(b.x
                        + r, b.y + r, b.width - r * 2, b.height - r * 2, 3, 3);
                final Point center = GeometryUtils.middle(b);
                final int radius = Math.max(b.width / 2, b.height / 2);
                g2d.setPaint(new RadialGradientPaint(center, radius, fractions,
                        cb.isEnabled() ? colors : disabledColors));
                g2d.fill(shape);
            }
            
            // Check icon
            if (checkStep > -1) {
                final ImageIcon icon = enabled ? CHECK_STATES.get(checkStep)
                        : DISABLED_CHECK_STATES.get(checkStep);
                g2d.drawImage(icon.getImage(), x + w / 2 - getIconWidth() / 2,
                        y + h / 2 - getIconHeight() / 2, null);
            }
        }
    }
}