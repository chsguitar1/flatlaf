...
public class WebDecoratedImage extends JComponent implements SwingConstants,
    ...
    public void updatePreview() {
        ...
        if (zoomBlur && rotationBlur) {
            ImageFilterUtils.applyMotionBlurFilter(image, image, 0f, 0f,
                    rotationBlurFactor, zoomBlurFactor, blurAlignX, blurAlignY);
        } else if (zoomBlur) {
            ImageFilterUtils.applyZoomBlurFilter(image, image, zoomBlurFactor,
                    blurAlignX, blurAlignY);
        } else if (rotationBlur) {
            ImageFilterUtils.applyRotationBlurFilter(image, image,
                    rotationBlurFactor, blurAlignX, blurAlignY);
        }
        
        // Applying rounded corners
        if (round > 0) {
            image = ImageUtils.cutImage(
                    new RoundRectangle2D.Double(0, 0, icon.getIconWidth(), icon
                            .getIconHeight(), round * 2, round * 2), image);
        }
        
        // Creating additional effects
        if (shadeWidth > 0 || drawGlassLayer || drawBorder) {
            Dimension ps = getPreferredSize();
            BufferedImage img = ImageUtils.createCompatibleImage(ps.width,
                    ps.height, Transparency.TRANSLUCENT);
            Graphics2D g2d = img.createGraphics();
            GraphicsUtils.setupAntialias(g2d);
            Shape bs = getBorderShape();
            
            // Shade
            GraphicsUtils.drawShade(g2d, bs, WebDecoratedImageStyle.shadeType,
                    new Color(90, 90, 90), shadeWidth);
            ...
            // Glass-styled shade
            if (drawGlassLayer) {
                g2d.setPaint(new GradientPaint(0, shadeWidth, new Color(255,
                        255, 255, 160), 0, shadeWidth
                        + (ps.height - shadeWidth * 2) / 2, new Color(255, 255,
                        255, 32)));
    ...
    private Shape getGlanceShape() {
        Dimension ps = getPreferredSize();
        if (round > 0) {
            return new RoundRectangle2D.Double(shadeWidth, shadeWidth, ps.width
                    - shadeWidth * 2, (ps.height - shadeWidth * 2) / 2,
                    round * 2, round * 2);
        } else {
            return new Rectangle(shadeWidth, shadeWidth, ps.width - shadeWidth
                    * 2, (ps.height - shadeWidth * 2) / 2);
        }
    }
    ...
    private Shape getBorderShape(int x, int y) {
        Dimension ps = getPreferredSize();
        if (round > 0) {
            return new RoundRectangle2D.Double(x + shadeWidth, y + shadeWidth,
                    ps.width - shadeWidth * 2 - 1, ps.height - shadeWidth * 2
                            - 1, round * 2, round * 2);
        } else {
            return new Rectangle(x + shadeWidth, y + shadeWidth, ps.width
                    - shadeWidth * 2 - 1, ps.height - shadeWidth * 2 - 1);
        }
    }
    ...
    public Dimension getPreferredSize() {
        if (icon != null) {
            return new Dimension(shadeWidth * 2 + icon.getIconWidth(),
                    shadeWidth * 2 + icon.getIconHeight());
        } else {
