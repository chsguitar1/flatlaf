...
/**
 * This component allows you to display images in many different ways. This
 * component uses less resources than a label and has a few optimization.
 *
 * @author Mikle Garin
 */

public class WebImage extends JComponent implements EventMethods,
        ToolTipMethods, SwingConstants {
    /**
     * Image source.
     */
    ...
    /**
     * Image horizontal alignment. Doesn't affect anything in case fitComponent
     * display type is used.
     */
    private int horizontalAlignment;
    
    /**
     * Image vertical alignment. Doesn't affect anything in case fitComponent
     * display type is used.
     */
    ...
    /**
     * Last cached image size. This is used to determine when image component
     * was resized since last paint call.
     */
    private Dimension lastDimention = null;
    
    /**
     * Last cached image preview. This variable is used when actual painted
     * image is smaller than source image. In that case source image is getting
     * scaled and saved into this variable.
     */
    ...
    /**
     * Constructs component with an image loaded from package near specified
     * class.
     *
     * @param nearClass
     *            class near which image is located
     * @param src
     *            image file location
     */
    ...
    /**
     * Constructs component with an image retrieved from the specified image
     * icon.
     *
     * @param icon
     *            image icon to process
     */
    ...
    public WebImage(final BufferedImage image) {
        ...
        addPropertyChangeListener(FlatLafConstants.ENABLED_PROPERTY,
    ...
    protected void calculateDisabledImage() {
        disabledImage = image != null ? ImageUtils.createDisabledCopy(image)
                : null;
    ...
    /**
     * Returns last image preview. This image might be the modified version of
     * original image set into this component.
     *
    ...
    /**
     * Returns icon representing last image preview. This image icon might
     * contain modified version of original image set into this component.
     *
     * @return icon representing last image preview
     */
    public ImageIcon getPreviewIcon() {
        return lastPreviewImage != null ? new ImageIcon(lastPreviewImage)
                : null;
    }
     ...
     *            new image horizontal alignment
     * @return this image component
     */
     ...
     * @return this image component
     */
    public WebImage setMargin(final int top, final int left, final int bottom,
            final int right) {
    ...
    protected void paintComponent(final Graphics g) {
        ...
        // todo Optimize for repaint (check if image is out of repainted/clipped
        // bounds)
            ...
            if (getSize().equals(getRequiredSize())) {
                ...
                case preferred: {
                    ...
                    final int x = horizontalAlignment == LEFT ? insets.left
                            : horizontalAlignment == RIGHT ? getWidth()
                                    - currentImage.getWidth() - insets.right
                                    : getCenterX(insets)
                                            - currentImage.getWidth() / 2;
                    final int y = verticalAlignment == TOP ? insets.top
                            : verticalAlignment == BOTTOM ? getHeight()
                ...
                case fitComponent: {
                    ...
                    g2d.drawImage(preview,
                            getCenterX(insets) - preview.getWidth() / 2,
                ...
                case repeat: {
                    ...
                    g2d.setPaint(new TexturePaint(currentImage,
                            new Rectangle2D.Double(x, y, currentImage
                                    .getWidth(), currentImage.getHeight())));
                    g2d.fillRect(insets.left, insets.top, getWidth()
    ...
    protected BufferedImage getPreviewImage(final Insets insets) {
        if (image.getWidth() > getWidth() || image.getHeight() > getHeight()) {
            ...
            size.setSize(size.width - insets.left - insets.right, size.height
                    - insets.top - insets.bottom);
            if (lastPreviewImage == null || lastDimention != null
                    && !lastDimention.equals(size)) {
                if (lastPreviewImage != null) {
                    lastPreviewImage.flush();
                    lastPreviewImage = null;
                }
                lastPreviewImage = ImageUtils.createPreviewImage(
                        getCurrentImage(), size);
    ...
    protected Dimension getRequiredSize() {
        ...
        return new Dimension(insets.left
                + (image != null ? image.getWidth() : 0) + insets.right,
                insets.top + (image != null ? image.getHeight() : 0)
                        + insets.bottom);
    }
