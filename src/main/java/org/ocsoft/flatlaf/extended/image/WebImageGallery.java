...
public class WebImageGallery extends JComponent {
            ...
            public void keyPressed(final KeyEvent e) {
                if (images.size() > 0) {
                    ...
                    if (Hotkey.LEFT.isTriggered(e)) {
                        setSelectedIndex(si == -1 || si == 0 ? images.size() - 1
                                : si - 1);
                    } else if (Hotkey.RIGHT.isTriggered(e)) {
                        setSelectedIndex(si == -1 || si == images.size() - 1 ? 0
                                : si + 1);
        ...
        if (view == null) {
            view = new WebScrollPane(WebImageGallery.this, withBorder) {
                ...
                public Dimension getPreferredSize() {
                    final int columns = Math.min(images.size(),
                            preferredColumnCount);
                    final JScrollBar hsb = getHorizontalScrollBar();
                    final int sbh = hsb != null && hsb.isShowing() ? hsb
                            .getPreferredSize().height : 0;
                    return new Dimension(spacing * (columns + 1) + columns
                            * maxWidth,
    ...
    public void setSelectedIndex(final int selectedIndex) {
        ...
        if (scrollOnSelection) {
            ...
            SwingUtils.scrollSmoothly(getView(), rect.x + rect.width / 2
                    - WebImageGallery.this.getVisibleRect().width / 2, rect.y);
        }
        moveReflection();
    }
    ...
    private void moveReflection() {
        ...
        progress = 0f;
        reflectionMover = new WebTimer("WebImageGallery.reflectionMoveTimer",
                FlatLafStyleConstants.fastAnimationDelay, new ActionListener() {
                    @Override
    ...
    public Rectangle getImageRect(final int index) {
        ...
        final int x = (getWidth() > ps.width ? (getWidth() - ps.width) / 2 : 0)
                + spacing + (maxWidth + spacing) * index + maxWidth / 2;
        final int y = getHeight() / 2 - spacing / 2 - iconHeight / 2;
        return new Rectangle(x - iconWidth / 2, y - iconHeight / 2, iconWidth,
                iconHeight);
    }
    ...
    public void addImage(final int index, final ImageIcon image) {
        try {
            final ImageIcon previewIcon = ImageUtils.createPreviewIcon(image,
                    imageLength);
            final int rwidth = previewIcon.getIconWidth();
            final int rheight = previewIcon.getIconHeight();
            
            final BufferedImage reflection = ImageUtils.createCompatibleImage(
                    rwidth, rheight, Transparency.TRANSLUCENT);
            final Graphics2D g2d = reflection.createGraphics();
            GraphicsUtils.setupAntialias(g2d);
            g2d.drawImage(previewIcon.getImage(), 0, 0, null);
            g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.DST_IN));
            g2d.setPaint(new GradientPaint(0, rheight * (1f - fadeHeight),
                    new Color(0, 0, 0, 0), 0, rheight, new Color(0, 0, 0,
                            opacity)));
            ...
            descriptions.add(index,
                    image.getIconWidth() + " x " + image.getIconHeight()
                            + " px");
    ...
    protected void paintComponent(final Graphics g) {
        ...
        for (int i = 0; i < images.size(); i++) {
            ...
            final int x = (getWidth() > ps.width ? (getWidth() - ps.width) / 2
                    : 0) + spacing + (maxWidth + spacing) * i + maxWidth / 2;
            final int y = height / 2 - spacing / 2 - imageHeight / 2;
            final int y2 = height / 2 + spacing / 2 + imageHeight / 2;
            
            // Initial image
            
            final float add = selectedIndex == i ? progress * 0.4f
                    : (oldSelectedIndex == i ? 0.4f - progress * 0.4f : 0);
            g2d.setComposite(AlphaComposite.getInstance(
                    AlphaComposite.SRC_OVER, 0.6f + add));
            
            g2d.drawImage(bi, x - imageWidth / 2, y - imageHeight / 2, null);
            
            g2d.setPaint(selectedIndex == i ? Color.WHITE : Color.GRAY);
            Area gp = new Area(new RoundRectangle2D.Double(x - imageWidth / 2
                    - borderWidth, y - imageHeight / 2 - borderWidth,
                    imageWidth + borderWidth * 2,
                    imageHeight + borderWidth * 2, borderWidth * 2,
                    borderWidth * 2));
            gp.subtract(new Area(new Rectangle(x - imageWidth / 2, y
                    - imageHeight / 2, imageWidth, imageHeight)));
            g2d.fill(gp);
            ...
            if (selectedIndex == i || oldSelectedIndex == i) {
                final float opacity = selectedIndex == i ? progress
                        : 1f - progress;
                g2d.setComposite(AlphaComposite.getInstance(
                ...
                final Point ts = LafUtils.getTextCenterShear(
                        g2d.getFontMetrics(), infoText);
                g2d.drawString(infoText, x + ts.x, getHeight() / 2 + spacing
                        / 2 + ts.y);
                g2d.setComposite(oldComposite);
            }
            ...
            final int addition = selectedIndex == i ? Math.round(progress
                    * spacing) : (oldSelectedIndex == i ? spacing
                    - Math.round(progress * spacing) : 0);
            if (reflections.get(i) != null) {
                g2d.drawImage(reflections.get(i), x - imageWidth / 2, y2
                        + imageHeight / 2 + addition, imageWidth, -imageHeight,
                        null);
            }
            
            gp = new Area(new RoundRectangle2D.Double(x - rwidth / 2, y2
                    - rheight / 2 + addition, rwidth, rheight, borderWidth * 2,
                    borderWidth * 2));
            gp.subtract(new Area(new Rectangle(x - rwidth / 2 + borderWidth, y2
                    - rheight / 2 + addition + borderWidth, rwidth
                    - borderWidth * 2, rheight - borderWidth * 2)));
            g2d.setPaint(new GradientPaint(0, y2 - imageHeight / 2 + addition,
                    selectedIndex == i ? selectedLight : light, 0, y2
                            - imageHeight / 2 + addition + imageHeight
                            * fadeHeight,
    ...
    @Override
    public Dimension getPreferredSize() {
        return new Dimension(spacing * (images.size() + 1) + maxWidth
                * images.size(), spacing * 3 + maxHeight * 2);
    }
}
