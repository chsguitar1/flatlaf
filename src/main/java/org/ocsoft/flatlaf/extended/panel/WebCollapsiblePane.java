 ...
 * collapsible pane. Pane title, content and style can be modified in any way
 * you like.
 *
 * @author Mikle Garin
 */
...
public class WebCollapsiblePane extends WebPanel implements SwingConstants,
        ShapeProvider, LanguageMethods, SettingsMethods {
    /**
     ...
     */
    protected List<CollapsiblePaneListener> listeners = new ArrayList<CollapsiblePaneListener>(
            1);
    ...
    /**
     * Handler that dynamically enable and disable collapsible pane state
     * changes by providing according boolean value.
     */
    ...
    /**
     * Current collapsible pane transition progress. When pane is fully expanded
     * this variable becomes 1f. When pane is fully collapsed this variable
     * becomes 0f.
     */
     ...
     *            collapsible pane content
     */
    public WebCollapsiblePane(final Icon icon, final String title,
        ...
        headerPanel.addMouseListener(new MouseAdapter() {
            ...
            private boolean isAllowAction(final MouseEvent e) {
                return SwingUtilities.isLeftMouseButton(e)
                        && SwingUtils.size(WebCollapsiblePane.this).contains(
                                e.getPoint());
            }
        });
            ...
            public Dimension getPreferredSize() {
                ...
                if (titlePanePostion == TOP || titlePanePostion == BOTTOM) {
                    if (WebCollapsiblePane.this.content != null) {
                        ...
                        ps.width = insets.left
                                + WebCollapsiblePane.this.content
     ...
     *            collapsible pane title text
     */
    protected void updateDefaultTitleComponent(final Icon icon,
            final String title) {
        if (!customTitle) {
    ...
    protected void updateDefaultTitleBorder() {
        if (titleComponent != null && !customTitle) {
            ...
            Insets margin = getIcon() != null || titlePanePostion != LEFT
                    || titlePanePostion == RIGHT ? new Insets(2, 2, 2, 2)
                    : new Insets(2, 4, 2, 2);
            if (titlePanePostion == LEFT) {
                margin = new Insets(margin.right, margin.top, margin.left,
                        margin.bottom);
            } else if (titlePanePostion == RIGHT) {
                margin = new Insets(margin.left, margin.bottom, margin.right,
                        margin.top);
            }
    ...
    protected void updateHeaderSides() {
        headerPanel.setPaintSides(expanded && titlePanePostion == BOTTOM,
                expanded && titlePanePostion == RIGHT, expanded
                        && titlePanePostion == TOP, expanded
                        && titlePanePostion == LEFT);
    }
        ...
        if (showStateIcon) {
            if (titlePanePostion == TOP || titlePanePostion == BOTTOM) {
                headerPanel.add(expandButton,
                        stateIconPostion == RIGHT ? BorderLayout.LINE_END
                                : BorderLayout.LINE_START);
            } else if (titlePanePostion == LEFT) {
                headerPanel.add(expandButton,
                        stateIconPostion == RIGHT ? BorderLayout.PAGE_START
                                : BorderLayout.PAGE_END);
            } else if (titlePanePostion == RIGHT) {
                headerPanel.add(expandButton,
                        stateIconPostion == RIGHT ? BorderLayout.PAGE_END
     ...
     */
    protected JComponent createDefaultTitleComponent(final Icon icon,
            final String title) {
        // todo RTL orientation support
        final WebLabel defaultTitle;
        if (titlePanePostion == LEFT) {
            defaultTitle = new WebVerticalLabel(title, icon, WebLabel.LEADING,
                    false);
        } else if (titlePanePostion == RIGHT) {
            defaultTitle = new WebVerticalLabel(title, icon, WebLabel.LEADING,
                    true);
    ...
    /**
     * Returns handler that dynamically enable and disable collapsible pane
     * state changes by providing according boolean value.
     *
    ...
    /**
     * Sets handler that dynamically enable and disable collapsible pane state
     * changes by providing according boolean value.
     *
    ...
    public void setStateChangeHandler(
            final DataProvider<Boolean> stateChangeHandler) {
    ...
    /**
     * Returns whether collapsible pane is performing animated transition at the
     * moment or not.
     *
     * @return true if collapsible pane is performing animated transition at the
    ...
    /**
     * Changes expanded state to opposite and returns whether operation succeed
     * or not. Operation might fail in case state change is disabled for some
     * reason.
     *
    ...
    /**
     * Changes expanded state to specified one and returns whether operation
     * succeed or not. Operation might fail in case state change is disabled for
     * some reason.
     *
     * @return true if operation succeed, false otherwise
     */
    ...
    /**
     * Changes expanded state to collapsed and returns whether operation succeed
     * or not. Operation might fail in case state change is disabled for some
     * reason.
     *
     * @return true if operation succeed, false otherwise
    ...
    public boolean collapse(final boolean animate) {
        ...
        if (animate && isShowing()) {
            animator = new WebTimer("WebCollapsiblePane.collapseTimer",
                    FlatLafStyleConstants.fastAnimationDelay,
                    new ActionListener() {
                        @Override
                        public void actionPerformed(final ActionEvent e) {
                            if (transitionProgress > 0f) {
                                transitionProgress = Math.max(0f,
                                        transitionProgress - expandSpeed);
                                revalidate();
                            } else {
    ...
    /**
     * Changes expanded state to expanded and returns whether operation succeed
     * or not. Operation might fail in case state change is disabled for some
     * reason.
     *
     * @return true if operation succeed, false otherwise
    ...
    public boolean expand(final boolean animate) {
        ...
        if (animate && isShowing()) {
            animator = new WebTimer("WebCollapsiblePane.expandTimer",
                    FlatLafStyleConstants.fastAnimationDelay,
                    new ActionListener() {
                        @Override
                        public void actionPerformed(final ActionEvent e) {
     ...
     *            new title pane position in collapsible pane
     */
     ...
     */
    public void setContentMargin(final int top, final int left,
     ...
     *
     * @param icon
     *            new default title component icon
     */
     ...
     *
     * @param title
     *            new default title component text
     */
     ...
     *
     * @param alignment
     *            new default title component text alignment
     */
    ...
    /**
     * Returns whether rotate state icon according to title pane position or
     * not.
     *
     * @return true if stat icon must be rotated according to title pane
     ...
     *            whether rotate state icon according to title pane position or
     *            not
     */
     ...
     *
     * @return true if state icon must be displayed in title pane, false
     *         otherwise
     */
     ...
     *            new state icon position in title pane
     */
    ...
    protected ImageIcon getCachedCollapseIcon() {
        if (cachedCollapseIcon == null) {
            ...
            // boolean ltr = getComponentOrientation ().isLeftToRight ();
            if (!rotateStateIcon || titlePanePostion == TOP
                    || titlePanePostion == BOTTOM) {
    ...
    protected ImageIcon getCachedDisabledCollapseIcon() {
        if (cachedDisabledCollapseIcon == null) {
            cachedDisabledCollapseIcon = ImageUtils
                    .createDisabledCopy(getCachedCollapseIcon());
        }
    ...
    protected ImageIcon getCachedExpandIcon() {
        if (cachedExpandIcon == null) {
            ...
            } else if (ltr ? titlePanePostion == LEFT
                    : titlePanePostion == RIGHT) {
                cachedExpandIcon = ImageUtils.rotateImage90CCW(expandIcon);
            } else if (ltr ? titlePanePostion == RIGHT
                    : titlePanePostion == LEFT) {
    ...
    protected ImageIcon getCachedDisabledExpandIcon() {
        if (cachedDisabledExpandIcon == null) {
            cachedDisabledExpandIcon = ImageUtils
                    .createDisabledCopy(getCachedExpandIcon());
        }
     ...
     *
     * @param listeners
     *            new collapsible pane listeners
     */
    public void setCollapsiblePaneListeners(
            final List<CollapsiblePaneListener> listeners) {
     ...
     *
     * @param listener
     *            collapsible pane listener to add
     */
    public void addCollapsiblePaneListener(
            final CollapsiblePaneListener listener) {
     ...
     */
    public void removeCollapsiblePaneListener(
            final CollapsiblePaneListener listener) {
    ...
    public void fireExpanding() {
        for (final CollapsiblePaneListener listener : CollectionUtils
                .copy(listeners)) {
    ...
    /**
     * Returns current collapsible pane transition progress. Returns 1f when
     * pane is fully expanded and 0f when pane is fully collapsed.
     *
    ...
    /**
     * Returns preferred size without taking collapsible pane content into
     * account.
     *
     * @return preferred size without taking collapsible pane content into
     *         account
     */
        ...
        if (content == null || transitionProgress <= 0) {
            ...
            if (titlePanePostion == TOP || titlePanePostion == BOTTOM) {
                return new Dimension(ps.width, ps.height
                        - Math.round(cps.height * transitionProgress));
            } else {
                return new Dimension(ps.width
                        - Math.round(cps.width * transitionProgress), ps.height);
            }
        }
    }
