...
public class WebFileList extends WebList {
    ...
    /**
     * Returns specific for WebFileList renderer. Be aware that this method
     * might throw ClassCastException if renderer is altered by user.
     *
    ...
    public WebFileListCellRenderer getWebFileListCellRenderer()
        ...
        return cellRenderer instanceof WebFileListCellRenderer ? (WebFileListCellRenderer) cellRenderer
                : null;
    }
    
    /**
     * Returns specific for WebFileList model. Be aware that this method might
     * throw ClassCastException if model is altered by user.
     *
     ...
     *
     * @param preferredColumnCount
     *            new preferred visible column count
     */
     ...
     *
     * @param preferredRowCount
     *            new preferred visible row count
     */
     ...
     *
     * @return true if file list renderer should generate image file thumbnails,
     *         false otherwise
     */
    ...
    /**
     * Returns displayed directory. Returned File might be null in case custom
     * files list was set or no data is loaded into list yet.
     *
    ...
    /**
     * Sets displayed directory. This will force list to load and display files
     * from the specified directory.
     *
     * @param file
     *            new displayed directory
     */
    public void setDisplayedDirectory(final File file) {
        ...
        final File[] files = file != null ? FileUtils.sortFiles(file
                .listFiles(fileFilter)) : FileUtils.getDiskRoots();
    ...
    public File getSelectedFile() {
        ...
        return selectedValue != null ? ((FileElement) selectedValue).getFile()
                : null;
    }
    ...
    public void setSelectedFiles(final Collection<File> files) {
        final List<FileElement> elements = new ArrayList<FileElement>(
                files.size());
    ...
    /**
     * Returns scroll pane with fixed preferred size that fits file list
     * settings.
     *
     * @return scroll pane with fixed preferred size that fits file list
     *         settings
     */
    ...
    /**
     * Returns new scroll pane with fixed preferred size that fits file list
     * settings.
     *
     * @return new scroll pane with fixed preferred size that fits file list
     *         settings
     */
    public WebScrollPane createScrollView() {
        return new WebScrollPane(WebFileList.this) {
            ...
            public Dimension getPreferredSize() {
                ...
                final int sbw = vsb != null && vsb.isShowing() ? vsb
                        .getPreferredSize().width : 0;
                ps.width = oneCell.width * preferredColumnCount + bi.left
                        + bi.right + sbw + 1;
                ps.height = oneCell.height * preferredRowCount + bi.top
                        + bi.bottom + 1;
                return ps;
            }
        };
