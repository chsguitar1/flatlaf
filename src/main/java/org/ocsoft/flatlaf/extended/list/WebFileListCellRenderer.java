 ...
 * able to generate image thumbnails for image file elements.
 *
 * @author Mikle Garin
 */
...
public class WebFileListCellRenderer extends WebListCellRenderer {
     ...
     *
     * @param fileList
     *            file list in which this cell renderer is used
     */
    public WebFileListCellRenderer(final WebFileList fileList) {
        ...
        fileList.addPropertyChangeListener(
                ...
                new PropertyChangeListener() {
                    @Override
                    public void propertyChange(final PropertyChangeEvent evt) {
                        final ComponentOrientation orientation = fileList
    ...
    public Component getListCellRendererComponent(final JList list,
        ...
        if (iconLabel.isEnabled()) {
            ...
            synchronized (thumbnailsLock) {
                if (!element.isThumbnailQueued()
                        && !element.isDisabledThumbnailQueued()) {
        ...
        if (fileList.getEditedCell() != index) {
            ...
            final FileDescription fileDescription = FileUtils
                    .getFileDescription(file, imageSize);
     ...
     *            element to add
     */
    protected void queueThumbnailLoad(final FileElement element,
        ...
        executorService.submit(new Runnable() {
            ...
            public void run() {
                ...
                final String ext = FileUtils.getFileExtPart(
                        element.getFile().getName(), false).toLowerCase();
                if (fileList.isGenerateThumbnails()
                        && FlatLafConstants.IMAGE_FORMATS.contains(ext)) {
                    final ImageIcon thumb = element.getEnabledThumbnail() != null ? element
                            .getEnabledThumbnail() : ImageUtils
                            .createThumbnailIcon(absolutePath, thumbSize);
                    if (thumb != null) {
                        element.setEnabledThumbnail(thumb);
                        if (disabled) {
                            element.setDisabledThumbnail(ImageUtils
                                    .createDisabledCopy(thumb));
                        }
                    } else {
                        element.setEnabledThumbnail(FileUtils
                                .getStandartFileIcon(element.getFile(), true,
                                        true));
                        if (disabled) {
                            element.setDisabledThumbnail(FileUtils
                                    .getStandartFileIcon(element.getFile(),
                                            true, false));
                        }
                    }
                } else {
                    element.setEnabledThumbnail(FileUtils.getStandartFileIcon(
                            element.getFile(), true, true));
                    if (disabled) {
                        element.setDisabledThumbnail(FileUtils
                                .getStandartFileIcon(element.getFile(), true,
                                        false));
    ...
    protected class FileCellLayout extends AbstractLayoutManager {
        ...
        public void layoutContainer(final Container parent) {
            ...
            final boolean ltr = fileList.getComponentOrientation()
                    .isLeftToRight();
            final Insets i = getInsets();
            final boolean tilesView = isTilesView();
            final boolean hasName = nameLabel.getText() != null;
            final boolean hasDescription = tilesView
                    && descriptionLabel.getText() != null;
            final boolean hasFileSize = tilesView
                    && sizeLabel.getText() != null;
            ...
            if (tilesView) {
                // Left-middle icon
                iconLabel.setBounds(ltr ? i.left : tileCellSize.width - i.right
                        - imageSide, i.top, imageSide, imageSide);
                
                // Description elements
                if (hasName) {
                    // Constants for futher description positioning calculations
                    final Dimension nps = nameLabel.getPreferredSize();
                    final Dimension dps = hasDescription ? descriptionLabel
                            .getPreferredSize() : new Dimension(0, 0);
                    final Dimension sps = hasFileSize ? sizeLabel
                            .getPreferredSize() : new Dimension(0, 0);
                    final int dh = nps.height
                            + (hasDescription ? gap + dps.height : 0)
                            + (hasFileSize ? gap + sps.height : 0);
                    final int dx = ltr ? i.left + imageSide + gap : i.left;
                    final int dw = tileCellSize.width - i.left - i.right
                            - imageSide - gap;
                    
                    // Name element
                    int dy = i.top + (tileCellSize.height - i.top - i.bottom)
                            / 2 - dh / 2;
                ...
                iconLabel.setBounds(i.left + cw / 2 - 27, i.top, imageSide,
                        imageSide);
                
                // Name element
                if (hasName) {
                    final int ny = i.top + imageSide + gap;
                    nameLabel.setBounds(i.left, ny, cw, iconCellSize.height
                            - ny - i.bottom);
                }
            }
        }
        ...
        public Rectangle getDescriptionBounds() {
            ...
            if (tilesView) {
                ...
                if (ltr) {
                    ...
                    return new Rectangle(x, i.top, tileCellSize.width - x
                            - i.right, tileCellSize.height - i.top - i.bottom);
                } else {
                    // Icon at the right side, description at the left side
                    return new Rectangle(i.left, i.top, tileCellSize.width
                            - gap - imageSide - i.right, tileCellSize.height
                            - i.top - i.bottom);
                }
            } else {
                // Icon view
                final int ny = i.top + imageSide + gap;
                return new Rectangle(i.left, ny, iconCellSize.width - i.left
                        - i.right, iconCellSize.height - ny - i.bottom);
            }
        }
    }
}