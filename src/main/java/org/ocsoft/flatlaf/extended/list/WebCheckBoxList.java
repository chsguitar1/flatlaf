 ...
 * separate checkbox, but it could be selected like the elements in simple list.
 * You can also perform mass check/uncheck operations by selection more than one
 * checkbox and pressing space.
 *
 * @author Mikle Garin
 */
...
public class WebCheckBoxList extends WebList {
    ...
    public WebCheckBoxListCellRenderer getWebCheckBoxListCellRenderer() {
        ...
        return renderer instanceof WebCheckBoxListCellRenderer ? (WebCheckBoxListCellRenderer) renderer
                : null;
    }
    ...
    /**
     * Returns whether checkbox under the specified cell index is selected or
     * not.
     *
     * @param index
     *            cell index
     * @return true if checkbox at the specified cell index is selected, false
     *         otherwise
     */
    ...
    /**
     * Returns whether checkbox selection should be performed only when user
     * clicks exactly on the check icon or not.
     *
     * @return true if checkbox selection should be performed only when user
     *         clicks exactly on the check icon, false otherwise
     */
    ...
    /**
     * Sets whether checkbox selection should be performed only when user clicks
     * exactly on the check icon or not.
     *
     * @param checkOnIconOnly
     *            whether checkbox selection should be performed only when user
     *            clicks exactly on the check icon or not
     */
    ...
    /**
     * Custom checkbox list mouse adapter that handles checkbox selection
     * changes.
     */
    protected class CheckBoxListMouseAdapter extends MouseAdapter {
        @Override
        public void mousePressed(final MouseEvent e) {
            final int index = getUI().locationToIndex(WebCheckBoxList.this,
                    e.getPoint());
            if (index != -1 && WebCheckBoxList.this.isEnabled()) {
                if (checkOnIconOnly) {
                    final WebCheckBoxListCellRenderer renderer = getWebCheckBoxListCellRenderer();
                    final WebCheckBoxListElement element = renderer
                            .getElement(getCheckBoxListModel().get(index));
                    final Rectangle cellRect = getWebUI().getCellBounds(
                            WebCheckBoxList.this, index, index);
    ...
    protected class CheckBoxListKeyAdapter extends KeyAdapter {
        ...
        public void keyReleased(final KeyEvent e) {
            if (Hotkey.SPACE.isTriggered(e) && getSelectedIndex() != -1
                    && isEnabled()) {
