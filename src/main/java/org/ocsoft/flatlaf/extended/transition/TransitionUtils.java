...
import org.ocsoft.flatlaf.extended.transition.effects.blocks.BlockType;
import org.ocsoft.flatlaf.extended.transition.effects.curtain.CurtainSlideDirection;
import org.ocsoft.flatlaf.extended.transition.effects.curtain.CurtainType;
import org.ocsoft.flatlaf.extended.transition.effects.slide.SlideType;
import org.ocsoft.flatlaf.extended.transition.effects.zoom.ZoomType;
import org.ocsoft.flatlaf.utils.MathUtils;

/**
 * This class provides a set of utilities for transition components. This is a
 * library utility class and its not intended for use outside of transition
 * components.
 *
 * @author Mikle Garin
 */
...
public final class TransitionUtils {
    ...
    public static Direction getActualValue(final Direction direction) {
        if (direction.equals(Direction.random)) {
            ...
            return new Direction[] { Direction.left, Direction.right }[MathUtils
                    .random(2)];
        } else if (direction.equals(Direction.vertical)) {
            return new Direction[] { Direction.up, Direction.down }[MathUtils
                    .random(2)];
        } else {
            return direction;
        }
     ...
     */
    public static CurtainSlideDirection getActualValue(
            final CurtainSlideDirection slideDirection) {
        if (slideDirection.equals(CurtainSlideDirection.random)) {
            final CurtainSlideDirection[] slideDirections = CurtainSlideDirection
                    .values();
