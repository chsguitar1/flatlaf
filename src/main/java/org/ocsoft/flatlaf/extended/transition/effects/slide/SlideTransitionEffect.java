...
public class SlideTransitionEffect extends DefaultTransitionEffect {
    ...
    public boolean performAnimation(final ImageTransition imageTransition) {
        ...
        final int maxProgress = direction.equals(Direction.left)
                || direction.equals(Direction.right) ? imageTransition
                .getWidth() : imageTransition.getHeight();
        
        // Incrementing grow states
        final int resultSpeed = Math.max(minimumSpeed, Math.round(speed
                * (float) Math.sqrt((float) Math.abs(type
                        .equals(SlideType.moveOld) ? slideProgress
                        : maxProgress - slideProgress)
                        / maxProgress)));
    ...
    public void paint(final Graphics2D g2d, final ImageTransition transition) {
        ...
        final float floatProgress = (float) slideProgress
                / (direction.isHorizontal() ? width : height);
        ...
        if (type.equals(SlideType.moveNew)) {
            ...
            g2d.drawImage(transition.getCurrentImage(), 0, 0, width, height,
                    null);
            
            // New image sliding in
            final Composite old = GraphicsUtils.setupAlphaComposite(g2d,
                    floatProgress, fade);
            g2d.drawImage(transition.getOtherImage(), np.x, np.y, width,
                    height, null);
            ...
            final Composite old = GraphicsUtils.setupAlphaComposite(g2d,
                    1f - floatProgress, fade);
            g2d.drawImage(transition.getCurrentImage(), cp.x, cp.y, width,
                    height, null);
            GraphicsUtils.restoreComposite(g2d, old, fade);
