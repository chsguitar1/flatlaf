...
/**
 * Custom label component that quickly renders multi-styled text. Its rendering
 * speed is superior to HTML rendering within simple JLabel and its usage is
 * preferred.
 *
 * @author Mikle Garin
 */

public class WebStyledLabel extends JLabel implements EventMethods,
        ToolTipMethods, Styleable, LanguageMethods, FontMethods<WebStyledLabel> {
    /**
     ...
     *            language data, may not be passed
     */
    public WebStyledLabel(final String text, final Insets margin,
            final Object... data) {
        super(LanguageUtils.getInitialText(text, data));
        LanguageUtils.registerInitialLanguage(this, text, data);
        setMargin(margin);
     ...
     *            language data, may not be passed
     */
    public WebStyledLabel(final String text, final int horizontalAlignment,
            final Object... data) {
     ...
     */
    public WebStyledLabel(final String text, final int horizontalAlignment,
            final Insets margin, final Object... data) {
        super(LanguageUtils.getInitialText(text, data), horizontalAlignment);
        LanguageUtils.registerInitialLanguage(this, text, data);
        setMargin(margin);
    }
     ...
     */
    public WebStyledLabel(final String text, final Icon icon,
            final int horizontalAlignment, final Object... data) {
        super(LanguageUtils.getInitialText(text, data), icon,
                horizontalAlignment);
        LanguageUtils.registerInitialLanguage(this, text, data);
    }
     ...
     */
    public WebStyledLabel(final String text, final Icon icon,
            final int horizontalAlignment, final Insets margin,
            final Object... data) {
        super(LanguageUtils.getInitialText(text, data), icon,
                horizontalAlignment);
        LanguageUtils.registerInitialLanguage(this, text, data);
     ...
     * @return this label
     */
    public WebStyledLabel setMargin(final int top, final int left,
    ...
    protected StyleRange addStyleRangeImpl(final StyleRange styleRange) {
        final StyleRange removed = clearSimilarRangeImpl(
                styleRange.getStartIndex(), styleRange.getLength());
    ...
    protected void removeStyleRangeImpl(final StyleRange styleRange) {
        if (!getStyleRangesImpl().remove(styleRange)) {
            final Iterator<StyleRange> iterator = getStyleRangesImpl()
                    .iterator();
            while (iterator.hasNext()) {
                final StyleRange range = iterator.next();
                if (range.getStartIndex() == styleRange.getStartIndex()
                        && range.getLength() == styleRange.getLength()) {
                    iterator.remove();
                    return;
                }
            }
     ...
     *
     * @param wrap
     *            whether text lines should be wrapped or not
     */
     ...
     * <p/>
     * Note that it has lower priority than preferred width. If preferred width
    ...
    /**
     * Sets maximum rows amount visible after wrapping. By default it is set to
     * zero.
     *
     * @param maximumRows
    ...
    /**
     * Sets minimum rows amount visible after wrapping. By default it is set to
     * zero.
     *
     * @param minimumRows
    ...
    public void updateUI() {
        if (getUI() == null || !(getUI() instanceof WebStyledLabelUI)) {
            try {
                setUI((WebStyledLabelUI) ReflectUtils
                        .createInstance(FlatLafSettings.styledLabelUI));
    ...
    public Dimension getMaximumSize() {
        return isLineWrap() ? new Dimension(Integer.MAX_VALUE,
                Integer.MAX_VALUE) : super.getMaximumSize();
    }
     ...
     *
     * @param width
     *            new preferred width of the label
     */
     ...
     */
    @Override
    public WebStyledLabel setFontSizeAndStyle(final int fontSize,
