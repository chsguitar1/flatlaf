...
public final class StyledLabelUtils implements SwingConstants {
    ...
    /**
     * Parses label style ranges into text ranges. All parsed text ranges are
     * stored within provided list.
     *
     * @param label
     *            label to parse text ranges for
     * @param textRanges
     *            list to store text ranges into
     */
    public static void buildTextRanges(final WebStyledLabel label,
        ...
        if (s != null && s.length() > 0) {
            ...
            for (final StyleRange styleRange : styleRanges) {
                ...
                if (styleRange.getStartIndex() > index) {
                    final String text = s.substring(index,
                            Math.min(styleRange.getStartIndex(), s.length()));
                    final StyleRange newRange = new StyleRange(index,
                            styleRange.getStartIndex() - index);
                ...
                if (styleRange.getStartIndex() == index) {
                    ...
                    if (styleRange.getLength() == -1) {
                        ...
                        final String text = s.substring(
                                index,
                                Math.min(index + styleRange.getLength(),
                                        s.length()));
                        addStyledTexts(text, styleRange, textRanges);
            ...
            if (index < s.length()) {
                ...
                final StyleRange range = new StyleRange(index, s.length()
                        - index);
    ...
    private static void addStyledTexts(String text, StyleRange range,
            final List<TextRange> textRanges) {
        ...
        while (index1 >= 0 || index2 >= 0) {
            ...
            textRanges.add(new TextRange(text.substring(index, index + length),
                    newRange));
    ...
    public static int findNextWordStartIndex(final String string,
            final int firstRowEndIndex) {
    ...
    /**
     * Computes compound label icon and text positions relative to viewR
     * rectangle. Component orientation is also taken into account and properly
     * applied.
     *
     * @param label
     *            painted label
     ...
     */
    public static String layoutCompoundLabel(final WebStyledLabel label,
        ...
        // Translate LEADING/TRAILING values in horizontalAlignment to
        // LEFT/RIGHT values depending on the components orientation
        ...
        // Translate LEADING/TRAILING values in horizontalTextPosition to
        // LEFT/RIGHT values depending on the components orientation
        ...
        return layoutCompoundLabelImpl(label, text, icon, verticalAlignment,
     ...
     * @return layout string
     */
    public static String layoutCompoundLabel(final String text,
            final Icon icon, final int verticalAlignment,
            final int horizontalAlignment, final int verticalTextPosition,
            final int horizontalTextPosition, final Rectangle viewR,
            final Rectangle iconR, final Rectangle textR, final int textIconGap) {
        return layoutCompoundLabelImpl(null, text, icon, verticalAlignment,
     ...
     */
    private static String layoutCompoundLabelImpl(final WebStyledLabel label,
            String text, final Icon icon, final int verticalAlignment,
            final int horizontalAlignment, final int verticalTextPosition,
            final int horizontalTextPosition, final Rectangle viewR,
            final Rectangle iconR, final Rectangle textR, final int textIconGap) {
        ...
        // If a null or and empty String was specified we substitute "" here and
        // use 0,0,0,0 for textR
        final boolean textIsEmpty = (text == null) || text.equals("");
        final int lsb = 0;
        
        // Unless both text and icon are non-null, the value of textIconGap is
        // ignored
        final int gap;
        ...
        final int labelR_width = Math.max(iconR.x + iconR.width, textR.x
                + textR.width)
                - labelR_x;
        final int labelR_y = Math.min(iconR.y, textR.y);
        final int labelR_height = Math.max(iconR.y + iconR.height, textR.y
                + textR.height)
                - labelR_y;
    ...
    public static String getPlainText(final String text,
        ...
        if (begin != -1) {
            while (begin != -1) {
                ...
                if (end != -1) {
                    // Parsing statement
                    final TextRange range = parseStatement(trimmedText
    ...
    private static int nextUnescaped(final String trimmedText,
            final String pattern, final int from) {
        // todo
        ...
        if (sep != -1) {
            try {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    } else if (token.equals("sp") || token.equals("sup")
                            || token.equals("superscript")) {
                        customStyles.add(CustomStyle.superscript);
                    } else if (token.equals("sb") || token.equals("sub")
                            || token.equals("subscript")) {
                ...
                final int style = b && i ? Font.BOLD | Font.ITALIC
                        : b ? Font.BOLD : i ? Font.ITALIC : p ? Font.PLAIN : -1;
                final CustomStyle[] cs = customStyles
                        .toArray(new CustomStyle[customStyles.size()]);
                return new TextRange(text, new StyleRange(0, text.length(),
                        style, fg, bg, cs));
