...
public class WebStyledLabelPainter<E extends WebStyledLabel> extends
        WebLabelPainter<E> implements StyledLabelPainter<E>, SwingConstants {
    /**
     * Style settings.
     */
     ...
     */
    @Override
    protected void paintEnabledText(final E label, final Graphics2D g2d,
            final String text, final int textX, final int textY) {
     ...
     */
    @Override
    protected void paintDisabledText(final E label, final Graphics2D g2d,
            final String text, final int textX, final int textY) {
        paintStyledText(label, g2d, textX, textY);
    }
     ...
     *            text Y coordinate
     */
    protected void paintStyledText(final E label, final Graphics2D g,
    ...
    protected int getLabelWidth(final E label) {
        ...
        if (label.isLineWrap()) {
            ...
            try {
                ...
                label.setPreferredWidth(oldPreferredWidth > 0 ? Math.min(
                        label.getWidth(), oldPreferredWidth) : label.getWidth());
    ...
    protected int getTextWidth(final E label) {
        ...
        if (label.getIcon() != null
                && label.getHorizontalTextPosition() != SwingConstants.CENTER) {
            textWidth -= label.getIcon().getIconWidth()
     ...
     */
    protected int paintStyledTextImpl(final E label, final Graphics2D g,
            final int textX, int textY, final int w) {
        ...
        if (UIManager.getLookAndFeel() instanceof FlatLookAndFeel
                && FlatLookAndFeel.isMnemonicHidden()
        ...
        case LEADING:
            horizontalAlignment = label.getComponentOrientation()
                    .isLeftToRight() ? LEFT : RIGHT;
            break;
        case TRAILING:
            horizontalAlignment = label.getComponentOrientation()
                    .isLeftToRight() ? RIGHT : LEFT;
            break;
        }
        
        for (final TextRange textRange : textRanges) {
            final StyleRange style = textRange.styleRange;
            final int size = (style != null && (style.isSuperscript() || style
                    .isSubscript())) ? Math.round((float) defaultFontSize
                    / scriptFontRatio) : defaultFontSize;
            
            font = StyledLabelUtils.getFont(label);
            if (style != null
                    && ((style.getStyle() != -1 && font.getStyle() != style
                            .getStyle()) || font.getSize() != size)) {
                font = getCachedDerivedFont(
                        font,
                        style.getStyle() == -1 ? font.getStyle() : style
                                .getStyle(), size);
        ...
        for (int i = 0; i < textRanges.size(); i++) {
            ...
            if (mnemonicIndex >= 0
                    && textRange.text.length() - nextRowStartIndex > mnemonicIndex
                            - charDisplayed) {
            ...
            if (textRange.text.contains("\r") || textRange.text.contains("\n")) {
                final boolean lastRow = (label.getMaximumRows() > 0 && rowCount >= label
                        .getMaximumRows() - 1)
                        || textY + maxRowHeight
                                + Math.max(0, label.getRowGap()) > labelHeight;
                if (horizontalAlignment != LEFT && g != null) {
                    ...
                    paintRow(label, g, startX, x, endX, textY, rowStartOffset,
                            style.getStartIndex() + textRange.text.length(),
                            lastRow);
                }
                ...
                if (!lastRow) {
                    ...
                    // continue to paint truncated text suffix if lastRow is
                    // true
                    continue;
            ...
            boolean stop = false;
            String s = textRange.text.substring(Math.min(nextRowStartIndex,
            ...
            if (widthLeft < strWidth && widthLeft >= 0) {
                if (label.isLineWrap()
                        && ((label.getMaximumRows() > 0 && rowCount < label
                                .getMaximumRows() - 1) || label
                                .getMaximumRows() <= 0)
                    ...
                    do {
                        final String subString = s.substring(0,
                                Math.max(0, Math.min(availLength, s.length())));
                        int firstRowWordEndIndex = StyledLabelUtils
                                .findFirstRowWordEndIndex(subString);
                        nextWordStartIndex = firstRowWordEndIndex < 0 ? 0
                                : StyledLabelUtils.findNextWordStartIndex(s,
                                        firstRowWordEndIndex);
                        if (firstRowWordEndIndex < 0) {
                            if (x != startX) {
                                final boolean lastRow = label.getMaximumRows() > 0
                                        && rowCount >= label.getMaximumRows() - 1;
                                if (horizontalAlignment != LEFT && g != null) {
                                    paintRow(
                                            label,
                                            g,
                                ...
                                textY += maxRowHeight
                                        + Math.max(0, label.getRowGap());
                                x = startX;
                                i--;
                                rowCount++;
                                rowStartOffset = style.getStartIndex()
                                        + Math.min(nextRowStartIndex,
                                                textRange.text.length());
                                if (lastRow) {
                                    needBreak = true;
                                }
                                ...
                                nextWordStartIndex = Math.min(s.length(),
                                        availLength);
                            }
                        }
                        nextRowStartIndexInSubString = firstRowWordEndIndex + 1;
                        final String subStringThisRow = s.substring(
                                0,
                                Math.min(nextRowStartIndexInSubString,
                                        s.length()));
                        strWidth = fm2.stringWidth(subStringThisRow);
                        if (strWidth > widthLeft) {
                            availLength = subString.length() * widthLeft
                                    / strWidth;
                        }
                        loopCount++;
                        if (loopCount > 15) {
                            FlatLafLogger.error("Styled label paint error: "
                                    + textRange);
                            break;
                        }
                    ...
                    while (nextRowStartIndexInSubString < nextWordStartIndex) {
                        strWidth += fm2.charWidth(s
                                .charAt(nextRowStartIndexInSubString));
                        if (strWidth >= widthLeft) {
                            break;
                        }
                    ...
                    s = s.substring(0,
                            Math.min(nextRowStartIndexInSubString, s.length()));
                    strWidth = fm2.stringWidth(s);
                    charDisplayed -= textRange.text.length()
                            - nextRowStartIndex;
                    if (displayMnemonic) {
                        if (mnemonicIndex >= 0
                                && s.length() > mnemonicIndex - charDisplayed) {
                            displayMnemonic = true;
                            mneIndex = mnemonicIndex - charDisplayed;
                        } else {
                            displayMnemonic = false;
                ...
                final int size = (nextStyle != null && (nextStyle
                        .isSuperscript() || nextStyle.isSubscript())) ? Math
                        .round((float) defaultFontSize / scriptFontRatio)
                        : defaultFontSize;
                
                font = StyledLabelUtils.getFont(label);
                if (nextStyle != null
                        && ((nextStyle.getStyle() != -1 && font.getStyle() != nextStyle
                                .getStyle()) || font.getSize() != size)) {
                    font = getCachedDerivedFont(font,
                            nextStyle.getStyle() == -1 ? font.getStyle()
                                    : nextStyle.getStyle(), size);
                    nextFm2 = label.getFontMetrics(font);
                } else {
                    nextFm2 = fm;
                }
                if (nextFm2.stringWidth(nextText) > widthLeft - strWidth) {
                    nextS = SwingUtilities.layoutCompoundLabel(label, nextFm2,
                            nextText, null, label.getVerticalAlignment(), label
                                    .getHorizontalAlignment(), label
                                    .getVerticalTextPosition(), label
                                    .getHorizontalTextPosition(),
                            new Rectangle(x + strWidth, y,
                                    widthLeft - strWidth, labelHeight),
                            new Rectangle(), new Rectangle(), 0);
                    if (nextFm2.stringWidth(nextS) > widthLeft - strWidth) {
                        s = SwingUtilities.layoutCompoundLabel(label, fm2, s,
                                null, label.getVerticalAlignment(),
                                label.getHorizontalAlignment(),
                                label.getVerticalTextPosition(),
                                label.getHorizontalTextPosition(),
                                new Rectangle(x, y, strWidth - 1, labelHeight),
                                new Rectangle(), new Rectangle(), 0);
                        strWidth = fm2.stringWidth(s);
                        stop = true;
                    }
            ...
            if (horizontalAlignment == LEFT && g != null) {
                ...
                if (style != null && style.getBackground() != null) {
                    ...
                    g.fillRect(x, y - fm2.getHeight(), strWidth,
                            fm2.getHeight() + 4);
                }
                
                Color textColor = (style != null && !ignoreColorSettings && style
                        .getForeground() != null) ? style.getForeground()
                        : label.getForeground();
                if (!label.isEnabled()) {
                ...
                if (style != null) {
                    if (style.isStrikeThrough()) {
                        final int lineY = y
                                + (fm2.getDescent() - fm2.getAscent()) / 2;
                    ...
                    if (style.isWaved()) {
                        ...
                        for (int waveX = x; waveX < x + strWidth; waveX += 4) {
                            ...
                            if (waveX + 4 <= x + strWidth - 1) {
                                g.drawLine(waveX + 3, waveY + 1, waveX + 4,
                                        waveY + 2);
                            }
                        }
                    }
            ...
            if (wrapped) {
                ...
                rowCount++;
                rowStartOffset = style.getStartIndex()
                        + Math.min(nextRowStartIndex, textRange.text.length());
                if (lastRow) {
                    break;
                }
            } else {
    ...
    protected int getStartX(final E label, final int textX, final Insets insets) {
        ...
        if (label.getIcon() != null) {
            ...
            if ((horizontalTextPosition == SwingConstants.TRAILING && label
                    .getComponentOrientation().isLeftToRight())
                    || (horizontalTextPosition == SwingConstants.LEADING && !label
                            .getComponentOrientation().isLeftToRight())) {
                horizontalTextPosition = SwingConstants.RIGHT;
            }
            if (horizontalTextPosition == SwingConstants.RIGHT) {
                leftMostX += label.getIcon().getIconWidth()
                        + label.getIconTextGap();
            }
        }
     ...
     * @param g
     *            graphics context
     * @param leftAlignmentX
     *            left alignment X coordinate
     * @param thisLineEndX
    ...
    protected void paintRow(final E label, final Graphics2D g,
            final int leftAlignmentX, final int thisLineEndX,
            final int rightMostX, final int textY, final int startOffset,
            final int endOffset, final boolean lastRow) {
        if (g == null) {
            return;
        }
        int horizontalTextPosition = label.getHorizontalTextPosition();
        ...
        if ((horizontalTextPosition == SwingConstants.LEADING && !label
                .getComponentOrientation().isLeftToRight())
                || (horizontalTextPosition == SwingConstants.TRAILING && label
                        .getComponentOrientation().isLeftToRight())) {
            horizontalTextPosition = SwingConstants.RIGHT;
        }
        if ((horizontalAlignment == SwingConstants.TRAILING && !label
                .getComponentOrientation().isLeftToRight())
                || (horizontalAlignment == SwingConstants.LEADING && label
                        .getComponentOrientation().isLeftToRight())) {
            horizontalAlignment = SwingConstants.LEFT;
        }
        if ((horizontalAlignment == SwingConstants.LEADING && !label
                .getComponentOrientation().isLeftToRight())
                || (horizontalAlignment == SwingConstants.TRAILING && label
                        .getComponentOrientation().isLeftToRight())) {
        ...
        if (horizontalAlignment == RIGHT) {
            ...
            if (label.getIcon() != null
                    && horizontalTextPosition == SwingConstants.LEFT) {
                textX -= label.getIcon().getIconWidth()
                        + label.getIconTextGap();
            }
            ...
            if (label.getIcon() != null
                ...
                labelWidth -= label.getIcon().getIconWidth()
                        + label.getIconTextGap();
            }
        ...
        int x = textX;
        for (int i = 0; i < textRanges.size()
                && (endOffset < 0 || charDisplayed < endOffset); i++) {
            final TextRange textRange = textRanges.get(i);
            final StyleRange style = textRange.styleRange;
            ...
            final int nextRowStartIndex = style.getStartIndex() >= startOffset ? 0
                    : startOffset - style.getStartIndex();
            ...
            if (s.contains("\r") || s.contains("\n")) {
                if (textRange.styleRange.getStartIndex()
                        + textRange.styleRange.getLength() >= endOffset) {
                    break;
                }
                s = truncatedTextSuffix;
            }
            ...
            if (widthLeft < strWidth) {
                ...
                if (label.isLineWrap() && !lastRow) {
                    ...
                    s = SwingUtilities.layoutCompoundLabel(label, fm2, s, null,
                            label.getVerticalAlignment(), label
                                    ...
                                    .getVerticalTextPosition(), label
                                    .getHorizontalTextPosition(),
                            new Rectangle(x, y, widthLeft, label.getHeight()),
                            new Rectangle(), new Rectangle(), 0);
                    strWidth = fm2.stringWidth(s);
                }
                ...
                if (nextFm2.stringWidth(nextText) > widthLeft - strWidth) {
                    final String nextS = SwingUtilities.layoutCompoundLabel(
                            label, nextFm2, nextText, null, label
                                    .getVerticalAlignment(), label
                                    .getHorizontalAlignment(), label
                                    .getVerticalTextPosition(), label
                                    .getHorizontalTextPosition(),
                            new Rectangle(x + strWidth, y,
                                    widthLeft - strWidth, label.getHeight()),
                            new Rectangle(), new Rectangle(), 0);
                    if (nextFm2.stringWidth(nextS) > widthLeft - strWidth) {
                        s = SwingUtilities.layoutCompoundLabel(
                                label,
                                fm2,
                                s,
                                null,
                                ...
                                label.getVerticalTextPosition(),
                                label.getHorizontalTextPosition(),
                                new Rectangle(x, y, strWidth - 1, label
                                        .getHeight()), new Rectangle(),
                                new Rectangle(), 0);
                        strWidth = fm2.stringWidth(s);
                    }
                }
            ...
            Color textColor = (style != null && !ignoreColorSettings && style
                    .getForeground() != null) ? style.getForeground() : label
                    .getForeground();
            if (!label.isEnabled()) {
                textColor = UIManager.getColor("Label.disabledForeground");
            }
            ...
            if (style != null) {
                if (style.isStrikeThrough()) {
                    final int lineY = y + (fm2.getDescent() - fm2.getAscent())
                            / 2;
                    g.drawLine(x, lineY, x + strWidth - 1, lineY);
                }
                if (style.isDoubleStrikeThrough()) {
    ...
    protected String layoutCL(final E label, final FontMetrics fontMetrics,
            final String text, final Icon icon, final Rectangle viewR,
        ...
        if (label instanceof WebStyledLabel) {
            ...
            try {
                ...
                if (oldPreferredWidth > 0
                        && oldPreferredWidth < label.getWidth()) {
                    label.setPreferredWidth(oldPreferredWidth);
                    size = getPreferredSize(label);
        ...
        if (label.getIcon() != null) {
            textR.width -= label.getIcon().getIconWidth()
                    + label.getIconTextGap();
        }
        ...
        return StyledLabelUtils.layoutCompoundLabel(label, text, icon, va, ha,
                vtp, htp, viewR, iconR, textR, gap);
    }
    ...
    protected Dimension getPreferredSizeImpl(final E label) {
        ...
        final boolean lineWrap = label.isLineWrap()
                || (label.getText() != null && (label.getText().contains("\r") || label
                        .getText().contains("\n")));
        
        final TextRange[] texts = textRanges.toArray(new TextRange[textRanges
                .size()]);
        ...
        for (final TextRange textRange : textRanges) {
            ...
            if (s.startsWith("\r") || s.startsWith("\n")) {
                ...
                width = 0;
                naturalRowCount++;
                if (label.getMaximumRows() > 0
                        && naturalRowCount >= label.getMaximumRows()) {
                    break;
                }
                continue;
            }
        ...
        // if getPreferredWidth() is not set but getRows() is set, get maximum
        // width and row count based on the required rows.
        if (lineWrap && label.getPreferredWidth() <= 0 && label.getRows() > 0) {
            maxWidth = getMaximumWidth(label, maxWidth, naturalRowCount,
                    label.getRows());
        }
        
        // if calculated maximum width is larger than label's maximum size, wrap
        // again to get the updated row count and use the label's maximum width
        ...
        if (label.getIcon() != null
                && label.getHorizontalTextPosition() != SwingConstants.CENTER) {
            preferredWidth -= label.getIcon().getIconWidth()
                    + label.getIconTextGap();
        }
        ...
        if (lineWrap && label.getPreferredWidth() <= 0
                && label.getMinimumRows() > 0
                && preferredRowCount < label.getMinimumRows()) {
            maxWidth = getMaximumWidth(label, maxWidth, naturalRowCount,
                    label.getMinimumRows());
        }
        if (retrievingPreferredSize
                && label.getRows() > 0
                && preferredRowCount > label.getRows()
                && (label.getPreferredWidth() <= 0
                        || label.getPreferredWidth() >= maxLineWidth || naturalRowCount > label
                        .getRows())) {
            preferredRowCount = label.getRows();
            maxLineWidth = 0;
        ...
        Dimension dimension = new Dimension(Math.min(maxWidth, maxLineWidth),
                (maxRowHeight + Math.max(0, label.getRowGap()))
                        * preferredRowCount);
        if (label.getIcon() != null) {
            dimension = new Dimension(dimension.width + label.getIconTextGap()
                    + label.getIcon().getIconWidth(), dimension.height);
        }
        ...
        for (int i = 0; i < textRanges.size(); i++) {
            ...
            if (widthLeft < strWidth) {
                ...
                do {
                    ...
                    nextRowStartIndexInSubString = firstRowWordEndIndex + 1;
                    final String subStringThisRow = s.substring(0,
                            Math.min(nextRowStartIndexInSubString, s.length()));
                    strWidth = fm2.stringWidth(subStringThisRow);
                    if (strWidth > widthLeft) {
     ...
     */
    protected int getMaximumWidth(final E label, int maxWidth,
            final int natural, final int limited) {
        int textWidth = label.getWidth() - label.getInsets().left
                - label.getInsets().right;
        if (label.getIcon() != null) {
            textWidth -= label.getIcon().getIconWidth()
                    + label.getIconTextGap();
        }
        ...
        for (int i = 0; i < textRanges.size(); i++) {
            ...
            if (widthLeft < strWidth) {
                ...
                int firstRowWordEndIndex = StyledLabelUtils
                        .findFirstRowWordEndIndex(subString);
                final int nextWordStartIndex = StyledLabelUtils
                        .findNextWordStartIndex(s, firstRowWordEndIndex);
                if (firstRowWordEndIndex < 0) {
                    if (nextWordStartIndex < s.length()) {
                        firstRowWordEndIndex = StyledLabelUtils
                                .findFirstRowWordEndIndex(s.substring(0,
                                        nextWordStartIndex));
                    } else {
    ...
    /**
     * Get derived font by font, style and size. At first it will get the
     * derived font from cache. If it cannot hit the derived font, it will
     * invoke font.deriveFont to derive a font.
     *
     * @param font
     *            original font
     * @param style
     ...
     */
    public static Font getCachedDerivedFont(final Font font, final int style,
            final int size) {
        final DerivedFontAttributes attribute = getFontAttribute(font, style,
                size);
     ...
     */
    protected static DerivedFontAttributes getFontAttribute(final Font font,
