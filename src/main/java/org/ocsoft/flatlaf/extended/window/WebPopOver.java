...
import org.ocsoft.flatlaf.managers.style.skin.web.WebPopupPainter;
 ...
 * component. It may also act as a simple dialog with custom styling if
 * configured so.
 *
 * @author Mikle Garin
 * @see WebDialog
...
public class WebPopOver extends WebDialog implements Styleable,
        PopOverEventMethods {
    /**
     * todo 1. Fix directional WebPopOver location when dragged to/from
     * secondary screen todo 2. Add "opened" listener event
     */
     ...
     */
    protected List<PopOverListener> popOverListeners = new ArrayList<PopOverListener>(
            1);
     ...
     *            dialog owner frame
     * @param title
     *            dialog title (might not be displayed depending on styling)
     */
    ...
    protected void initialize() {
        ...
        // return WebPopOver.this.isFocused () ? actualShadeOpacity :
        // actualShadeOpacity * 0.7f;
        // }
        // };
        ...
        final ComponentMoveAdapter moveAdapter = new ComponentMoveAdapter() {
            ...
            protected Rectangle getDragStartBounds(final MouseEvent e) {
                if (movable) {
                    ...
                    return new Rectangle(sw, sw, container.getWidth() - sw * 2,
                            container.getHeight() - sw * 2);
                } else {
                    return null;
                }
            }
     ...
     * @return WebPopOver
     */
    public WebPopOver setMargin(final int top, final int left,
     ...
     *
     * @param movable
     *            whether this WebPopOver should be movable or not
     */
     ...
     *
     * @param popOverSourcePoint
     *            popup display source point
     */
    public void setPopOverSourcePoint(
            final PopOverSourcePoint popOverSourcePoint) {
     ...
     *
     * @param round
     *            new popup menu border corners rounding
     */
    @Override
     ...
     *
     * @param transparency
     *            new popup menu shade transparency
     */
    public void setShadeTransparency(final float transparency) {
        StyleManager.setCustomPainterProperty(container, "shadeTransparency",
                transparency);
    }
    ...
    public void setTransparency(final float transparency) {
        StyleManager.setCustomPainterProperty(container, "transparency",
                transparency);
    }
    
    /**
     * Returns popup style. This method is made protected because requesting
     * style is necessary only within the WebPopOver.
     *
     * @return popup style
     */
    ...
    /**
     * Sets popup style. This method is made protected because modifying style
     * is necessary only within the WebPopOver.
     *
     * @param style
     *            new popup style
     */
    ...
    protected void setCornerSide(final int cornerSide) {
        StyleManager.setCustomPainterProperty(container, "cornerSide",
                cornerSide);
    }
     ...
     *
     * @param cornerAlignment
     *            dropdown corner alignment
     */
    protected void setCornerAlignment(final int cornerAlignment) {
        StyleManager.setCustomPainterProperty(container, "cornerAlignment",
                cornerAlignment);
    }
    ...
    public void setPopOverFocused(final boolean focused) {
        StyleManager.setCustomPainterProperty(container, "popOverFocused",
                focused);
    }
    ...
    public WebPopOver show(final PopOverLocation location) {
        ...
        switch (location) {
        case center: {
            setLocation(ss.width / 2 - getWidth() / 2, ss.height / 2
                    - getHeight() / 2);
            break;
        }
        case topLeft: {
     ...
     *
     * @param x
     *            WebPopOver X location
     * @param y
     *            WebPopOver Y location
     * @return displayed WebPopOver
     */
    ...
    /**
     * Displays WebPopOver attached to the invoker component and faced to
     * preferred direction. WebPopOver opened in this way will always
     * auto-follow invoker's ancestor window.
     *
     * @param invoker
     *            invoker component
     * @return displayed WebPopOver
    ...
    /**
     * Displays WebPopOver attached to the invoker component and faced to
     * specified direction. WebPopOver opened in this way will always
     * auto-follow invoker's ancestor window.
     *
     * @param invoker
     *            invoker component
    ...
    /**
     * Displays WebPopOver attached to the invoker component and faced to
     * specified direction. It will also be aligned using the specified
     * alignment type when possible. WebPopOver opened in this way will always
     * auto-follow invoker's ancestor window.
     *
     * @param invoker
     *            invoker component
    ...
    /**
     * Displays WebPopOver attached to the invoker component coordinates and
     * faced to specified direction. It will also be aligned using the specified
     * alignment type when possible. WebPopOver opened in this way will always
     * auto-follow invoker's ancestor window.
     *
     * @param invoker
     *            invoker component
     * @param x
     *            source area X coordinate in invoker's component coordinate
     *            system
     * @param y
     *            source area Y coordinate in invoker's component coordinate
     *            system
     * @return displayed WebPopOver
     */
    ...
    /**
     * Displays WebPopOver attached to the invoker component area and faced to
     * specified direction. It will also be aligned using the specified
     * alignment type when possible. WebPopOver opened in this way will always
     * auto-follow invoker's ancestor window.
     *
     ...
     */
    public WebPopOver show(final Component invoker, final int x, final int y,
            final int w, final int h, final PopOverDirection direction) {
    ...
    public WebPopOver show(final Component invoker,
            final DataProvider<Rectangle> boundsProvider,
            final PopOverDirection direction) {
        return show(invoker, boundsProvider, direction,
                PopOverAlignment.centered);
    }
    ...
    public WebPopOver show(final Component invoker,
        ...
        final DataProvider<Rectangle> actualBoundsProvider = boundsProvider == null ? null
                : new DataProvider<Rectangle>() {
                    private Rectangle lastBounds = new Rectangle();
                    
                    @Override
                    public Rectangle provide() {
                        // Invoker might be hidden while WebPopOver is still
                        // visible
                        // This is why we should simply stop updating its
                        // position when that happens
                        // It is not the best workaround but at least it will
                        // keep us safe from exceptions
                        if (invoker.isShowing()) {
                            final Rectangle bounds = boundsProvider.provide();
                            final Point los = invoker.getLocationOnScreen();
                            lastBounds = new Rectangle(los.x + bounds.x, los.y
        ...
        // Updating WebPopOver variables
        attached = true;
        preferredDirection = direction != null ? direction
        ...
        // Fire opened event
        // Note that if this pop-over is modal this event will be fired only
        // after it is closed
        // Unfortunately there is no good way to provide this event after dialog
        // is opened but before it is closed in that case
        fireOpened();
    ...
    protected void updatePopOverLocation(final Component invoker) {
        if (invoker instanceof Window) {
            ...
            setLocation(ib.x + ib.width / 2 - size.width / 2, ib.y + ib.height
                    / 2 - size.height / 2);
     ...
     *
     * @param invokerBounds
     *            invoker component bounds on screen
     */
    protected void updatePopOverLocation(final Rectangle invokerBounds) {
        ...
        final Dimension ps = new Dimension(size.width - sw * 2, size.height
                - sw * 2);
        final Rectangle screenBounds = getGraphicsConfiguration().getBounds();
        final boolean ltr = getComponentOrientation().isLeftToRight();
        
        // Determining actual direction
        final PopOverDirection actualDirection = getActualDirection(
                invokerBounds, ltr, cw, ps, screenBounds);
        ...
        final Point actualLocation = getActualLocation(invokerBounds, ltr,
                round, cw, ps, screenBounds, actualDirection);
        actualLocation.x -= sw;
        actualLocation.y -= sw;
        
        // Updating corner position
        setCornerAlignment(-1);
        setRelativeCorner(getRelativeCorner(invokerBounds, actualDirection,
                actualLocation));
     ...
     */
    protected int getRelativeCorner(final Rectangle ib,
            final PopOverDirection actualDirection, final Point actualLocation) {
    ...
    /**
     * Returns actual WebPopOver location. Shade width is not yet taken into
     * account within this location.
     *
     * @param ib
     *            invoker component bounds on screen
     * @param ltr
     *            whether LTR orientation is active or not
     * @param round
     *            corners round
     * @param cw
     *            corner width
     ...
     */
    protected Point getActualLocation(final Rectangle ib, final boolean ltr,
            final int round, final int cw, final Dimension ps,
        ...
        if (actualDirection == PopOverDirection.up) {
            if (preferredAlignment == PopOverAlignment.centered) {
                ...
                return checkRightCollision(
                        checkLeftCollision(location, screenBounds), ps,
                        screenBounds);
            } else if (preferredAlignment == (ltr ? PopOverAlignment.leading
                    : PopOverAlignment.trailing)) {
                return checkLeftCollision(new Point(sp.x + cw * 2 + round
                        - ps.width, sp.y - cw - ps.height), screenBounds);
            } else if (preferredAlignment == (ltr ? PopOverAlignment.trailing
                    : PopOverAlignment.leading)) {
                return checkRightCollision(new Point(sp.x - cw * 2 - round,
                        sp.y - cw - ps.height), ps, screenBounds);
            }
            ...
            if (preferredAlignment == PopOverAlignment.centered) {
                ...
                return checkRightCollision(
                    ...
                    : PopOverAlignment.trailing)) {
                return checkLeftCollision(new Point(sp.x + cw * 2 + round
                        - ps.width, sp.y + cw), screenBounds);
            } else if (preferredAlignment == (ltr ? PopOverAlignment.trailing
                    : PopOverAlignment.leading)) {
                return checkRightCollision(new Point(sp.x - cw * 2 - round,
                        sp.y + cw), ps, screenBounds);
            }
        } else if (actualDirection == (ltr ? PopOverDirection.left
                : PopOverDirection.right)) {
            if (preferredAlignment == PopOverAlignment.centered) {
                final Point location = new Point(sp.x - cw - ps.width, sp.y
                        - ps.height / 2);
                return checkBottomCollision(
                        checkTopCollision(location, screenBounds), ps,
                        screenBounds);
            } else if (preferredAlignment == PopOverAlignment.leading) {
                return checkTopCollision(new Point(sp.x - cw - ps.width, sp.y
                        + cw * 2 + round - ps.height), screenBounds);
            } else if (preferredAlignment == PopOverAlignment.trailing) {
                return checkBottomCollision(new Point(sp.x - cw - ps.width,
                        sp.y - cw * 2 - round), ps, screenBounds);
            }
        } else if (actualDirection == (ltr ? PopOverDirection.right
                : PopOverDirection.left)) {
            if (preferredAlignment == PopOverAlignment.centered) {
                final Point location = new Point(sp.x + cw, sp.y - ps.height
                        / 2);
                return checkBottomCollision(
                        checkTopCollision(location, screenBounds), ps,
                        screenBounds);
            } else if (preferredAlignment == PopOverAlignment.leading) {
                return checkTopCollision(new Point(sp.x + cw, sp.y + cw * 2
                        + round - ps.height), screenBounds);
            } else if (preferredAlignment == PopOverAlignment.trailing) {
                return checkBottomCollision(new Point(sp.x + cw, sp.y - cw * 2
                        - round), ps, screenBounds);
            }
        }
        return null;
    }
    ...
    /**
     * Checks whether WebPopOver will collide with top screen border and
     * modifies location accordingly.
     *
     * @param location
    ...
    /**
     * Checks whether WebPopOver will collide with bottom screen border and
     * modifies location accordingly.
     *
     * @param location
     *            approximate WebPopOver location
     ...
     * @return either modified or unmodified WebPopOver location
     */
    protected Point checkBottomCollision(final Point location,
            final Dimension ps, final Rectangle screenBounds) {
    ...
    /**
     * Checks whether WebPopOver will collide with left screen border and
     * modifies location accordingly.
     *
     * @param location
     *            approximate WebPopOver location
    ...
    /**
     * Checks whether WebPopOver will collide with right screen border and
     * modifies location accordingly.
     *
     * @param location
     *            approximate WebPopOver location
     ...
     * @return either modified or unmodified WebPopOver location
     */
    protected Point checkRightCollision(final Point location,
            final Dimension ps, final Rectangle screenBounds) {
    ...
    /**
     * Returns actual direction depending on preferred WebPopOver direction, its
     * sizes and source point.
     *
     * @param ib
     *            invoker component bounds on screen
     * @param ltr
     ...
     */
    protected PopOverDirection getActualDirection(final Rectangle ib,
            final boolean ltr, final int cw, final Dimension ps,
            final Rectangle screenBounds) {
        for (final PopOverDirection checkedDirection : preferredDirection
                .getPriority()) {
            ...
            } else if (checkedDirection == (ltr ? PopOverDirection.left
                    : PopOverDirection.right)) {
                if (sp.x - cw - ps.width > screenBounds.x) {
                    return checkedDirection;
                }
            } else if (checkedDirection == (ltr ? PopOverDirection.right
                    : PopOverDirection.left)) {
    ...
    /**
     * Returns actual source point depending on WebPopOver direction and invoker
     ...
     *            whether LTR orientation is active or not
     * @param direction
     *            WebPopOver direction @return actual source point
     */
    protected Point getActualSourcePoint(final Rectangle ib, final boolean ltr,
            final PopOverDirection direction) {
        if (popOverSourcePoint == PopOverSourcePoint.componentCenter) {
            ...
            } else if (direction == (ltr ? PopOverDirection.left
                    : PopOverDirection.right)) {
                return new Point(ib.x, ib.y + ib.height / 2);
            } else if (direction == (ltr ? PopOverDirection.right
                    : PopOverDirection.left)) {
    ...
    public void fireOpened() {
        for (final PopOverListener listener : CollectionUtils
                .copy(popOverListeners)) {
