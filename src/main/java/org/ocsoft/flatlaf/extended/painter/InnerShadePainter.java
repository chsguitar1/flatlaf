...
public class InnerShadePainter<E extends JComponent> extends
        NinePatchIconPainter<E> {
    ...
    public InnerShadePainter(final int shadeWidth, final int round,
    ...
    public InnerShadePainter(final boolean drawTop, final boolean drawLeft,
            final boolean drawBottom, final boolean drawRight) {
        super();
    ...
    protected void updateNinePatchIcon() {
        ...
        setNinePatchIcon(NinePatchUtils.createInnerShadeIcon(shadeWidth, round,
                shadeOpacity));
    }
    ...
    public InnerShadePainter<E> setDrawSides(final boolean drawTop,
            final boolean drawLeft, final boolean drawBottom,
    ...
    public Insets getMargin(final E c) {
        return new Insets(drawTop ? shadeWidth : 0, drawLeft ? shadeWidth : 0,
                drawBottom ? shadeWidth : 0, drawRight ? shadeWidth : 0);
    }
    ...
    public void paint(final Graphics2D g2d, final Rectangle bounds, final E c) {
        ...
        if (cachedShadeWidth != shadeWidth || cachedRound != round
                || cachedShadeOpacity != shadeOpacity || icon == null) {
            updateNinePatchIcon();
        }
        ...
        if (icon != null) {
            icon.setComponent(c);
            icon.paintIcon(g2d, bounds.x - (drawLeft ? 0 : shadeWidth),
                    bounds.y - (drawTop ? 0 : shadeWidth), bounds.width
                            + (drawLeft ? 0 : shadeWidth)
                            + (drawRight ? 0 : shadeWidth), bounds.height
                            + (drawTop ? 0 : shadeWidth)
                            + (drawBottom ? 0 : shadeWidth));
        }
    }
}