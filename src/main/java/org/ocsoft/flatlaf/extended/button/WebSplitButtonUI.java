...
public class WebSplitButtonUI extends WebButtonUI {
    ...
    protected Insets getBorderInsets() {
        ...
        final int splitPartWidth = splitIcon.getIconWidth() + 1 + splitIconGap
                * 2 + contentGap;
        if (ltr) {
    ...
    public void paint(final Graphics g, final JComponent c) {
        ...
        g.setColor(c.isEnabled() ? FlatLafStyleConstants.borderColor
    ...
    public Rectangle getSplitButtonBounds(final JComponent c) {
        ...
        final int styleSide = drawRight ? shadeWidth + 1 : (drawRightLine ? 1
                : 0);
        final int height = c.getHeight() - i.top - i.bottom;
        if (ltr) {
            final int width = i.right - contentGap - styleSide;
            return new Rectangle(c.getWidth() - i.right + contentGap, i.top,
                    width, height);
        } else {
    ...
    public Rectangle getSplitButtonHitbox(final JComponent c) {
        final boolean ltr = c.getComponentOrientation().isLeftToRight();
        final Insets i = c.getInsets();
        return ltr ? new Rectangle(c.getWidth() - i.right + contentGap, 0,
                i.right - contentGap, c.getHeight()) : new Rectangle(0, 0,
                i.left - contentGap, c.getHeight());
    }
}