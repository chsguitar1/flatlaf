...
public class DefaultTreeCheckingModel<E extends DefaultMutableTreeNode>
    ...
    /**
     * Sets checkbox tree which uses this checking model. This method call also
     * forces cache to clear all stored values.
     *
     * @param checkBoxTree
     *            checkbox tree which uses this checking model.
     */
    ...
    /**
     * Returns list of nodes for the specified state. For a reasonable cause
     * this will not work for unchecked state.
     *
     * @param state
     *            check state
     * @param optimize
     *            whether should optimize selected nodes or not
     ...
     * @param toUpdate
     *            list of nodes for later update
     * @param changes
     *            list to collect state changes into
     */
    protected void setCheckedImpl(final E node, final boolean checked,
            final List<E> toUpdate, final List<CheckStateChange<E>> changes) {
        // Remembering old and new states
        final CheckState oldState = getCheckState(node);
        final CheckState newState = checked ? CheckState.checked
     ...
     *
     * @param node
     *            node to start checking parents from
     * @param toUpdate
     *            list of nodes for later update
     * @param changes
     *            list to collect state changes into
    ...
    protected void updateParentStates(final E node, final List<E> toUpdate,
        ...
        while (parent != null) {
            ...
            for (int i = 0; i < parent.getChildCount(); i++) {
                final CheckState checkState = getCheckState((E) parent
                        .getChildAt(i));
     ...
     *            list to collect state changes into
     */
    protected void updateChildNodesState(final E node,
            final CheckState newState, final List<E> toUpdate,
            final List<CheckStateChange<E>> changes) {
        for (int i = 0; i < node.getChildCount(); i++) {
            ...
            // Saving changes
            if (changes != null) {
                changes.add(new CheckStateChange<E>(childNode,
                        getCheckState(childNode), newState));
            }
     ...
     *            list of nodes for later update
     */
    protected void updateNodeState(final E node, final CheckState newState,
            final List<E> toUpdate) {
    ...
    public void invertCheck(final E node) {
        ...
        final List<E> toUpdate = new ArrayList<E>();
        setCheckedImpl(node,
                getNextState(getCheckState(node)) == CheckState.checked,
                toUpdate, changes);
        repaintTreeNodes(toUpdate);
    ...
    public void uncheckAll() {
        ...
        if (checkStateChangeListeners.size() > 0) {
            ...
            for (final Map.Entry<E, CheckState> entry : nodeCheckStates
                    .entrySet()) {
                final CheckState state = entry.getValue();
                if (state == CheckState.mixed || state == CheckState.checked) {
                    changes.add(new CheckStateChange<E>(entry.getKey(), state,
                            CheckState.unchecked));
                }
            }
        }
        ...
        if (checkStateChangeListeners.size() > 0) {
            ...
            for (final E node : allNodes) {
                ...
                if (state != CheckState.checked) {
                    changes.add(new CheckStateChange<E>(node, state,
                            CheckState.checked));
                }
            }
        }
    ...
    protected CheckState getNextState(final CheckState checkState) {
        ...
        case mixed:
            return checkBoxTree.isCheckMixedOnToggle() ? CheckState.checked
                    : CheckState.unchecked;
        default:
            return CheckState.unchecked;
        }
    ...
    public void checkingModeChanged(final boolean recursive) {
        ...
        if (recursive) {
            ...
            // We don't need to update them because they will get checked in the
            // process
            ...
            final Iterator<Map.Entry<E, CheckState>> iterator = nodeCheckStates
                    .entrySet().iterator();
            while (iterator.hasNext()) {
                ...
                if (entry.getValue() == CheckState.mixed) {
                    ...
                    // Saving changes
                    if (changes != null) {
                        changes.add(new CheckStateChange<E>(node,
                                CheckState.mixed, CheckState.unchecked));
                    }
                }
            }
        }
    ...
    /**
     * Filters out all nodes which are childs of other nodes presented in the
     * list.
     *
     * @param nodes
    ...
    /**
     * Returns whether the specified node is a child of another node or some of
     * its child nodes or not.
     *
     * @param node
     *            node to process
     * @param childOf
     *            node to compare parent nodes with
     * @return true if the specified node is a child of another node or some of
     *         its child nodes, false otherwise
     */
