...
public class AsyncTreeModel<E extends AsyncUniqueNode> extends WebTreeModel<E> {
     ...
     */
    protected final List<AsyncTreeModelListener> asyncTreeModelListeners = new ArrayList<AsyncTreeModelListener>(
            1);
    ...
    /**
     * Nodes cached states (parent ID -> childs cached state). If child nodes
     * for some parent node are cached then this map contains "true" value under
     * that parent node ID as a key.
     */
    protected final Map<String, Boolean> nodeCached = new HashMap<String, Boolean>();
    
    /**
     * Cache for childs nodes returned by data provider (parent ID -> list of
     * raw child nodes). This map contains raw childs which weren't affected by
     * sorting and filtering operations. If childs needs to be re-sorted or
     * re-filtered they are simply taken from the cache and re-organized once
     * again.
     */
    protected final Map<String, List<E>> rawNodeChildsCache = new HashMap<String, List<E>>();
    
    /**
     * Direct nodes cache (node ID -> node). Used for quick node search within
     * the tree.
     */
     ...
     *            data provider
     */
    public AsyncTreeModel(final WebAsyncTree<E> tree,
            final AsyncTreeDataProvider<E> dataProvider) {
        super(null);
        this.tree = tree;
     ...
     */
    protected void clearNodeChildsCache(final List<E> nodes,
     ...
     *            whether should clear nodes cache or not
     */
    protected void clearNodeChildsCache(final E[] nodes,
            final boolean clearNodes) {
        synchronized (cacheLock) {
            for (final E node : nodes) {
                clearNodeChildsCache(node, clearNodes);
    ...
    /**
     * Loads (or reloads) node childs and returns zero or childs count if async
     * mode is off. This is base method that uses installed
     * AsyncTreeDataProvider to retrieve tree node childs.
     *
     * @param parent
    ...
    protected int loadChildsCount(final E parent) {
        ...
        if (asyncLoading) {
            ...
            // This queue will also take care of amount of threads to execute
            // async trees requests
                ...
                public void run() {
                    ...
                    dataProvider.loadChilds(parent, new ChildsListener<E>() {
                        ...
                        public void childsLoadCompleted(final List<E> childs) {
                            ...
                            final List<E> realChilds = filterAndSort(parent,
                                    childs);
                            ...
                            // Performing UI updates and event notification in
                            // EDT
                            SwingUtils.invokeLater(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    if (realChilds != null
                                        ...
                                        insertNodesIntoImpl(realChilds, parent,
                                                0);
                                    }
                        ...
                        public void childsLoadFailed(final Throwable cause) {
                            // Caching childs
                            synchronized (cacheLock) {
                                rawNodeChildsCache.put(parent.getId(),
                                        new ArrayList<E>(0));
                                nodeCached.put(parent.getId(), true);
                            }
        ...
        SwingUtils.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                if (realChilds != null && realChilds.size() > 0) {
                    ...
                    // That might be required in case nodes were moved inside of
                    // the tree
     ...
     */
    @Override
    public void insertNodesInto(final List<E> children, final E parent,
            final int index) {
        // Simply ignore if parent node is not yet loaded
        if (!parent.isLoaded()) {
            return;
     ...
     */
    @Override
    public void insertNodesInto(final E[] children, final E parent,
            final int index) {
        // Simply ignore if parent node is not yet loaded
        if (!parent.isLoaded()) {
            return;
     ...
     *            insert index
     */
    protected void insertNodeIntoImpl(final E child, final E parent,
            final int index) {
     ...
     *            insert index
     */
    protected void insertNodesIntoImpl(final List<E> children, final E parent,
            final int index) {
     ...
     *            insert index
     */
    protected void insertNodesIntoImpl(final E[] children, final E parent,
            final int index) {
        super.insertNodesInto(children, parent, index);
     ...
     *            node which childs sorting and filtering should be updated
     * @param recursively
     *            whether should update the whole childs structure recursively
     *            or not
     */
    public void updateSortingAndFiltering(final E parentNode,
            final boolean recursively) {
        // Process only this is not a root node
        // We don't need to update root sorting as there is always one root in
        // the tree
        if (parentNode != null) {
            // Process this action only if node childs are already loaded and
            // cached
            if (parentNode.isLoaded()
                    && rawNodeChildsCache.containsKey(parentNode.getId())) {
                // Childs are already loaded, simply updating their sorting and
                // filtering
    ...
    /**
     * Updates node childs using current comparator and filter. Updates the
     * whole node childs structure if recursive update requested.
     *
     * @param parentNode
     *            node which childs sorting and filtering should be updated
     * @param recursively
     ...
     *            or not
     */
    protected void performSortingAndFiltering(final E parentNode,
            final boolean recursively) {
    ...
    /**
     * Performs raw childs filtering and sorting before they can be passed into
     * real tree and returns list of filtered and sorted childs.
     *
     * @param childs
    ...
    protected List<E> filterAndSort(final E parentNode, List<E> childs) {
        ...
        final Comparator<E> comparator = dataProvider
                .getChildsComparator(parentNode);
        if (filter != null) {
     ...
     *
     * @param listener
     *            asynchronous tree model listener to add
     */
     ...
     *
     * @param listener
     *            asynchronous tree model listener to remove
     */
    public void removeAsyncTreeModelListener(
            final AsyncTreeModelListener listener) {
        synchronized (modelListenersLock) {
