...
public class ExTreeModel<E extends UniqueNode> extends WebTreeModel<E> {
     ...
     *            data provider
     */
    public ExTreeModel(final WebTree<E> tree,
            final ExTreeDataProvider<E> dataProvider) {
        super(null);
        this.tree = tree;
    ...
    /**
     * Forces model to cache the whole structure so any node can be accessed
     * right away. Note that this might take some time in case tree structure is
     * large. Though this doesn't force any repaints or other visual updates, so
     * the speed depends only on ExTreeDataProvider.
     */
    ...
    public void reload(final TreeNode node) {
        ...
        // We don't need to inform about child nodes removal here due to later
        // structural update call
