 ...
 * childs that are accepted by filter. This can be used in any kind of filter
 * fields to provide a proper visual feedback in tree.
 *
 * @author Mikle Garin
 */

public class StructuredTreeNodesFilter<E extends UniqueNode> implements
        Filter<E> {
    /**
    ...
    /**
     * Sets nodes text provider. If set to null DefaultTextProvider will be used
     * instead.
     *
     * @param textProvider
     *            new nodes text provider
     */
    public void setTextProvider(final TextProvider<E> textProvider) {
        this.textProvider = textProvider != null ? textProvider
     ...
     *
     * @return true if should use space character as requests separator, false
     *         otherwise
     */
     ...
     *
     * @param useSpaceAsSeparator
     *            whether should use space character as requests separator or
     *            not
     */
    ...
    public boolean accept(final E node) {
        if (originalFilter == null || originalFilter.accept(node)) {
            ...
            final String searchRequest = matchCase ? searchText : searchText
                    .toLowerCase();
            return searchRequest.equals("")
                    || acceptIncludingChilds(node, searchRequest);
        } else {
            return false;
        }
    }
    ...
    /**
     * Returns whether the specified node or any of its childs match the filter
     * or not.
     *
     * @param node
     *            node to match
     * @param searchRequest
     *            search request text
     * @return true if the specified node or any of its childs match the filter,
     *         false otherwise
     */
    protected boolean acceptIncludingChilds(final E node,
    ...
    /**
     * Returns whether the specified node matches the filter or not. This method
     * might return cached value if it exists, otherwise it will retrieve and
     * cache a new value.
     *
     * @param node
     *            node to match
    ...
    protected boolean acceptNodeImpl(final E node, final String searchRequest) {
        final String nodeText = matchCase ? textProvider.provide(node)
                : textProvider.provide(node).toLowerCase();
     ...
     * @param searchFromStart
     *            whether should start searching from the beginning of the node
     *            text
     * @return true if filter accepts specified node text, false otherwise
     */
    protected boolean accept(final String nodeText, final String searchRequest,
            final boolean searchFromStart) {
        return searchFromStart ? nodeText.startsWith(searchRequest) : nodeText
