...
public abstract class AbstractTreeTransferHandler<N extends UniqueNode, T extends WebTree<N>, M extends WebTreeModel<N>>
    ...
    /**
     * Whether or not should expand single dragged node when it is dropped onto
     * the tree.
     */
    protected boolean expandSingleNode = true;
    
    /**
     * Whether or not should expand multiply dragged nodes when they are dropped
     * onto the tree.
     */
    ...
    /**
     * Map of node indices lists under their parent node IDs. This map is used
     * to properly adjust drop index in the destination node if D&D performed
     * within one tree.
     */
     ...
     *
     * @return true if should optimize dragged nodes list to minimum, false
     *         otherwise
     */
    ...
    /**
     * Returns whether should expand single dragged node when it is dropped onto
     * the tree or not.
     *
     * @return true if should expand single dragged node when it is dropped onto
    ...
    /**
     * Sets whether should expand single dragged node when it is dropped onto
     * the tree or not.
     *
     * @param expand
     *            whether should expand single dragged node when it is dropped
     *            onto the tree or not
     */
    ...
    /**
     * Returns whether or not should expand multiply dragged nodes when they are
     * dropped onto the tree.
     *
     * @return true if should expand multiply dragged nodes when they are
     *         dropped onto the tree, false otherwise
     */
    ...
    /**
     * Sets whether or not should expand multiply dragged nodes when they are
     * dropped onto the tree.
     *
     * @param expand
     *            whether or not should expand multiply dragged nodes when they
     *            are dropped onto the tree
     */
     ...
     *         property associated with component is null
     */
    @Override
    ...
    public boolean canImport(final TransferSupport support) {
        ...
        try {
            // Check whether actual TransferHandler accepts drop to this
            // location
            ...
            // Do not allow drop inside one of dragged elements if this is a
            // MOVE operation
            // Will not work when dragged to another tree, but it doesn't matter
            // in that case
            ...
            // This should be checked inside specific TransferHandler with its
            // own checks
            // // Do not allow drop into old location of one of the dragged
            // nodes
            // if ( isMoveAction ( support.getDropAction () ) )
            // {
            ...
            final List<N> nodes = (List<N>) support.getTransferable()
    ...
    public boolean importData(final TransferHandler.TransferSupport support) {
        ...
        final List<N> nodes;
        try {
            nodes = (List<N>) support.getTransferable().getTransferData(
                    nodesFlavor);
        } catch (final UnsupportedFlavorException ufe) {
            FlatLafLogger.warn(this, "UnsupportedFlavor: " + ufe.getMessage());
            return false;
        ...
        return prepareDropOperation(support, nodes, dropIndex, parent, tree,
                model);
    }
    
    /**
     * Performs all preparations required to perform drop operation and calls
     * for actual drop when ready.
     *
     * @param support
     *            transfer support data
     * @param nodes
     *            list of nodes to drop
     * @param dropIndex
     *            preliminary nodes drop index
     * @param parent
     *            parent node to drop nodes into
     * @param tree
     *            tree to drop nodes onto
     * @param model
     *            tree model
     * @return true if drop operation was successfully completed, false
     *         otherwise
     */
    protected abstract boolean prepareDropOperation(
            final TransferSupport support, final List<N> nodes,
            final int dropIndex, final N parent, final T tree, final M model);
     ...
     *         otherwise
     */
    protected abstract boolean performDropOperation(final List<N> nodes,
            final N parent, final T tree, final M model, final int index);
    
    /**
     * Returns properly adjusted nodes drop index.
     *
     * @param dropIndex
     *            drop index if dropped between nodes under dropLocation node or
     *            -1 if dropped directly onto dropLocation node
     * @param dropAction
     *            actual drop action
     * @param parent
     *            parent node to drop nodes into @return properly adjusted nodes
     *            drop index
     */
    protected int getAdjustedDropIndex(final int dropIndex,
            final int dropAction, final N parent) {
        // Fixing drop index for case when dropped to non-leaf element
        int adjustedDropIndex = dropIndex == -1 ? parent.getChildCount()
                : dropIndex;
        
        // Adjusting drop index for MOVE operation
        if (isMoveAction(dropAction) && draggedNodeIndices != null
                && draggedNodeIndices.containsKey(parent.getId())) {
            for (final Integer index : draggedNodeIndices.get(parent.getId())) {
                if (index < adjustedDropIndex) {
                    // We simply decrement inserted index in case some node
                    // which was higher than this one was deleted
                    // That allows us to have index that is correct when dragged
                    // nodes are already removed from the tree
                    adjustedDropIndex--;
                }
            }
        }
     ...
     * that was transferred if the action was MOVE. This method is invoked from
     * EDT so it is safe to perform tree operations.
     *
     * @param source
    ...
    /**
     * Checks whether specified target is acceptable for drop or not. This check
     * is performed before another check for nodes drop possibility.
     *
     * @param dropLocation
     *            node onto which drop was performed
     * @return true if the specified target is acceptable for drop, false
     *         otherwise
     */
    ...
    /**
     * Returns whether nodes can be dropped to the specified location and index
     * or not.
     *
     * @param nodes
     *            list of nodes to drop
     ...
     *            drop index if dropped between nodes under dropLocation node or
     *            -1 if dropped directly onto dropLocation node
     * @return true if nodes can be dropped to the specified location and index,
     *         false otherwise
     */
    protected abstract boolean canBeDropped(List<N> nodes, N dropLocation,
            int dropIndex);
    
    /**
     * Returns node copy used in createTransferable. Used each time when node is
     * moved within tree or into another tree. Node copy should have the same ID
     * and content but must be another instance of node type class.
     *
     * @param node
     *            node to copy
     * @return node copy
    ...
    /**
     * Informs about nodes drop operation completition in a separate tree
     * thread. This method should be used to perform actual data move operation.
     *
     * @param nodes
     *            list of nodes to drop
     * @param parent
     ...
     *            nodes drop index
     */
    public abstract void nodesDropped(final List<N> nodes, final N parent,
            final T tree, final M model, final int index);
