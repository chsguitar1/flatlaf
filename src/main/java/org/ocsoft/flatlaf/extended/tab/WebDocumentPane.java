 ...
 * described by DocumentData class. You can also override DocumentData class and
 * for example include your own data into the document itself.
 * <p/>
 * This component uses either single or multiply tabbed panes and allow tabs
 * reorder, drag, split and closability. All those features are of course
 * configurable within the WebDocumentPane instance.
 *
 * @param <T>
 *            document type
 * @author Mikle Garin
 ...
 * @see org.ocsoft.flatlaf.extended.tab.PaneData
 * @see org.ocsoft.flatlaf.extended.tab.SplitData
 * @see org.ocsoft.flatlaf.extended.tab.DocumentData
 */

public class WebDocumentPane<T extends DocumentData> extends WebPanel implements
        DocumentPaneEventMethods<T>, SettingsMethods, SwingConstants {
    /**
     ...
     * Used icons.
     */
    public static final ImageIcon closeTabIcon = new ImageIcon(
            PaneData.class.getResource("icons/close.png"));
    public static final ImageIcon closeTabRolloverIcon = new ImageIcon(
            PaneData.class.getResource("icons/close-rollover.png"));
     ...
     */
    protected List<DocumentListener<T>> documentListeners = new ArrayList<DocumentListener<T>>(
            1);
    
    /**
     * Document pane event listeners.
     */
    protected List<DocumentPaneListener<T>> documentPaneListeners = new ArrayList<DocumentPaneListener<T>>(
            1);
    
    /**
     * Unique document pane ID. Used to allow or disallow documents drag between
     * different document panes.
     */
     ...
     * Constructs new document pane.
     *
     * @param tabTitleComponentProvider
     *            tab title component customizer
     */
    public WebDocumentPane(
     ...
     * Constructs new document pane.
     *
     * @param splitPaneCustomizer
     *            split pane customizer
     * @param tabbedPaneCustomizer
     *            tabbed pane customizer
     */
    public WebDocumentPane(final Customizer<WebSplitPane> splitPaneCustomizer,
            final Customizer<WebTabbedPane> tabbedPaneCustomizer) {
    ...
    public WebDocumentPane(final Customizer<WebSplitPane> splitPaneCustomizer,
        ...
        this.tabTitleComponentProvider = tabTitleComponentProvider != null ? tabTitleComponentProvider
        ...
        final DocumentDragViewHandler dragViewHandler = new DocumentDragViewHandler(
                this);
    ...
    /**
     * Returns unique document pane ID. Might be used within D&D functionality
     * to determine whether drag source is the same as destination.
     *
    ...
    /**
     * Sets tabbed pane customizer and applies it to existing panes. Note that
     * changes made by previously set customizers are not reverted even if you
     * set this to null.
     *
     * @param customizer
     *            new tabbed pane customizer
     */
    public void setTabbedPaneCustomizer(
            final Customizer<WebTabbedPane> customizer) {
     ...
     *
     * @param provider
     *            new document tab title component provider
     */
    public void setTabTitleComponentProvider(
            final TabTitleComponentProvider<T> provider) {
        this.tabTitleComponentProvider = provider != null ? provider
                : createDefaultTabTitleComponentProvider();
        for (final PaneData<T> paneData : getAllPanes()) {
    ...
    /**
     * Sets split pane customizer and applies it to existing panes. Note that
     * changes made by previously set customizers are not reverted even if you
     * set this to null.
     *
     * @param customizer
     *            new split pane customizer
     */
     ...
     *
     * @return true if tabs in this document pane are globally closable, false
     *         otherwise
     */
    ...
    /**
     * Sets whether tabs in this document pane should be globally closable or
     * not.
     *
     * @param closeable
     *            whether tabs in this document pane should be globally closable
     *            or not
     */
     ...
     *
     * @param dragEnabled
     *            whether tabs drag is enabled or not
     */
    ...
    /**
     * Returns whether tabs drag between different tabbed panes is enabled or
     * not.
     *
     * @return true if tabs drag between different tabbed panes is enabled,
     *         false otherwise
     */
     ...
     *
     * @param dragBetweenPanesEnabled
     *            whether tabs drag between different tabbed panes is enabled or
     *            not
     */
    ...
    /**
     * Returns current root element data. This is either SplitData or PaneData
     * object.
     *
    ...
    /**
     * Sets new root element data. This call replaces all data stored in this
     * document pane with new one.
     *
     * @param root
     *            new root element data
     */
    ...
    /**
     * Splits document's pane into two panes using the specified direction to
     * decide split settings.
     *
     * @param movedDocument
     *            document that should be moved to new pane
     * @param direction
     *            split direction
     */
    ...
    /**
     * Splits specified pane into two panes using the specified direction to
     * decide split settings.
     *
     * @param splittedPane
     ...
     */
    protected PaneData<T> split(final PaneData<T> splittedPane,
            final T movedDocument, final int direction) {
        ...
        if (splittedPane != null) {
            ...
            final int orientation = direction == LEFT || direction == RIGHT ? VERTICAL
                    : HORIZONTAL;
            ...
            if (splittedPane.getTabbedPane().getParent() == WebDocumentPane.this) {
                ...
                splitData = new SplitData<T>(WebDocumentPane.this, orientation,
                        first, last);
                remove(splittedPane.getTabbedPane());
                add(splitData.getSplitPane(), BorderLayout.CENTER);
                
                // Restoring split locations
                splitData.getSplitPane().setDividerLocation(
                        orientation == VERTICAL ? size.width / 2
                                : size.height / 2);
                ...
                final WebSplitPane parentSplit = (WebSplitPane) splittedPane
                        .getTabbedPane().getParent();
                final SplitData<T> parentSplitData = getData(parentSplit);
                if (parentSplitData.getOrientation() == orientation && ltr
                        && parentSplitData.getFirst() == splittedPane
                ...
                } else if (parentSplitData.getOrientation() == orientation
                        && !ltr && parentSplitData.getLast() == splittedPane
                    ...
                    // Saving sizes to restore split locations
                    final int parentSplitLocation = parentSplitData
                    ...
                    final PaneData<T> first = ltr ? splittedPane : otherPane;
                    final PaneData<T> last = ltr ? otherPane : splittedPane;
                    splitData = new SplitData<T>(WebDocumentPane.this,
                    ...
                    splitData.getSplitPane().setDividerLocation(
                            orientation == VERTICAL ? size.width / 2
                                    : size.height / 2);
                    parentSplitData.getSplitPane().setDividerLocation(
                            parentSplitLocation);
                }
            }
    ...
    /**
     * Merges specified structure element and its sub-elements if it is
     * possible. If PaneData provided its parent split will be merged. If
     * SplitData provided it will be merged.
     *
     * @param toMerge
     *            structure element to merge
     */
    ...
    protected void mergeImpl(final SplitData<T> splitData) {
        ...
        if (isEmptyPane(first) || isEmptyPane(last)) {
            ...
            final PaneData<T> toPane = firstPane.count() > lastPane.count() ? firstPane
                    : lastPane;
            final PaneData<T> fromPane = firstPane.count() > lastPane.count() ? lastPane
                    : firstPane;
        ...
        if (splitPane.getParent() == WebDocumentPane.this) {
            ...
            final WebSplitPane parentSplit = (WebSplitPane) splitPane
                    .getParent();
            final SplitData<T> parentSplitData = getData(parentSplit);
    ...
    /**
     * Returns currently active pane data. This is the last pane that had focus
     * within this document pane.
     *
    ...
    /**
     * Returns document with the specified ID or null if it is not inside this
     * document pane.
     *
     * @param id
     *            document ID
     * @return document with the specified ID or null if it is not inside this
     *         document pane
     */
    ...
    /**
     * Collects all PaneData available under the specified stucture element into
     * list.
     *
     * @param structureData
     *            structure element
     ...
     *            PaneData list
     */
    protected void collectPanes(final StructureData structureData,
            final List<PaneData<T>> panes) {
    ...
    /**
     * Collects all SplitData available under the specified stucture element
     * into list.
     *
     * @param structureData
     *            structure element
     ...
     *            SplitData list
     */
    protected void collectSplitPanes(final StructureData structureData,
            final List<SplitData<T>> splits) {
    ...
    /**
     * Returns whether specified document is opened inside this document pane or
     * not.
     *
     * @param document
     *            document to look for
     * @return true if specified document is opened inside this document pane,
     *         false otherwise
     */
    ...
    /**
     * Returns whether document with the specified ID is opened inside this
     * document pane or not.
     *
     * @param documentId
     *            ID of the document to look for
     * @return true if document with the specified ID is opened inside this
     *         document pane, false otherwise
     */
    ...
    /**
     * Opens document with the specified ID in this document pane. This method
     * won't work in case you didn't set custom DocumentDataProvider.
     *
     * @param documentId
    ...
    /**
     * Closes all documents. Be aware that some documents might cancel their
     * close operation and will still be opened after this call.
     */
    ...
    /**
     * Checks selection state and fires selection event if required. This method
     * is separated from actual selection due to various difficulties
     * implementing it on-place. Selection is checked on pane activation,
     * documents tab changes.
     */
    ...
    /**
     * Returns custom documents provider. This may be used in to open documents
     * by ID instead of document references. It may also be used for state
     * restoration method.
     *
    ...
    /**
     * Sets custom documents provider. This may be used in to open documents by
     * ID instead of document references. It may also be used for state
     * restoration method.
     *
     * @param provider
     *            custom documents provider
     */
    ...
    /**
     * Returns current document pane state. This state contains opened document
     * IDs references and structure composition. It might basically be used to
     * save/restore document pane documents structure.
     *
    ...
    /**
     * Returns document pane state starting from the specified structure. This
     * state contains opened document IDs references and structure composition.
     * It might basically be used to save/restore document pane documents
     * structure.
     *
     * @param structure
     ...
     */
    protected DocumentPaneState getDocumentPaneStateImpl(
            final StructureData structure) {
        if (structure != null) {
            ...
            if (structure instanceof PaneData) {
                ...
                return new DocumentPaneState(
                        selected != null ? selected.getId() : null,
                        paneData.getDocumentIds());
            } else {
                final SplitData<T> splitData = (SplitData<T>) structure;
                final DocumentPaneState first = getDocumentPaneStateImpl(splitData
                        .getFirst());
                final DocumentPaneState last = getDocumentPaneStateImpl(splitData
                        .getLast());
                final Pair<DocumentPaneState, DocumentPaneState> splitState = new Pair<DocumentPaneState, DocumentPaneState>(
                        first, last);
                return new DocumentPaneState(splitData.getOrientation(),
                        splitData.getDividerLocation(), splitState);
            }
        } else {
            // This null case might occur in case one of split sides doesn't
            // have child
     ...
     * <p/>
     * This will remove all added documents and reopen them according to
     * restored state. Make sure that you add all required documents before
     * loading restoring the state. Otherwise in some cases you might get
     * unwanted effect like lost structure parts.
     * <p/>
     * Also be aware that this call might generate some unwanted events like
     * documents close and such. So make sure to add your listeners after this
     * call in case you don't want to listen to initial state restore events.
     *
     * @param state
     *            document pane state to restore
     * @see DocumentPaneState
     * @see #getDocumentPaneState()
     */
    public void setDocumentPaneState(final DocumentPaneState state) {
        // Mapping all opened documents
        // In case some are missing they can be filled-in later using document
        // provider
        // It doesn't really matter where these documents come from but we
        // assume that they might already be opened
        final List<T> openedDocuments = getDocuments();
        final Map<String, T> documents = new HashMap<String, T>(
    ...
    /**
     * Restores document pane state starting from the specified structure. If
     * null structure is provided restore operation will be started from root.
     *
     * @param state
     *            document pane state to restore
     * @param documents
     *            existing documents
     */
    protected StructureData<T> restoreStructureStateImpl(
            final DocumentPaneState state, final Map<String, T> documents) {
        final StructureData<T> restored;
        if (state.isSplit()) {
            // Restoring split data
            final Pair<DocumentPaneState, DocumentPaneState> splitState = state
                    .getSplitState();
            final StructureData<T> first = restoreStructureStateImpl(
                    splitState.getKey(), documents);
            final StructureData<T> last = restoreStructureStateImpl(
                    splitState.getValue(), documents);
            final SplitData<T> splitData = new SplitData<T>(this,
                    state.getSplitOrientation(), first, last);
            
            // todo Location updates should be queued and performed 1 by 1 from
            // top to bottom to avoid incorrect layouting
            // We have to wait until split is properly sized before restoring
            // divider location
            splitData.getSplitPane().addComponentListener(
                    new ComponentAdapter() {
                        @Override
                        public void componentResized(final ComponentEvent e) {
                            // Waiting until split is properly sized
                            final WebSplitPane splitPane = splitData
                                    .getSplitPane();
                            if (splitPane.getWidth() > 0
                                    || splitPane.getHeight() > 0) {
                                // Updating proportional divider location
                                splitData.setDividerLocation(state
                                        .getDividerLocation());
            ...
            if (state.getDocumentIds() != null) {
                for (final String id : state.getDocumentIds()) {
                    // In case document doesn't exist, try requesting it from
     ...
     *
     * @param listener
     *            document pane listener
     */
    public void removeDocumentPaneListener(
            final DocumentPaneListener<T> listener) {
     ...
     *            splitted PaneData
     * @param newSplitData
     *            newly created SplitData
     */
    public void fireSplitted(final PaneData<T> splittedPane,
            final SplitData<T> newSplitData) {
        for (final DocumentPaneListener<T> listener : CollectionUtils
                .copy(documentPaneListeners)) {
     ...
     * @param mergedSplit
     *            merged SplitData
     * @param newStructureData
     *            newly created StructureData
     */
    public void fireMerged(final SplitData<T> mergedSplit,
     ...
     *
     * @param splitData
     *            SplitData which orientation has changed
     */
     ...
     *
     * @param splitData
     *            SplitData which sides were swapped
     */
     ...
     *
     * @param splitData
     *            SplitData which divider location has changed
     */
     ...
     *            document's index
     */
    public void fireDocumentOpened(final T document, final PaneData<T> pane,
            final int index) {
        for (final DocumentListener<T> listener : CollectionUtils
                .copy(documentListeners)) {
     ...
     *            document's index
     */
    public void fireDocumentSelected(final T document, final PaneData<T> pane,
            final int index) {
        for (final DocumentListener<T> listener : CollectionUtils
                .copy(documentListeners)) {
    ...
    /**
     * Fires document closing event. Returns whether document is allowed to
     * close or not.
     *
     * @param document
     *            closing document
     ...
     */
    public boolean fireDocumentClosing(final T document,
            final PaneData<T> pane, final int index) {
        boolean allow = true;
        for (final DocumentListener<T> listener : CollectionUtils
                .copy(documentListeners)) {
     ...
     *            document's index
     */
    public void fireDocumentClosed(final T document, final PaneData<T> pane,
            final int index) {
        for (final DocumentListener<T> listener : CollectionUtils
                .copy(documentListeners)) {
     ...
     */
    @Override
    public DocumentAdapter<T> onDocumentOpen(
     ...
     */
    @Override
    public DocumentAdapter<T> onDocumentClose(
            final DocumentDataRunnable<T> runnable) {
     ...
     */
    @Override
    public <V extends DefaultValue> void registerSettings(final String key,
            final Class<V> defaultValueClass) {
     ...
     */
    @Override
    public <V extends DefaultValue> void registerSettings(final String group,
            final String key, final Class<V> defaultValueClass) {
     ...
     */
    public static <T extends DocumentData> PaneData<T> getData(
            final WebTabbedPane tabbedPane) {
     ...
     */
    public static <T extends DocumentData> SplitData<T> getData(
            final WebSplitPane splitPane) {
