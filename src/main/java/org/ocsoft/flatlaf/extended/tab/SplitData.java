 ...
 * pane and links to two other elements contained within split pane.
 *
 * @author Mikle Garin
 * @see <a
...
public final class SplitData<T extends DocumentData> implements
        StructureData<T> {
    /**
     ...
     *            last split element
     */
    public SplitData(final WebDocumentPane<T> documentPane,
            final int orientation, final StructureData first,
            final StructureData last) {
        super();
     ...
     *            first split element
     * @param last
     *            last split element @return new split component
     */
    protected WebSplitPane createSplit(final int orientation,
            final StructureData first, final StructureData last) {
        final WebSplitPane splitPane = new WebSplitPane(orientation,
                first.getComponent(), last.getComponent());
                ...
                new PropertyChangeListener() {
                    ...
                    public void propertyChange(final PropertyChangeEvent evt) {
                        SwingUtilities.invokeLater(new Runnable() {
                            @Override
                            public void run() {
                                getDocumentPane().fireDividerLocationChanged(
                                        SplitData.this);
                            }
                        });
                    }
     ...
     *            parent WebDocumentPane
     */
    protected void updateSplitPaneCustomizer(
            final WebDocumentPane<T> documentPane) {
        final Customizer<WebSplitPane> customizer = documentPane
    ...
    public void changeOrientation() {
        setOrientation(orientation == WebSplitPane.HORIZONTAL_SPLIT ? WebSplitPane.VERTICAL_SPLIT
     ...
     *
     * @param location
     *            new proportional split divider location
     */
     ...
     *            element replacement
     */
    public void replace(final StructureData element,
            final StructureData replacement) {
        if (first == element) {
            first = replacement;
