 ...
 * specially for WebDocumentPane component.
 *
 * @author Mikle Garin
 * @see <a
...
public class DocumentDragHandler extends TransferHandler {
    /**
     * Distance from pane side within which drop is determined as a split
     * action.
     */
    ...
    /**
     * Dragged away document index. Used to restore document position if drag is
    ...
    protected void exportDone(final JComponent source, final Transferable data,
        ...
        if (!documentPane.isDragBetweenPanesEnabled()
                && documentPane.getDocument(document.getId()) == null) {
            // We cannot drag to other document pane and yet document is not in
            // this pane
            // That means that it was dragged out somewhere and didn't find a
            // new place
    ...
    public boolean canImport(final TransferSupport support) {
        try {
            return support.getTransferable().isDataFlavorSupported(
                    DocumentTransferable.flavor)
                    && support.getTransferable().getTransferData(
                            DocumentTransferable.flavor) != null;
    ...
    public boolean importData(final TransferSupport support) {
        try {
            ...
            // 1. Dropped in the middle of tabbed pane - simply add document to
            // end
            ...
            if (index != -1) {
                ...
                if (dropSide != -1) {
                    ...
                    final PaneData otherPane = paneData.getDocumentPane()
                            .split(paneData, document, dropSide);
     ...
     *
     * @param paneData
     *            pane to transfer focus into
     */
    protected void checkFocusState(final PaneData paneData) {
        // We have to perform focus transfer AFTER the drag ends, otherwise
        // focus may be lost again
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                if (!SwingUtils.hasFocusOwner(pane)) {
                    ...
                    if (c.isFocusable() || c instanceof Container
                            && SwingUtils.hasFocusableComponent((Container) c)) {
                        pane.transferFocus();
                    } else {
    ...
    public static void install(final PaneData paneData) {
        ...
        final MouseAdapter dragAdapter = new MouseAdapter() {
            ...
            public void mousePressed(final MouseEvent e) {
                readyToDrag = paneData.getDocumentPane().isDragEnabled()
            ...
            public void mouseDragged(final MouseEvent e) {
                if (readyToDrag) {
                    readyToDrag = false;
                    tabbedPane.getTransferHandler().exportAsDrag(tabbedPane, e,
