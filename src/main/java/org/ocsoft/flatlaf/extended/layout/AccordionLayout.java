...
public class AccordionLayout extends AbstractLayoutManager {
    ...
    private Dimension getLayoutSize(Container parent, boolean preferred) {
        ...
        final int gap = accordion.getAccordionStyle() == AccordionStyle.separated ? accordion
                .getGap() : 0;
        ...
        for (final WebCollapsiblePane pane : panes) {
            final Dimension cps = preferred || !accordion.isFillSpace() ? pane
                    .getPreferredSize() : pane.getBasePreferredSize();
            if (hor) {
                ps.width += cps.width;
    ...
    public void layoutContainer(Container parent) {
        ...
        if (accordion.isFillSpace()) {
            ...
            float totalStates = 0;
            int totalFillLength = (hor ? size.width - insets.left
            ...
            for (int i = 0; i < panes.size(); i++) {
                ...
                if (expandState > 0f) {
                    ...
                    length += Math.round((float) Math.floor(totalFillLength
                            * expandState / totalStates))
                            + (i == lastFillIndex ? Math.round(end) : 0);
                }
                panes.get(i)
                        .setBounds(x, y, hor ? length : w, hor ? h : length);
                if (hor) {
                    x += length + gap;
                } else {
                    y += length + gap;
