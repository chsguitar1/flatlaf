 ...
 * overlaying components atop of single main component.
 *
 * @author Mikle Garin
 */

public class OverlayLayout extends AbstractLayoutManager implements
        SwingConstants {
    ...
    public void addComponent(Component component, Object constraints) {
        ...
        if (value == null || !value.equals(COMPONENT) && !value.equals(OVERLAY)) {
            throw new IllegalArgumentException(
                    "Cannot add to layout: constraint must be 'COMPONENT' or 'OVERLAY' string");
        }
    ...
    public Dimension preferredLayoutSize(Container parent) {
        ...
        for (Component component : parent.getComponents()) {
            ...
            if (constraint != null && constraint.equals(COMPONENT)) {
                ...
                ps = SwingUtils.max(ps, new Dimension(bi.left + ci.left
                        + cps.width + ci.right + bi.right, bi.top + ci.top
                        + cps.height + ci.bottom + bi.bottom));
            }
        }
        return ps;
    }
    ...
    protected Insets getActualComponentInsets(Container parent) {
        return componentMargin != null ? (parent.getComponentOrientation()
                .isLeftToRight() ? componentMargin : SwingUtils
                .toRTL(componentMargin)) : new Insets(0, 0, 0, 0);
    }
        ...
        for (Component component : parent.getComponents()) {
            ...
            if (constraint != null) {
                ...
                if (constraint.equals(COMPONENT)) {
                    component.setBounds(bi.left + ci.left, bi.top + ci.top, pw
                            - bi.left - bi.right - ci.left - ci.right, ph
                            - bi.top - bi.bottom - ci.top - ci.bottom);
                    ...
                    Insets om = overlayMargin != null ? overlayMargin
                            : new Insets(0, 0, 0, 0);
                    if (data.getLocation().equals(OverlayLocation.fill)) {
                        component.setBounds(bi.left + om.left, bi.top + om.top,
                                pw - bi.left - om.left - bi.right - om.right,
                        ...
                        component.setBounds(x, y, halign == -1 ? pw - bi.left
                                - om.left - bi.right - om.right : ps.width,
                                valign == -1 ? ph - bi.top - om.top - bi.bottom
                                        - om.bottom : ps.height);
                    } else {
                        component.setBounds(data.getRectangleProvider()
                                .provide());
                    }
                }
            }
