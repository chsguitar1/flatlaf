...
/**
 * Custom layout that allows multiply layout strategies to be applied to single
 * container. Be aware that standard Swing layouts aren't developed to be used
 * together so they aren't suitable for this layout. You will need custom
 * layouts which work only with specific components and won't affect the same
 * components.
 *
 * @author Mikle Garin
 */
...
public class MultiLayout extends AbstractLayoutManager {
    ...
    public void addLayoutComponent(final Component comp,
        ...
        for (final LayoutManager layoutManager : layoutManagers) {
            if (layoutManager instanceof LayoutManager2) {
                ((LayoutManager2) layoutManager).addLayoutComponent(comp,
                        constraints);
            }
        }
    }
    ...
    public void addComponent(final Component component, final Object constraints) {
        for (final LayoutManager layoutManager : layoutManagers) {
            if (layoutManager instanceof LayoutManager2) {
                ((LayoutManager2) layoutManager).addLayoutComponent(component,
                        constraints);
            } else {
                layoutManager.addLayoutComponent(constraints == null ? null
                        : constraints.toString(), component);
            }
        }
    }
    ...
    public Dimension maximumLayoutSize(final Container parent) {
        ...
        for (final LayoutManager layoutManager : layoutManagers) {
            if (layoutManager instanceof LayoutManager2) {
                ms = SwingUtils.min(ms, ((LayoutManager2) layoutManager)
                        .maximumLayoutSize(parent));
            }
        }
        return ms;
    }
