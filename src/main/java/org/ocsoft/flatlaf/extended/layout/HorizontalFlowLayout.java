...
public class HorizontalFlowLayout extends AbstractLayoutManager {
    ...
    public void layoutContainer(final Container parent) {
        ...
        final Dimension available = new Dimension(required.width,
        ...
        final int height = Math.max(available.height, required.height)
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (c.isVisible()) {
                ...
                final int width = fillLast && i == count - 1
                        && parent.getWidth() - x - rs > 0 ? parent.getWidth()
                        - x - rs : w;
                if (ltr) {
    ...
    protected Dimension getLayoutSize(final Container parent, final boolean min) {
        ...
        for (int i = 0; i < count; i++) {
            final Component c = parent.getComponent(i);
            final Dimension tmp = (min) ? c.getMinimumSize() : c
                    .getPreferredSize();
