 ...
 * toolbar-like components without overloading interface with lots of panels and
 * different layouts.
 *
 * @author Mikle Garin
 */

public class ToolbarLayout extends AbstractLayoutManager implements
        SwingConstants {
    ...
    // Forces component to fill all the space left between leading and trailing
    // sides
    ...
    /**
     * Start-end parts spacing This one does not affect layout if there are any
     * components in FILL part
     */
    ...
    /**
     * Layout sides margin In case this value is null component border is taken
    ...
    public void addComponent(final Component component, final Object constraints) {
        ...
        if (value != null && !value.trim().equals("") && !value.equals(START)
                && !value.equals(MIDDLE) && !value.equals(FILL)
                && !value.equals(END)) {
    ...
    public Dimension preferredLayoutSize(final Container parent) {
        ...
        final Dimension ps = new Dimension(insets.left + insets.right,
        ...
        for (int i = 0; i < componentCount; i++) {
            ...
            if (orientation == HORIZONTAL) {
                ...
                ps.height = Math.max(ps.height, cps.height + insets.top
                        + insets.bottom);
            } else {
                ps.width = Math.max(ps.width, cps.width + insets.left
                        + insets.right);
                ps.height += cps.height
                        + (i < componentCount - 1 ? spacing : 0);
            }
        }
        
        // Additional spacing between start and end parts
        final boolean addPartsSpacing = hasElement(START) && hasElement(END)
                && !hasElement(MIDDLE) && !hasElement(FILL);
        if (orientation == HORIZONTAL) {
        ...
        if (orientation == HORIZONTAL) {
            if (parent.getComponentOrientation().isLeftToRight()) {
                ...
                for (int i = 0; i < parent.getComponentCount(); i++) {
                    ...
                    if (constraints.get(component) == null
                        ...
                        component
                                .setBounds(startX, insets.top, ps.width,
                                        parent.getHeight() - insets.top
                                                - insets.bottom);
                ...
                if (hasElement(END)) {
                    for (int i = parent.getComponentCount() - 1; i >= 0; i--) {
                        final Component component = parent.getComponent(i);
                        if (constraints.get(component) != null
                                && constraints.get(component).equals(END)) {
                            final Dimension ps = component.getPreferredSize();
                            endX -= ps.width;
                            component.setBounds(endX, insets.top, ps.width,
                                    parent.getHeight() - insets.top
                                            - insets.bottom);
                            endX -= spacing;
                        }
                    ...
                    for (final Component component : parent.getComponents()) {
                        if (constraints.get(component) != null) {
                            if (constraints.get(component).equals(MIDDLE)) {
                                final Dimension ps = component
                                        .getPreferredSize();
                                component.setBounds(
                                        Math.max(startX, (startX + endX) / 2
                                                - ps.width / 2), insets.top,
                                        Math.min(ps.width, endX - startX),
                                        parent.getHeight() - insets.top
                                                - insets.bottom);
                            } else if (constraints.get(component).equals(FILL)) {
                                component.setBounds(startX, insets.top,
                ...
                if (hasElement(END)) {
                    for (int i = parent.getComponentCount() - 1; i >= 0; i--) {
                        ...
                        if (constraints.get(component) != null
                                && constraints.get(component).equals(END)) {
                            final Dimension ps = component.getPreferredSize();
                            component.setBounds(startX, insets.top, ps.width,
                                    parent.getHeight() - insets.top
                                            - insets.bottom);
                            startX += ps.width + spacing;
                        }
                ...
                for (int i = 0; i < parent.getComponentCount(); i++) {
                    ...
                    if (constraints.get(component) == null
                        ...
                        endX -= ps.width;
                        component
                                .setBounds(endX, insets.top, ps.width,
                                        parent.getHeight() - insets.top
                                                - insets.bottom);
                        endX -= spacing;
                    }
            ...
            for (int i = 0; i < parent.getComponentCount(); i++) {
                ...
                if (constraints.get(component) == null
                        || constraints.get(component).equals(START)) {
                    final Dimension ps = component.getPreferredSize();
                    component.setBounds(insets.left, startY, parent.getWidth()
                            - insets.left - insets.right, ps.height);
            ...
            if (hasElement(END)) {
                for (int i = parent.getComponentCount() - 1; i >= 0; i--) {
                    ...
                    if (constraints.get(component) != null
                        ...
                        final Dimension ps = component.getPreferredSize();
                        endY -= ps.height;
                        component.setBounds(insets.left, endY,
                ...
                for (final Component component : parent.getComponents()) {
                    if (constraints.get(component) != null) {
                        if (constraints.get(component).equals(MIDDLE)) {
                            ...
                            component.setBounds(
                                    insets.left,
                                    Math.max(startY, (startY + endY) / 2
                                            - ps.height / 2), parent.getWidth()
                                            - insets.left - insets.right,
                                    Math.min(ps.height, endY - startY));
                        } else if (constraints.get(component).equals(FILL)) {
                            component.setBounds(insets.left, startY,
                                    parent.getWidth() - insets.left
                                            - insets.right,
