 ...
 * standart Swing constants.
 *
 * @author Mikle Garin
 */

public class AlignLayout extends AbstractLayoutManager implements
        SwingConstants {
    /**
     ...
     */
    public static final List<Integer> horizontals = Arrays.asList(LEFT, CENTER,
            RIGHT);
    
    /**
     * Vertical alignment constraints.
     */
    public static final List<Integer> verticals = Arrays.asList(TOP, CENTER,
            BOTTOM);
     ...
     *
     * @param hgap
     *            new horizontal gap between components
     */
     ...
     *
     * @param vgap
     *            new vertical gap between components
     */
    ...
    /**
     * Returns whether components should fill all available horizontal space or
     * not.
     *
     * @return true if components should fill all available horizontal space,
     *         false otherwise
     */
    ...
    /**
     * Sets whether components should fill all available horizontal space or
     * not.
     *
     * @param hfill
     *            whether components should fill all available horizontal space
     *            or not
     */
    ...
    /**
     * Returns whether components should fill all available vertical space or
     * not.
     *
     * @return true if components should fill all available vertical space,
     *         false otherwise
     */
     ...
     *
     * @param vfill
     *            whether components should fill all available vertical space or
     *            not
     */
    ...
    protected void illegalArgument() {
        throw new IllegalArgumentException(
                "Cannot add to layout: please specify proper alignment constraints");
    }
    ...
    protected int getHalign(final String name) {
        return name == null ? CENTER : Integer.parseInt(name.substring(0,
                name.indexOf(SEPARATOR)));
    }
    ...
    protected int getValign(final String name) {
        return name == null ? CENTER : Integer.parseInt(name.substring(name
                .indexOf(SEPARATOR) + SEPARATOR.length()));
    }
        ...
        if (parent.getComponentCount() > 1) {
            ...
            for (final int halign : horizontals) {
                for (final int valign : verticals) {
                    ...
                    if (size != null) {
                        if (!hfill) {
                            final int width = widths.containsKey(halign) ? widths
                                    .get(halign) : 0;
                            widths.put(halign, Math.max(width, size.width));
                        }
                        if (!vfill) {
                            final int height = widths.containsKey(valign) ? widths
                                    .get(valign) : 0;
    ...
    /**
     * Returns size for the side specified by horizontal and vertical
     * alignments.
     *
     * @param parent
     *            container
     ...
     */
    protected Dimension getSideSize(final Container parent, final int halign,
            final int valign) {
    ...
    public void layoutContainer(final Container parent) {
        ...
        for (final Component component : parent.getComponents()) {
            ...
            // Placing component
            component.setBounds(x, y, hfill ? cw : ps.width, vfill ? ch
                    : ps.height);
        }
    }
}