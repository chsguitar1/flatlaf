...
package org.ocsoft.flatlaf.weblaf.label;
 ...
 * It also provides a few additional constructors nad methods to setup the
 * label.
 * <p/>
...
public class WebLabel extends JLabel implements Styleable, EventMethods,
        ToolTipMethods, LanguageMethods, FontMethods<WebLabel>,
    ...
    /**
     * todo This constructor causes compile-time issues on JDK7 and replaced
     * with its simple version. todo Reference to the issue:
     * https://github.com/mgarin/weblaf/issues/175 todo Return better
     * constructor when fully switched to JDK8+ support only
     */
    // /**
    // * Constructs label with the specified preferences.
    // *
    ...
    // * @param data language data, may not be passed
    // */
    // public WebLabel ( final String text, final Icon icon, final Object...
    // data )
    // {
     ...
     * @return this label
     */
    public WebLabel setMargin(final int top, final int left, final int bottom,
            final int right) {
    ...
    public void updateUI() {
        if (getUI() == null || !(getUI() instanceof WebLabelUI)) {
            try {
                setUI((WebLabelUI) ReflectUtils
                        .createInstance(FlatLafSettings.labelUI));
     ...
     */
    @Override
    public WebLabel setFontSizeAndStyle(final int fontSize, final boolean bold,
