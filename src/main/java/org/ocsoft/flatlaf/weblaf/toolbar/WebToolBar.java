...
public class WebToolBar extends JToolBar implements ShapeProvider,
        SizeMethods<WebToolBar>, LanguageContainerMethods {
    public WebToolBar() {
        super();
    }
    ...
    public WebSeparator addSeparator(final String constrain, final int spacing) {
        ...
        final WebSeparator separator = new WebSeparator(hor ? VERTICAL
                : HORIZONTAL);
        add(separator.setMargin(hor ? 0 : spacing, hor ? spacing : 0, hor ? 0
                : spacing, hor ? spacing : 0), constrain);
        return separator;
    }
    ...
    public void updateUI() {
        if (getUI() == null || !(getUI() instanceof WebToolBarUI)) {
            try {
                setUI((WebToolBarUI) ReflectUtils
                        .createInstance(FlatLafSettings.toolBarUI));
