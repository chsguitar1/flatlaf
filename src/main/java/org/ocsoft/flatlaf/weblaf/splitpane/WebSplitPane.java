...
package org.ocsoft.flatlaf.weblaf.splitpane;
...
public class WebSplitPane extends JSplitPane {
    ...
    public WebSplitPane setMargin(final int top, final int left,
    ...
    public double getProportionalDividerLocation() {
        final int l = getOrientation() == WebSplitPane.HORIZONTAL_SPLIT ? getWidth()
                : getHeight();
        return Math.max(0.0, Math.min((double) getDividerLocation()
                / (l - getDividerSize()), 1.0));
    }
    ...
    public void updateUI() {
        if (getUI() == null || !(getUI() instanceof WebSplitPaneUI)) {
            try {
                setUI((WebSplitPaneUI) ReflectUtils
                        .createInstance(FlatLafSettings.splitPaneUI));
