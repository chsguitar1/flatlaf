...
package org.ocsoft.flatlaf.weblaf.tree;
 ...
 * There is also a set of additional methods to simplify some operations with
 * tree.
 * <p/>
...
public class WebTree<E extends DefaultMutableTreeNode> extends JTree implements
        EventMethods, SettingsMethods, FontMethods<WebTree<E>> {
    /**
     * Bound property name for tree data provider. Data provider is not
     * supported by WebTree, but it is a base for various extensions so property
     * is located here.
     */
    public final static String TREE_DATA_PROVIDER_PROPERTY = "dataProvider";
    
    /**
     * Bound property name for tree filter. Filtering is not supported by
     * WebTree, but it is a base for various extensions so property is located
     * here.
     */
    public final static String TREE_FILTER_PROPERTY = "filter";
    
    /**
     * Bound property name for tree comparator. Sorting is not supported by
     * WebTree, but it is a base for various extensions so property is located
     * here.
     */
    ...
    /**
     * Discontiguous selection mode. Any amount of nodes can be selected
     * anywhere.
     */
    public static final int DISCONTIGUOUS_TREE_SELECTION = TreeSelectionModel.DISCONTIGUOUS_TREE_SELECTION;
    
    /**
     * Tree cell editor listeners. These listeners act separately from the cell
     * editor and will be moved to new tree cell editor automatically on set.
     */
    protected List<CellEditorListener> cellEditorListeners = new ArrayList<CellEditorListener>(
            1);
    
    /**
     * Listener that forces tree to scroll view to selection. It is disabled by
     * default and null in that case.
     */
    protected TreeSelectionListener scrollToSelectionListener = null;
    
    /**
     * Special state provider that can be set to check whether or not specific
     * nodes are editable.
     */
    ...
    /**
     * Adds tree cell editor listener. These listeners act separately from the
     * cell editor and will be moved to new tree cell editor automatically on
     * set.
     *
     * @param listener
     *            cell editor listener to add
     */
    ...
    /**
     * Returns special state provider that can be set to check whether or not
     * specific nodes are editable. By default this provider is not specified
     * and simply ignored by the tree.
     *
     * @return special state provider that can be set to check whether or not
     *         specific nodes are editable
     */
    ...
    /**
     * Sets special state provider that can be set to check whether or not
     * specific nodes are editable. You can provide null to disable this state
     * check at all.
     *
     * @param stateProvider
     *            special state provider that can be set to check whether or not
     *            specific nodes are editable
     */
    ...
    public boolean isPathEditable(final TreePath path) {
        return super.isPathEditable(path)
                && isNodeEditable((E) path.getLastPathComponent());
    }
    
    /**
     * Returns whether the specified tree node is editable or not. This method
     * is an improved version of default JTree method isPathEditable.
     *
     * @param node
    ...
    public boolean isNodeEditable(final E node) {
        return editableStateProvider == null
                || editableStateProvider.provide(node);
    }
     ...
     *
     * @param listener
     *            cell editor listener to remove
     */
    ...
    /**
     * Expands all tree nodes in a single call. It is not recommended to expand
     * large trees this way since that might cause huge interface lags.
     */
    ...
    /**
     * Expands all tree nodes accepted by filter in a single call. It is not
     * recommended to expand large trees this way since that might cause huge
     * interface lags.
     *
     * @param shouldExpand
     *            expand filter
    ...
    /**
     * Expands all child nodes until the specified max structure depth is
     * reached.
     *
     * @param node
     ...
     *            max depth level
     */
    private void expandAllImpl(final E node, final int currentDepth,
            final int maxDepth) {
    ...
    public Rectangle getNodeBounds(final List<E> nodes) {
        if (nodes == null || nodes.size() == 0) {
            ...
            for (final E node : nodes) {
                combined = GeometryUtils.getContainingRect(combined,
                        getNodeBounds(node));
            }
            return combined;
        }
    ...
    /**
     * Returns full row bounds including the space before the node and after the
     * node.
     *
     * @param node
     *            node to retrieve full row bounds for
     * @return full row bounds including the space before the node and after the
     *         node
     */
     ...
     *
     * @param row
     *            row to retrieve full bounds for
     * @return full row bounds including the space before the node and after the
     *         node
     */
     ...
     * This will include nodes which are fully and partially visible in tree
     * area.
     *
    ...
    /**
     * Returns first visible leaf node from the top of the tree. This doesn't
     * include nodes under collapsed paths but does include nodes which are not
     * in visible rect.
     *
    ...
    /**
     * Selects row next to currently selected. First row will be selected if
     * none or last row was selected.
     */
    ...
    /**
     * Selects row next to currently selected. First row will be selected if
     * none was selected. First row will be selected if last row was selected
    ...
    /**
     * Selects row next to currently selected. Last row will be selected if none
     * or first row was selected.
     */
    ...
    /**
     * Selects row previous to currently selected. Last row will be selected if
     * none or last was selected. Last row will be selected if first row was
     * selected and cycling is allowed.
     */
     ...
     *
     * @param nodes
     *            list into which all nodes should be collected
     * @param node
    ...
    /**
     * Sets whether multiply nodes selection allowed or not. This call simply
     * changes selection mode according to provided value.
     *
     * @param allowed
     *            whether multiply nodes selection allowed or not
     */
    public void setMultiplySelectionAllowed(final boolean allowed) {
        setSelectionMode(allowed ? DISCONTIGUOUS_TREE_SELECTION
     ...
     *
     * @param scroll
     *            whether tree should automatically scroll to selection or not
     */
    ...
    /**
     * Updates all visible nodes. This might be used to update node sizes if
    ...
    /**
     * Returns tree expansion and selection states. Tree nodes must be instances
     * of UniqueNode class.
     *
    ...
    /**
     * Restores tree expansion and selection states. Tree nodes must be
     * instances of UniqueNode class.
     *
     * @param treeState
     ...
     *            whether to restore selection states or not
     */
    public void setTreeState(final TreeState treeState,
     ...
     *
     * @return true if tree should auto-expand nodes on selection, false
     *         otherwise
     */
     ...
     *
     * @param autoExpand
     *            whether tree should auto-expand nodes on selection or not
     */
    ...
    /**
     * Returns whether tree should auto-expand single child nodes or not. If set
     * to true when any node is expanded and there is only one single child node
     * in it - it will be automatically expanded.
     *
     * @return true if tree should auto-expand single child nodes, false
     *         otherwise
     */
    ...
    /**
     * Sets whether tree should auto-expand single child nodes or not. If set to
     * true when any node is expanded and there is only one single child node in
     * it - it will be automatically expanded.
     *
     * @param autoExpand
     *            whether tree should auto-expand single child nodes or not
     */
     ...
     *
     * @return true if rollover selection is enabled for this list, false
     *         otherwise
     */
     ...
     *
     * @param enabled
     *            whether rollover selection is enabled for this list or not
     */
     ...
     *
     * @param highlight
     *            whether tree should highlight rollover node or not
     */
     ...
     *
     * @param paint
     *            whether tree should paint structure lines or not
     */
    ...
    /**
     * Returns whether selection should be web-colored or not. In case it is not
    ...
    /**
     * Sets whether selection should be web-colored or not. In case it is not
     * web-colored selectionBackgroundColor value will be used as background
     * color.
     *
     * @param webColored
    ...
    /**
     * Returns selection background color. It is used only when
     * webColoredSelection is set to false.
     *
    ...
    /**
     * Sets selection background color. It is used only when webColoredSelection
     * is set to false.
     *
     * @param color
     ...
     *
     * @param dropCellShadeWidth
     *            new drop cell highlight shade width
     */
    ...
    public void updateUI() {
        if (getUI() == null || !(getUI() instanceof WebTreeUI)) {
            try {
                setUI((WebTreeUI) ReflectUtils
                        .createInstance(FlatLafSettings.treeUI));
    ...
    public void repaint(final int from, final int to) {
        ...
        final Rectangle rect = GeometryUtils.getContainingRect(fromBounds,
                toBounds);
        if (rect != null) {
            repaint(rect);
        }
    ...
    public void repaint(final List<E> nodes) {
        if (nodes != null && nodes.size() > 0) {
            Rectangle summ = null;
            for (final E node : nodes) {
                summ = GeometryUtils.getContainingRect(summ,
                        getNodeBounds(node));
            }
            if (summ != null) {
                repaint(summ);
     ...
     */
    @Override
    public WebTree<E> setFontSizeAndStyle(final int fontSize,
    ...
    /**
     * Returns a TreeModel wrapping the specified object. If the object is an
     * array of Object, Hashtable or Vector then a new root node is created with
     * each of the incoming objects as children. Otherwise, a new root is
     * created with the specified object as its value.
     *
     * @param value
