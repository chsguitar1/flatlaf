...
import org.ocsoft.flatlaf.extended.tree.AsyncNodeState;
import org.ocsoft.flatlaf.extended.tree.LoaderIconType;
import org.ocsoft.flatlaf.extended.tree.WebAsyncTreeStyle;

/**
 * Custom UniqueNode for WebAsyncTree. In addition to UniqueNode it contains a
 * loader icon and busy state indicator.
 *
 * @author Mikle Garin
 */

public abstract class AsyncUniqueNode extends UniqueNode implements
        Serializable {
    /**
     * Special separate loader icon for each tree node. This is required to
     * provide separate image observers to optimize tree repaints around the
     * animated icon.
     */
    ...
    /**
     * Sets async node state. Do not change this value on your own since that
     * might break the tree.
     *
     * @param state
     *            new async node state
     */
    ...
    protected ImageIcon createLoaderIcon() {
        return WebAsyncTreeStyle.loaderIconType.equals(LoaderIconType.none) ? null
                : ResourceLoader.loadIcon("tree/"
                        + WebAsyncTreeStyle.loaderIconType + ".gif");
    }
