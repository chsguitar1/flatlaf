 ...
 * add/remove methods and works with typed elements.
 *
 * @author Mikle Garin
 */

public class WebTreeModel<E extends DefaultMutableTreeNode> extends
        DefaultTreeModel {
    /**
     * Constructs tree model with a specified node as root.
     *
     * @param root
     *            TreeNode object that is the root of the tree
     */
    ...
    public void removeNodesFromParent(final List<E> nodes) {
        ...
        // final Map<E, List<Integer>> removedNodes = new HashMap<E,
        // List<Integer>> ();
    ...
    public void valueForPathChanged(final TreePath path, final Object newValue) {
        final MutableTreeNode aNode = (MutableTreeNode) path
    ...
    public void updateNode(final E node) {
        if (node != null) {
            fireTreeNodesChanged(WebTreeModel.this, getPathToRoot(node), null,
                    null);
        }
    }
    ...
    public void updateNodeStructure(final E node) {
        if (node != null) {
            fireTreeStructureChanged(WebTreeModel.this, getPathToRoot(node),
                    null, null);
        }
    }
    
    /**
     * Forces update of all visible node sizes and view. This call might be
     * useful if renderer changes dramatically and you have to update the whole
     * tree.
     */
    public void updateTree() {
        fireTreeStructureChanged(WebTreeModel.this, getRootNode().getPath(),
                null, null);
    }
}