...
import org.ocsoft.flatlaf.weblaf.list.editor.DefaultListCellEditor;
import org.ocsoft.flatlaf.weblaf.list.editor.ListCellEditor;
import org.ocsoft.flatlaf.weblaf.list.editor.ListEditListener;
...
public class WebList extends JList implements EventMethods,
    ...
    /**
     * Whether list allows an empty selection or not. This setting doesn't force
     * initial selection though.
     */
    ...
    /**
     * Sets whether multiply values selection allowed or not. This call simply
     * changes selection mode according to provided value.
     *
     * @param allowed
     *            whether multiply values selection allowed or not
     */
    public void setMultiplySelectionAllowed(final boolean allowed) {
        setSelectionMode(allowed ? ListSelectionModel.MULTIPLE_INTERVAL_SELECTION
                : ListSelectionModel.SINGLE_SELECTION);
    }
    
    /**
     * Returns specific web list cell renderer or null if a custom non-web
     * renderer is installed.
     *
     * @return specific web list cell renderer or null if a custom non-web
     *         renderer is installed
     */
    public WebListCellRenderer getWebListCellRenderer() {
        final ListCellRenderer renderer = getCellRenderer();
        return renderer instanceof WebListCellRenderer ? (WebListCellRenderer) renderer
                : null;
    }
    ...
    /**
     * Selects all specified values. Values which are not in the list are simply
     * ignored. This method might be slow in case model cannot provide all
     * separate values quickly.
     *
     * @param values
     *            values to select
     */
    ...
    public boolean stopCellEditing() {
        ...
        return cellEditor != null && cellEditor.isEditing()
                && cellEditor.stopEdit(WebList.this);
     ...
     *
     * @return true if some list cell is being edited at the moment, false
     *         otherwise
     */
    ...
    public void setUnselectable(final boolean unselectable) {
        ...
        setSelectionModel(unselectable ? new DefaultListSelectionModel()
    ...
    public void updateUI() {
        if (getUI() == null || !(getUI() instanceof WebListUI)) {
            try {
                setUI((WebListUI) ReflectUtils
                        .createInstance(FlatLafSettings.listUI));
    ...
    public void fireEditStarted(final int index) {
        editedCell = index;
        for (final ListEditListener listener : CollectionUtils
                .copy(editListeners)) {
     ...
     *            new cell value
     */
    public void fireEditFinished(final int index, final Object oldValue,
     ...
     */
    @Override
    public WebList setFontSizeAndStyle(final int fontSize, final boolean bold,
